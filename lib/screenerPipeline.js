...
import { budgetFor, freeUpLocalScans, payloadChars, reportStorageOutcome, safeRead, safeWrite, STORAGE_KEYS } from "@/lib/localState";
...
import { countryRs, countryRsSortValue, countryRsValue } from "@/lib/countryRs";
import { themeRs, themeRsSortValue, themeRsValue } from "@/lib/themeRs";
import { normalizeScanErrorGroups } from "@/lib/scanErrorGroups";
import { ipoDateSortValue, ipoDesdeSalidaSortValue } from "@/lib/ipoDiscoveryView";
...
import { canUseScreenPassedFastPath, populationNeedsRescore } from "@/lib/screenerFilterFastPath";
...
function scanSettingsSignatureParts(marketsValue = [], manualValue = "", scanModeValue = "all") {
  ...
  return { markets: marketsPart, manual: manualPart, mode: modePart };
}

function scanSettingsSignature(marketsValue = [], manualValue = "", scanModeValue = "all") {
  const parts = scanSettingsSignatureParts(marketsValue, manualValue, scanModeValue);
  return `v1|markets=${parts.markets}|manual=${parts.manual}|mode=${parts.mode}`;
}

function parseScanSettingsSignature(signature = "") {
  const match = /^v1\|markets=([^|]*)\|manual=([^|]*)\|mode=(.*)$/.exec(String(signature || ""));
  if (!match) return null;
  return { markets: match[1], manual: match[2], mode: match[3] };
}

/**
 * ¿El scan cargado está stale respecto a markets/manual/scanMode actuales?
 *
 * Retención #4 (Global→US): si la mesa ya alinea con la selección, un
 * settingsSignature que aún lleva mercados de la selección anterior (p. ej.
 * Global) NO debe disparar el banner «criterios de cobertura cambiaron».
 * En ese caso solo cuentan manual/scanMode.
 */
function isScanSettingsStale({
  ...
  if (!settingsSignature) return false;
  const current = scanSettingsSignature(markets, manual, scanMode);
  if (settingsSignature === current) return false;
  if (marketsMisaligned) return true;
  const parsed = parseScanSettingsSignature(settingsSignature);
  if (!parsed) return true;
  const parts = scanSettingsSignatureParts(markets, manual, scanMode);
  return parsed.manual !== parts.manual || parsed.mode !== parts.mode;
}

/** Firma vigente tras alinear mesa↔selección (cura drift de mercados en signature). */
function alignedScanSettingsSignature({
  settingsSignature = null,
  markets = [],
  manual = "",
  scanMode = "all",
  ...
  const current = scanSettingsSignature(markets, manual, scanMode);
  if (!settingsSignature || settingsSignature === current) return current;
  const parsed = parseScanSettingsSignature(settingsSignature);
  if (!parsed) return current;
  const parts = scanSettingsSignatureParts(markets, manual, scanMode);
  // No pisar staleness real de manual/mode: solo reescribir el componente markets.
  if (parsed.manual !== parts.manual || parsed.mode !== parts.mode) return settingsSignature;
  return current;
}
...
function sortMetric(row = {}, key, settings = {}) {
  ...
  if (key === "weeklyCountryRsRating") return countryRsSortValue(row);
  if (key === "weeklyThemeRsRating") return themeRsSortValue(row);
  if (key === "rsRating") return rsBenchmarkValue(row) ?? 0;
  return firstFinite(row[key]) ?? 0;
}

const RS_TABLE_SORT_KEY = "rsGlobalPct";

function rowSortTieBreak(a = {}, b = {}) {
  return String(a.symbol || "").localeCompare(String(b.symbol || ""), "en", { sensitivity: "base" });
}

/**
 * Comparador estable para la tabla. En RS: con dato arriba, Sin dato al final
 * (también en ascendente); empates por símbolo.
 */
function compareRowsForSort(a = {}, b = {}, { sort = "", sortAsc = false, settings = {} } = {}) {
  if (sort === RS_TABLE_SORT_KEY) {
    const aHas = canonicalRs(a).available;
    const bHas = canonicalRs(b).available;
    if (aHas !== bHas) return aHas ? -1 : 1;

    if (aHas && bHas) {
      const aVal = canonicalRsValue(a);
      const bVal = canonicalRsValue(b);
      if (aVal !== bVal) {
        const diff = bVal - aVal;
        return sortAsc ? -diff : diff;
      }
    }
    return rowSortTieBreak(a, b);
  }
  if (sort === "weeklyCountryRsRating") {
    const aHas = countryRs(a).available;
    const bHas = countryRs(b).available;
    if (aHas !== bHas) return aHas ? -1 : 1;
    if (aHas && bHas) {
      const aVal = countryRsValue(a);
      const bVal = countryRsValue(b);
      if (aVal !== bVal) {
        const diff = bVal - aVal;
        return sortAsc ? -diff : diff;
      }
  ...
  if (sort === "weeklyThemeRsRating") {
    const aHas = themeRs(a).available;
    const bHas = themeRs(b).available;
    if (aHas !== bHas) return aHas ? -1 : 1;
    if (aHas && bHas) {
      const aVal = themeRsValue(a);
      const bVal = themeRsValue(b);
      if (aVal !== bVal) {
        const diff = bVal - aVal;
        return sortAsc ? -diff : diff;
      }
  ...
  if (sort === "ipoDate") {
    const aDate = ipoDateSortValue(a);
    const bDate = ipoDateSortValue(b);
  ...
  if (sort === "ipoDesdeSalidaPct") {
    const aVal = ipoDesdeSalidaSortValue(a);
    const bVal = ipoDesdeSalidaSortValue(b);
  ...
  const diff = sortMetric(b, sort, settings) - sortMetric(a, sort, settings);
  if (diff !== 0) return sortAsc ? -diff : diff;
  return rowSortTieBreak(a, b);
}
...
function defaultSortForSettings(set = {}) {
  if (set.setupMode === "weakness") return "weaknessScore";
  if (set.setupMode === "ipoRecent") return "ipoDate";
  return DEFAULT_PERFORMANCE_PERIOD;
}

function sortRowsForMode(list, set = {}, key = "") {
  const sortKey = key || defaultSortForSettings(set);
  return [...list].sort((a, b) => compareRowsForSort(a, b, { sort: sortKey, sortAsc: false, settings: set }));
}
function perfNow() {
...
function splitByPrecomputedScreen(rows = []) {
  const passed = [];
  const rejections = [];
  for (const row of rows) {
    if (row.screenPassed === true) passed.push(row);
    else {
      const field = row.screenRejectField || "";
      rejections.push({
        symbol: row.symbol,
        ...rejectReason(sharedRejectKey(field), row.screenRejectReason || "No cumple filtro", field),
      });
    }
  }
...
function filterAnalyzedRows(analyzedRows = [], set = {}, context = {}) {
  ...
  for (const row of analyzedRows) {
    ...
    if (!gate.passed) {
      ...
      // No clonar la fila: el qualityGate se recalcula barato si hace falta.
      // Clonar 3.309 filas gordas en cada gesto era parte del 1,3 s medido.
      qualityPassed.push(row);
    }
  }
  const rescored = populationNeedsRescore(qualityPassed);
  const sectorized = rescored ? sectorize(qualityPassed) : qualityPassed;
  const usedFastPath = canUseScreenPassedFastPath(sectorized, set, context);
  const hardFilterSplit = usedFastPath ? splitByPrecomputedScreen(sectorized) : splitByFilter(sectorized, set);
  ...
  return {
    ...
    sectorized,
    filterMs,
    path: usedFastPath ? "screen-passed" : "rules",
    rescored,
...
// ── Proyección de PERSISTENCIA local ───────────────────────────────────────
// La forma en que una fila puede vivir en localStorage. compactResearchRow no
// basta: la fila del scan servidor arrastra objectiveMetricAudit (~16 KB) y
// decisionTrace (~6 KB) — medido el 2026-08-15: 39,5 KB por fila "compacta",
// 5 veces la fila ligera del nocturno. Aquí la fila queda en la proyección
// ligera del escaneo (lib/scanLightProjection.js, la que el test de contrato
// garantiza que puede filtrar, pintar la tabla y ordenar las Listas) más la
// identidad que las superficies de revisión muestran. Lo descartado se
// recalcula al usarlo (decisionTraceForRow) o viaja resumido
// (metricAuditFlags); la fila completa siempre puede volver del servidor.
//
// Esta proyección es SOLO para escribir en localStorage. El export de
// auditoría (decisionAuditJson) sigue usando compactRowsForSession: un JSON
// de auditoría sin la auditoría dentro no sirve.
const PERSIST_IDENTITY_FIELDS = [
  "exchange", "currency", "businessEs", "businessSummary", "theme",
  "weaknessLabel", "rsQualityLabel", "compositeLabel", "objectiveLabel",
  "priceFreshnessLabel", "ipoCategory", "benchmarkSymbol", "chartEstimated",
  "compositeReasons", "compositeRisks", "metricAuditFlags",
  "screenPassed", "rowProjection", "screenRejectField", "screenRejectReason",
  // El quality gate del re-filtrado (lib/qualityGate.js) exige histórico: sin
  // chartBarsCount rechazaba las 500 filas rehidratadas con "histórico 0/180"
  // y la restauración quedaba en 0 visibles [REPRODUCIDO 2026-08-15]. El test
  // espejo de tests/persistenciaNavegador.test.js fija que la fila persistida
  // produce el MISMO veredicto de filtro que la completa.
  "chartBarsCount",
];
function persistRowForBrowser(row = null) {
  if (!row || typeof row !== "object") return row;
  const light = scanLightMetrics(row);
  // scanLightMetrics marca screenPassed:false (nació para filas rechazadas);
  // aquí la fila conserva su marca real si la trae, y si no, ninguna.
  delete light.screenPassed;
  delete light.rowProjection;
  delete light.screenRejectField;
  delete light.screenRejectReason;
  for (const key of PERSIST_IDENTITY_FIELDS) {
    const value = row[key];
    if (value !== undefined && value !== null) light[key] = value;
  }
  return light;
}
function persistRowsForBrowser(rows = []) {
  return Array.isArray(rows) ? rows.map(persistRowForBrowser) : [];
}

// La cola de revisión, escrita con su presupuesto y degradación: filas en
// proyección de persistencia; intenta conservar chartPreview aunque supere el
// presupuesto blando si localStorage lo admite; libera snapshots viejos antes de
// quitar miniaturas; strip solo como último recurso. Reporta UNA vez el
// desenlace (sin miniaturas o imposible) para que la superficie activa muestre
// el aviso.
function persistReviewQueue(review = {}) {
  const light = { ...review, rows: persistRowsForBrowser(review?.rows || []) };
  const withoutPreviews = () => ({
    ...light,
    rows: light.rows.map(({ chartPreview, ...rest }) => rest),
    storageNote: "Cola guardada sin miniaturas por falta de espacio.",
  });
  const tryWrite = (payload) => safeWrite(STORAGE_KEYS.review, payload, { silent: true });

  if (tryWrite(light)) return true;

  freeUpLocalScans(1);
  if (tryWrite(light)) return true;

  const stripped = withoutPreviews();
  ...
  reportStorageOutcome({ key: STORAGE_KEYS.review, failed: true });
  return false;
}

// Repone chartPreview en filas de cola review desde otra fuente en RAM (mesa o
// último scan) sin brief ni /api/chart. Solo rellena huecos: no pisa previews
// que la fila ya trae.
function mergeReviewRowChartPreviews(rows = [], sourceRows = []) {
  if (!Array.isArray(rows) || !rows.length || !Array.isArray(sourceRows) || !sourceRows.length) {
    return rows;
  }
  const previewBySymbol = new Map();
  for (const row of sourceRows) {
    const symbol = String(row?.symbol || "").toUpperCase();
    if (!symbol || previewBySymbol.has(symbol)) continue;
    if (!Array.isArray(row.chartPreview) || row.chartPreview.length < 2) continue;
    previewBySymbol.set(symbol, row.chartPreview);
  }
  if (!previewBySymbol.size) return rows;
  let changed = false;
  const merged = rows.map((row) => {
    const symbol = String(row?.symbol || "").toUpperCase();
    if (!symbol) return row;
    if (Array.isArray(row.chartPreview) && row.chartPreview.length >= 2) return row;
...
// Muestra repartida por todo el orden recibido: uno de cada k, nunca "los
// primeros". Las filas del escaneo llegan ordenadas por rank_index, que ordena
// por puntuación: quedarse con el principio deja fuera todos los valores
// débiles y cualquier filtro que los busque devuelve vacío sin motivo
// aparente. Mismo criterio que usa el servidor cuando no caben todas las
// páginas (scanResultPageOffsets, app/api/scans/route.js).
export function spreadSample(rows = [], max = 0) {
  const list = Array.isArray(rows) ? rows : [];
  const limit = Number(max) || 0;
  if (limit <= 0 || list.length <= limit) return list;
  const step = list.length / limit;
  const sample = [];
  for (let index = 0; index < limit; index += 1) sample.push(list[Math.floor(index * step)]);
  return sample;
}

// El universo entero NO cabe en localStorage y eso no es negociable: el
// nocturno del 17 de agosto de 2026 son 3.312 filas que, en proyección de
// persistencia, ocupan del orden de 25 M de caracteres frente a los 4,5 M de
// presupuesto de esta clave. Se transporta el universo (para poder filtrarlo)
// y se guarda una muestra repartida (para tener respaldo local sin sesgo).
// El escaneo guardado deja constancia: rowsReturned/rowsTruncated/rowsSampled
// son los mismos campos que usa el aviso de la pantalla.
function fitScanRowsToBudget(scan, budget) {
  const rows = Array.isArray(scan?.rows) ? scan.rows : [];
  if (!rows.length || !Number.isFinite(budget) || budget <= 0) return scan;
  let candidate = scan;
  // Tres pasadas: la primera estima con el peso medio por fila, las siguientes
  // corrigen si la estimación se quedó corta (las filas no pesan todas igual).
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const chars = payloadChars(candidate);
    if (chars <= budget) break;
    const currentRows = candidate.rows;
    const overhead = payloadChars({ ...candidate, rows: [] });
    const perRow = Math.max((chars - overhead) / currentRows.length, 1);
    const max = Math.max(1, Math.floor((budget - overhead) / perRow));
    if (max >= currentRows.length) break;
    const sample = spreadSample(currentRows, max);
    candidate = {
      ...candidate,
      rows: sample,
      rowsAvailable: Number(candidate.rowsAvailable) || rows.length,
      rowsReturned: sample.length,
      rowsTruncated: true,
      rowsSampled: true,
      storageNote: "Copia local guardada con una muestra repartida del escaneo por falta de espacio.",
    };
  }
  return candidate;
}

// Encaja la lista de snapshots locales en su presupuesto: filas en proyección
// de persistencia, el más reciente recortado a una muestra repartida si él
// solo no cabe, y después solo los que quepan enteros.
function fitScansForBrowser(scans = [], budget = budgetFor(STORAGE_KEYS.scans)) {
  const list = (Array.isArray(scans) ? scans : [])
    .filter(Boolean)
    .map((scan) => (Array.isArray(scan?.rows) ? { ...scan, rows: persistRowsForBrowser(scan.rows) } : scan));
  if (!Number.isFinite(budget) || budget <= 0 || !list.length) return list;
  const kept = [];
  let used = 0;
  for (const scan of list) {
    const fitted = kept.length ? scan : fitScanRowsToBudget(scan, budget);
    const chars = payloadChars(fitted);
    if (kept.length && used + chars > budget) break;
    kept.push(fitted);
    used += chars;
  }
  return kept;
...
// La etapa la calcula lib/weeklyStage.js y solo él. Esta función devuelve la
// etiqueta ya calculada; NO clasifica.
//
// Antes tenía aquí su propio árbol de decisión sobre las medias diarias de
// 50/150/200, que producía las palabras "Stage 2"/"Base / transicion" sin
// mirar la media de 30 semanas. Era una de las siete implementaciones de
// etapa que encontró docs/auditoria-etapas-2026-08-16.md (A.1, caso 3): un
// valor podía leerse "Etapa 2" aquí y otra cosa en la columna de al lado.
function stageLabel(r) {
  return r?.weeklyStageLabel || "Sin dato";
}
...
function sharedRejectKey(field = "") {
  ...
  if (["minRsRating", "minRsBenchmarkRating", "minRsCountryPct", "minThemeRsRating", "minRsSectorPct", "minRsQualityScore", "minSectorScore"].includes(field)) return "relativeStrength";
  if (["minLatestVolume", "minLatestTurnover", "minRelativeVolume", "minVolumeSurgePct", "minUpDownVolRatio", "minVolumeEffectScore", "requireUpVolume"].includes(field)) return "volumeSurge";
  ...
  if (["patternDataStatus", "contractionStructureStatus", "requireContractionsDecreasing", "minContractionCount", "maxContraction1DepthPct", "maxContraction2DepthPct", "maxContraction3DepthPct", "maxLastContractionDepthPct", "maxBaseDepthPct", "maxAbsDistanceToPivotPct", "maxVolumeDryUpRatio", "maxTightness10dPct", "minPatternQualityScore"].includes(field)) return "pattern";
  if (["requireVcpCandidate", "vcpRequireStage2", "vcpWeeklyStageStructure", "vcpMinContractionCount", "vcpMaxLastContractionDepthPct", "vcpMinDistanceToPivotPct", "vcpMaxDistanceToPivotPct", "vcpMaxVolumeDryUpRatio"].includes(field)) return "vcp";
  if (["requireStage2", "requireSma200Up", "requirePriceAboveSma50", "longBiasFloor", "minWeinsteinScore", "minMinerviniScore", "minWeeksAboveSma30w"].includes(field)) return "trend";
  if (["maxDistance20dHigh", "maxDistance50dHigh", "maxDistance52w", "maxDistanceATH", "maxHighsSpreadPct", "maxExtensionSma50"].includes(field)) return "proximity";
  if (["minPerf3m", "minPerf6m", "minPerf12m", "minMomentumScore"].includes(field)) return "momentum";
  if (["minRiskScore", "minVolumeScore", "minTotalScore", "minEpsGrowthProxyScore", "minAdProxyScore", "minWeaknessScore"].includes(field)) return "score";
...
function postFilterRejectReason(row, set) {
  ...
  if ((set.minRsCountryPct || 0) > 0) {
    const crs = countryRs(row);
    if (!crs.available || crs.value === null || crs.value < set.minRsCountryPct) {
      return rejectReason("relativeStrength", `RS país ${crs.available ? crs.value?.toFixed?.(0) : "sin dato"} < ${set.minRsCountryPct || 0}`, "minRsCountryPct");
    }
  }
  if ((set.minThemeRsRating || 0) > 0) {
    const trs = themeRs(row);
    if (!trs.available || trs.value === null || trs.value < set.minThemeRsRating) {
      return rejectReason("relativeStrength", `RS tema ${trs.available ? trs.value?.toFixed?.(0) : "sin dato"} < ${set.minThemeRsRating || 0}`, "minThemeRsRating");
    }
  }
...
export {
  ...
  scanSettingsSignatureParts,
  parseScanSettingsSignature,
  isScanSettingsStale,
  alignedScanSettingsSignature,
  cachedScreenerRow,
  cachedScreenerQuery,
  sortMetric,
  compareRowsForSort,
  ...
  persistRowForBrowser,
  persistRowsForBrowser,
  persistReviewQueue,
  mergeReviewRowChartPreviews,
  fitScansForBrowser,
  failureKind,
