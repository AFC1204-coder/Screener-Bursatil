...
// proyección de persistencia; si no cabe, sin miniaturas; si sigue sin caber,
// libera snapshots locales viejos y reintenta. Reporta UNA vez el desenlace
// (reducida o imposible) para que la superficie activa muestre el aviso.
function persistReviewQueue(review = {}) {
  const light = { ...review, rows: persistRowsForBrowser(review?.rows || []) };
  const budget = budgetFor(STORAGE_KEYS.review);
  ...
  const overBudget = Number.isFinite(budget) && budget > 0 && payloadChars(light) > budget;
  const primary = overBudget ? withoutPreviews() : light;
  if (safeWrite(STORAGE_KEYS.review, primary, { silent: true })) {
    if (overBudget) reportStorageOutcome({ key: STORAGE_KEYS.review, degraded: true });
    return true;
  }
  freeUpLocalScans(1);
  if (safeWrite(STORAGE_KEYS.review, primary, { silent: true })) {
  ...
  if (!overBudget && safeWrite(STORAGE_KEYS.review, withoutPreviews(), { silent: true })) {
    reportStorageOutcome({ key: STORAGE_KEYS.review, degraded: true });
    return true;
  }
