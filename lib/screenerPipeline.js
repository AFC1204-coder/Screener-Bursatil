...
function sortMetric(row = {}, key, settings = {}) {
  ...
  // Ordenar "RS" usa el RS semanal cuando el símbolo está en el ranking; si
  // no lo está, cae al percentil del lote (rsUniverseValue) en vez de tratar
  // la fila como 0 — para ordenación (a diferencia de minRsRating) no hay
  // instrucción de "omitir", así que se mantiene el comportamiento previo
  // para los símbolos sin RS semanal en vez de hundirlos al fondo del orden.
  if (key === "rsGlobalPct") return row.weeklyRsAvailable === true ? row.weeklyRsRating : (rsUniverseValue(row) ?? 0);
