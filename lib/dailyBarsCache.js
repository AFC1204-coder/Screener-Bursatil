...
// ── Guard de valores atípicos de volumen: una barra suelta dentro de un
// payload por lo demás diario no puede tener un volumen desproporcionado
// respecto a sus vecinas ──────────────────────────────────────────────────
//
// EL AGUJERO QUE ESTO CIERRA, medido el 2026-08-20: 32 filas en 27 símbolos
// con barras MENSUALES sueltas incrustadas dentro de payloads por lo demás
// diarios (AAPL, WELL, MSFT con cuatro cada uno; COST, XOM, MCD, 3988.HK con
// una). El guard de cadencia de arriba (nonDailyCadenceReason) opera sobre el
// payload COMPLETO — granularidad declarada y mediana de huecos de TODAS las
// barras — así que nunca ve esto: 1-4 barras contaminadas dentro de un
// payload de cientos no mueven ni la mediana ni la granularidad declarada.
// Las 32 filas se escribieron todas ANTES de este guard (nonDailyCadenceReason,
// commit a24957c, 2026-08-18: la más tardía es del 2026-08-12) — es limpieza,
// no recurrencia observada — pero el hueco en el código sigue abierto: nada
// impedía que volviera a pasar la próxima vez que alguien pida range=MAX/5A
// para uno de estos símbolos.
//
// La señal medida: para ciertas fechas —el día 1 de algunos meses dentro de
// un rango largo— Yahoo devuelve una barra cuyo volumen es la agregación del
// mes en vez de la sesión diaria. Medido contra la sesión inmediatamente
// anterior: COST 2026-01-01 es 91,6x (136.496.800 frente a 1.490.000 del
// 2025-12-31); AAPL trae tres de estas (2025-03-01, 06-01, 09-01) entre
// 48x y 82x. Las de WELL, MSFT, XOM y MCD no se han vuelto a medir barra a
// barra, pero comparten la misma firma: caen en el día 1 del mes y su
// volumen es uno o dos órdenes de magnitud mayor que el habitual del símbolo.
//
// La regla NO asume ningún calendario de mercado (día de la semana, festivo):
// eso rompería sesiones reales legítimas fuera de EE.UU. — los domingos de
// valores .AX son sesiones reales con la fecha desplazada por huso horario
// (docs/daily-bars-barras-mensuales), y .TA/.NS cotizan en días que EE.UU.
// tiene cerrados. En vez de eso, cada barra se compara contra la MEDIANA de
// volumen de sus vecinas más próximas EN EL PROPIO PAYLOAD, exigiendo que
// esas vecinas estén a ≤4 días naturales (para no comparar contra vecinas que
// ya tienen un hueco de calendario grande) y que haya al menos 5 a cada lado
// (para no evaluar los extremos del payload ni barras aisladas de histórico
// profundo sin vecinas diarias alrededor — el caso de ^FCHI/FTNT que hay que
// conservar, ver diseño-contracciones-v3). Una sesión real, sea cual sea su
// mercado, nunca cae aquí: su volumen es del mismo orden que el de sus
// vecinas reales.
//
// Deliberadamente NO rechaza el payload entero (a diferencia del guard de
// cadencia de arriba): descarta solo la barra atípica y escribe el resto.
// Rechazar todo por una sola fecha contaminada perdería cientos de barras
// buenas por una mala — sería peor que el problema que arregla.
const VOLUME_OUTLIER_RATIO = 20;
const VOLUME_OUTLIER_NEIGHBOR_WINDOW = 10;
const VOLUME_OUTLIER_MIN_NEIGHBORS = 5;
const VOLUME_OUTLIER_MAX_GAP_DAYS = 4;
...
// Exportada para que el test pueda ejercitar la regla sin montar un upsert.
// Recibe las barras en el orden del payload (desc por fecha, convención del
// fetcher) y devuelve { bars, dropped }: `bars` es la lista sin las atípicas,
// `dropped` lleva fecha/volumen/línea base de cada una excluida (telemetría).
export function dropVolumeOutlierBars(bars = []) {
  const list = Array.isArray(bars) ? bars : [];
  const times = list.map((bar) => dateMs(bar?.date));
  const dropped = [];
  const kept = list.filter((bar, i) => {
    const volume = numberOrNull(bar?.volume);
    const ms = times[i];
    if (!Number.isFinite(volume) || volume <= 0 || !Number.isFinite(ms)) return true;

    const neighborVolumes = [];
    let cursor = ms;
    for (let j = i - 1; j >= 0 && neighborVolumes.length < VOLUME_OUTLIER_NEIGHBOR_WINDOW; j -= 1) {
      const jMs = times[j];
      if (!Number.isFinite(jMs) || Math.abs(cursor - jMs) / 86400000 > VOLUME_OUTLIER_MAX_GAP_DAYS) break;
      const jVolume = numberOrNull(list[j]?.volume);
      if (Number.isFinite(jVolume) && jVolume > 0) neighborVolumes.push(jVolume);
      cursor = jMs;
    }
    const beforeCount = neighborVolumes.length;
    cursor = ms;
    for (let j = i + 1; j < list.length && neighborVolumes.length - beforeCount < VOLUME_OUTLIER_NEIGHBOR_WINDOW; j += 1) {
      const jMs = times[j];
      if (!Number.isFinite(jMs) || Math.abs(cursor - jMs) / 86400000 > VOLUME_OUTLIER_MAX_GAP_DAYS) break;
      const jVolume = numberOrNull(list[j]?.volume);
      if (Number.isFinite(jVolume) && jVolume > 0) neighborVolumes.push(jVolume);
    ...
    const afterCount = neighborVolumes.length - beforeCount;
    if (beforeCount < VOLUME_OUTLIER_MIN_NEIGHBORS || afterCount < VOLUME_OUTLIER_MIN_NEIGHBORS) return true;

    const baseline = median(neighborVolumes);
    if (!Number.isFinite(baseline) || baseline <= 0 || volume < VOLUME_OUTLIER_RATIO * baseline) return true;

    dropped.push({ date: toDate(bar.date), volume, baseline, ratio: volume / baseline });
    return false;
  });
...
export async function writeDailyBarsCache(symbol, chart = {}, options = {}) {
  ...
  // Guard de valores atípicos de volumen (ver el bloque de comentario junto a
  // dropVolumeOutlierBars): descarta barras sueltas cuyo volumen es un
  // múltiplo desproporcionado de sus vecinas antes de escribir. A diferencia
  // del guard de cadencia de arriba, esto NO rechaza el payload — solo la(s)
  // barra(s) contaminada(s).
  const { bars: outlierFilteredBars, dropped: droppedVolumeOutliers } = dropVolumeOutlierBars(cappedBars);

  const rows = outlierFilteredBars
  ...
  try {
    ...
    return {
      ...
      writeCap,
      droppedVolumeOutliers: droppedVolumeOutliers,
      referenced,
