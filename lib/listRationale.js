...
export const LIST_CONTRACTS = {
  favorites: {
    ...
    text: "Seleccion manual: no implica contrato automatico.",
  },
  leaders: {
    title: "Composite Leaders",
    tone: "bullish",
    label: "Largo coherente",
    text: "Composite alto con sesgo largo; excluye deterioro tecnico y bajismo estructural.",
  },
  rsQuality: {
    title: "RS Quality Leaders",
    tone: "bullish",
    label: "RS de calidad",
    text: "RS Quality y RS universo altos con sesgo largo; excluye deterioro tecnico y bajismo estructural.",
  },
  weakness: {
    title: "Deterioro tecnico",
    tone: "bearish",
    label: "Deterioro",
    text: "Lista defensiva/bajista: aqui si tiene sentido ver debilidad observable.",
  },
  weinstein: {
    title: "Weinstein Leaders",
    tone: "bullish",
    label: "Stage 2",
    text: "Estructura Weinstein/Stage 2 y medias coherentes; excluye deterioro tecnico.",
  },
  minervini: {
    title: "Minervini Leaders",
    tone: "bullish",
    label: "Trend template",
    text: "Trend template, momentum y cercania a maximos; excluye deterioro tecnico y bajismo estructural.",
  },
  nearPivot: {
    title: "Vigilancia pivot",
    tone: "watch",
    label: "Vigilancia",
    text: "Lider fuerte cerca de pivot observable; vigilancia, no plan automatico.",
  },
  ipo: {
    title: "IPO / New Leaders",
    tone: "watch",
    label: "IPO real",
    text: "IPO verificable y fuerza derivada; excluye deterioro tecnico y bajismo estructural.",
  },
  extended: {
    title: "Extended but strong",
    tone: "watch",
    label: "Extendida",
    text: "Fuerte pero extendida sobre SMA50; vigilancia, no entrada automatica.",
  },
  pullback: {
    title: "Pullback to SMA50",
    tone: "watch",
    label: "Pullback sano",
    text: "Lider con retroceso controlado hacia SMA50 y sesgo largo vigente.",
  },
};
...
export function rowPassesListContract(row = {}, listKey = "leaders") {
  ...
  if (listKey === "nearPivot") return total >= 55 && rs >= 55;
  if (listKey === "ipo") return (metric(row, "ipoScore") ?? 0) >= 45 || total >= 50 || rs >= 55;
  if (listKey === "extended") return total >= 70 && Number.isFinite(extSma50) && extSma50 >= 15 && (!Number.isFinite(distance52w) || distance52w >= -20);
  if (listKey === "pullback") return total >= 50 && Number.isFinite(extSma50) && extSma50 >= -3 && extSma50 <= 8;
  return total >= 50;
}
...
export function listInclusionReasons(row = {}, listKey = "leaders", limit = 4) {
  ...
  if (listKey === "rsQuality") {
    ...
    pushMetric(reasons, "Composite", metric(row, "totalScore"));
    pushMetric(reasons, "RS", metric(row, "rsGlobalPct"));
    ...
    const age = monthsSince(snapshotValue(row, "ipoDate"));
...
export function rowReliabilityIssues(row = {}, criteria = {}) {
  ...
  if (Number.isFinite(coverage) && coverage < minCoverageScore) {
  ...
  if (rowValue(row, "setupDisplayDataLimited") === true || rowValue(row, "methodologyBlocksPatternClaim") === true || rowValue(row, "setupDisplayBlocksPatternClaim") === true) {
    issues.push({ key: "dataLimited", label: "metodologia limitada" });
  }

  if (!theme || !sector || !industry || sector === "Sin sector" || industry === "Sin industria") {
    issues.push({ key: "taxonomy", label: "taxonomia incompleta" });
  }
...
export function summarizeListReliability(rows = [], criteria = {}) {
  ...
  const hardWarnings = [
    ...
    summary.missingTaxonomyRows ? `${summary.missingTaxonomyRows} taxonomia incompleta` : "",
  ].filter(Boolean);
  const softWarnings = [
    summary.dataLimitedRows ? `${summary.dataLimitedRows} plan/patron limitado` : "",
  ].filter(Boolean);
  ...
  if (softWarnings.length) {
    return {
      ...
      state: "watch",
      label: "Ranking OK",
      note: `Ranking coherente; ${softWarnings.join(", ")} para no inferir setups automaticamente.`,
    };
  }
  ...
  return {
    ...
    state: "pass",
    label: "Fiable",
    note: "Precio fresco, cobertura suficiente y taxonomia coherente.",
  };
}
...
function listSortValue(row = {}, listKey = "leaders") {
  ...
  return metric(row, "totalScore") ?? 0;
}
