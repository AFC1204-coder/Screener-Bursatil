...
// El "RS" que estas frases mencionan es el MISMO que la columna: el ranking
// semanal canónico (lib/rsCanonical.js). Antes leían row.rsGlobalPct —el
// percentil del lote del escaneo— y la misma fila decía "RS 87" en la frase y
// "91" (o "Sin dato") en la columna. Si el símbolo no está en el ranking, la
// frase simplemente no menciona el RS: la columna ya enseña la ausencia con
// su motivo.
...
export const LIST_CONTRACTS = {
  ...
  weinstein: {
    title: "Tendencia establecida",
    tone: "bullish",
    label: "Etapa 2",
    text: "Estructura de tendencia/etapa 2 y medias coherentes; excluye deterioro técnico.",
  },
  minervini: {
    title: "Rupturas con contracción",
    tone: "bullish",
    // "Trend template" era nombre propio de metodología (retirados de la UI)
    // y además en inglés; lo que comprueba es la estructura de tendencia.
    label: "Estructura de tendencia",
    text: "Estructura de tendencia, momentum y cercanía a máximos; excluye deterioro técnico y bajismo estructural.",
  },
  nearPivot: {
...
export function listInclusionReasons(row = {}, listKey = "leaders", limit = 4) {
  ...
  if (listKey === "rsQuality") {
    ...
    pushMetric(reasons, "RS", canonicalRsValue(row));
    pushMetric(reasons, "R/vol", metric(row, "returnToVol3m"));
    if (longBiasOk(row)) reasons.push("sin señales bajistas");
  } else if (listKey === "weinstein") {
    pushMetric(reasons, "Estructura", metric(row, "weinsteinScore"));
    if (trendTemplateOk(row)) reasons.push("Etapa 2/medias OK");
    pushMetric(reasons, "RS", canonicalRsValue(row));
    pushMetric(reasons, "3M", perf3m, pct);
  } else if (listKey === "minervini") {
    pushMetric(reasons, "Ruptura", metric(row, "minerviniScore"));
    if (trendTemplateOk(row)) reasons.push("estructura de tendencia OK");
    pushMetric(reasons, "3M", perf3m, pct);
