...
//
// slice(-N), no slice(0, N): el chartPreview del contrato es ASCENDENTE
// (lib/researchRow.chartPreviewBars hace `reverse()` y luego `slice(-96)`;
// verificado contra producción — las filas de un scan servidor traen 96 barras
// de 2026-03-19 a 2026-08-07, la más antigua primero). Tomar las 48 PRIMERAS
// devolvía la mitad VIEJA de la ventana: la sparkline terminaba ~48 sesiones
// antes del último cierre y el punto `sparkLast` marcaba un precio caduco.
// Los 48 puntos que quiere la miniatura son los ÚLTIMOS.
export function compactChartPreview(chartPreview = []) {
  if (!Array.isArray(chartPreview)) return [];
  return chartPreview.slice(-COMPACT_CHART_PREVIEW_POINTS).map((bar) => ({
    date: bar.date,
