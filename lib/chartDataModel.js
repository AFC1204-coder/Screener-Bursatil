...
// ─────────────────────────────────────────────────────────────────────────────
// Homogeneidad de cadencia.
//
// Medido el 2026-08-14 (docs/analisis-grafico-2026-08-14.md, segunda pasada):
// la serie que sirve /api/chart para rangos largos mezcla un tramo antiguo de
// cadencia MENSUAL (herencia del merge MAX del brief en la caché) con el
// tramo reciente DIARIO. Dibujar esa mezcla convierte cada barra mensual en
// una falsa "semana"/"día": 5A·W declaraba «dic 2021 – ago 2026» con 96
// barras y un eje que saltaba dos años entre etiquetas.
//
// Regla: los intervalos D/W/M se construyen sobre cierres DIARIOS. Antes de
// agregar, la serie se recorta al SUFIJO de cadencia diaria homogénea: desde
// la última ruptura (> COARSE_GAP_DAYS días naturales entre filas
// consecutivas) hasta el final. Un mercado real no separa dos sesiones
// consecutivas más de ~5 días naturales (fin de semana + festivos); 7 deja
// margen. Lo descartado no desaparece en silencio: el resolver emite la
// notice informativa `history-coarse-dropped` (principio 5: la ausencia se
// explica donde falta). El VALOR de las filas no se toca — esto decide qué
// filas son elegibles para dibujo, no cambia ningún cálculo.
const COARSE_GAP_DAYS = 7;
const CALENDAR_DAY_SECONDS = 86400;

function homogeneousDailyRowsImpl(rows = []) {
  if (!Array.isArray(rows) || rows.length < 2) {
    return { rows: Array.isArray(rows) ? rows : [], dropped: 0, coarseUntilDate: "" };
  }
  let start = 0;
  for (let index = 1; index < rows.length; index += 1) {
    const gapDays = (rows[index].time - rows[index - 1].time) / CALENDAR_DAY_SECONDS;
    if (gapDays > COARSE_GAP_DAYS) start = index;
  }
  if (start === 0) return { rows, dropped: 0, coarseUntilDate: "" };
  return {
    rows: rows.slice(start),
    dropped: start,
    coarseUntilDate: rows[start - 1]?.date || "",
  };
}
...
// Preview close-only que ya cubre el rango pero no es candle-grade: hay que
// pedir OHLC real para poder pasar de línea interina a velas (CHART-QR-1b).
function needsOhlcUpgradeImpl(localRows = [], preferredStyle = null) {
  const pendingStyle = String(preferredStyle || "");
  return Boolean(
    pendingStyle
    && !LINE_STYLES.has(pendingStyle)
    && localRows.length >= 2
    && !barsAreCandleGrade(localRows),
  );
}

function needsRemoteBarsImpl(localRows = [], rangeKey = DEFAULT_CHART_SETTINGS.range, interval = DEFAULT_CHART_SETTINGS.interval, preferredStyle = null) {
  return shouldRequestRemoteBarsImpl(localRows, rangeKey, interval)
    || needsOhlcUpgradeImpl(localRows, preferredStyle);
}
...
// y devuelve las filas visibles junto al CONTEXTO (la serie agregada sin
// recortar al rango): los indicadores de ventana móvil (medias) se calculan
// sobre el contexto para no perder su warm-up al principio del rango.
function shapeRows(normalizedRows = [], interval, range) {
  ...
  return { rows: trimmed, hasRows: trimmed.length >= 2, context: aggregated };
}

// Notice informativa del recorte de cadencia (prioridad mínima: solo se emite
// si ninguna otra notice aplica). `coarse` es el resultado de
// `homogeneousDailyRowsImpl` de la fuente que acabó dibujándose.
function coarseNoticeFor(coarse) {
  if (!coarse || !coarse.dropped) return null;
  return {
    kind: "info",
    code: "history-coarse-dropped",
    text: `Histórico anterior a ${coarse.rows[0]?.date || ""} sin cadencia diaria: no se dibuja`,
    title: `La serie almacenada trae ${coarse.dropped} barras antiguas con paso mensual o mayor (hasta ${coarse.coarseUntilDate}). Dibujarlas como sesiones diarias fabricaría velas que no existieron; el gráfico muestra solo el tramo con cadencia real de mercado.`,
  };
}
...
const NOTICE_TABLE = [
  ...
  { priority: 5, code: "history-expanding", text: "Ampliando histórico…", kind: "expanding", copyTitle: false },
  ...
  { priority: 8, code: "history-coarse-dropped", text: null, kind: "info", copyTitle: true }, // informativa: solo si ninguna otra aplica.
];
...
function noticeFor({ availability, requestState, requestError, localQuality, remoteQuality, dataRange, interval }) {
  ...
  if (availability === "ready" && requestState === "loading") {
    return { kind: "expanding", code: "history-expanding", text: "Ampliando histórico…", title: "" };
  }
  if (availability === "ready" && requestState === "error") {
    return {
      kind: "error",
...
function normalizeInput(input = {}) {
  ...
  return {
    ...
    preferredStyle: input.preferredStyle != null ? String(input.preferredStyle) : null,
  };
}
...
function resolve(input = {}) {
  const { symbol, localSource, config, request, preferredStyle } = normalizeInput(input);
  ...
  // Paso 2: normalize + homogeneidad de cadencia + decisión de suficiencia.
  // NO se cambia el criterio P0: la suficiencia se evalúa antes del guard de
  // calidad, y se evalúa sobre las filas ELEGIBLES (cadencia diaria): contar
  // barras mensuales antiguas como si fueran sesiones infló la suficiencia.
  const intraday = isIntradayInterval(config.interval);
  const localCoarse = intraday
    ? { rows: normalizeRowsImpl(localSource.bars), dropped: 0, coarseUntilDate: "" }
    : homogeneousDailyRowsImpl(normalizeRowsImpl(localSource.bars));
  const localRows = localCoarse.rows;
  const needsRemote = needsRemoteBarsImpl(localRows, config.dataRange, config.interval, preferredStyle);
  ...
  if (!needsRemote || !symbol) {
    ...
    if (local === "ready") {
      const { rows, context } = shapeRows(localRows, config.interval, config.dataRange);
      if (rows.length < 2) {
  ...
  // Calidad real + barras válidas → esas barras (recortadas a su tramo de
  // cadencia homogénea; el remoto de rangos largos arrastra un prefijo
  // mensual heredado de la caché — ver homogeneousDailyRowsImpl).
  if (remoteBars && remoteBars.length) {
    const remoteCoarse = intraday
      ? { rows: normalizeRowsImpl(remoteBars), dropped: 0, coarseUntilDate: "" }
      : homogeneousDailyRowsImpl(normalizeRowsImpl(remoteBars));
    const normalizedRemote = remoteCoarse.rows;
    if (normalizedRemote.length) {
      const { rows, context } = shapeRows(normalizedRemote, config.interval, config.dataRange);
      if (rows.length < 2) {
...
function fallbackToLocal({ local, localQuality, localRows, localCoarse = null, config, requestState, requestError }) {
  if (local === "blocked") {
    return {
      rows: [],
      rowTimes: [],
...
export {
  ...
  homogeneousDailyRowsImpl as homogeneousDailyRows,
  isIntradayInterval,
  normalizeChartInterval,
  normalizeRowsImpl as normalizeRows,
  shouldRequestRemoteBarsImpl as shouldRequestRemoteBars,
  needsRemoteBarsImpl as needsRemoteBars,
  needsOhlcUpgradeImpl as needsOhlcUpgrade,
  timeFromBar,
  weekKey,
};
