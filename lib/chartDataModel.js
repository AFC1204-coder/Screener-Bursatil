...
// ─────────────────────────────────────────────────────────────────────────────
// Resolución de la calidad local.
//
// El componente acepta hoy dos formas de "calidad local":
//   - chartEstimated (booleano): legado de la prop que cubría el caso en el
//     que `compactChartBars` borró el flag `estimated` por barra (ver ADR §3.2).
//   - localQuality (objeto canónico ChartQuality): nuevo contrato del ADR.
// `localQualityFromInput` mapea cualquiera de los dos al ChartQuality canónico.
// Si NO viene ninguna de las dos, se clasifica por las señales estructurales
// del payload (`chartQuality` camino legacy).
function localQualityFromInput(input = {}) {
  const { bars = [], localQuality = null, chartEstimated = false } = input;
  if (localQuality && typeof localQuality === "object" && typeof localQuality.status === "string") {
    return localQuality;
  }
  if (chartEstimated === true) {
    return chartQualityFromBrief({ bars, dataQuality: null, chartProvider: "chartEstimated-legacy" });
  }
...
function noticeFor({ availability, requestState, requestError, localQuality, remoteQuality, dataRange, interval }) {
  ...
  //    como si viene del local (prop chartEstimated / localQuality no-real).
  //    §3.4 prioridad 1. Cuando remoteQuality es el veredicto, gana el remoto.
...
function resolve(input = {}) {
  ...
  // que el guard necesita incluso si las barras crudas no los exponen.
  const localQuality = localQualityFromInput({
    bars: localSource.bars,
    localQuality: localSource.quality,
    chartEstimated: localSource.chartEstimated,
  });
