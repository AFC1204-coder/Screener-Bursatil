...
function globalEntryReusableForUsCountry(entry) {
  if (!entry) return false;
  // Exigir engine explícito: exclusiones de weeklyRsEntry no llevan
  // engineVersion; con `!engine` se colarían ratings/motivos del RS global
  // privado (≠ US country) en weeklyCountryRs*.
  return String(entry.engineVersion || "").trim() === US_COUNTRY_RS_ENGINE_VERSION;
}

function pickUsCountryFromReuse(usSymbols = [], reuseGlobalRsBySymbol = null) {
  const bySymbol = new Map();
  const pending = [];
  for (const symbol of usSymbols) {
    const entry = reuseGlobalRsBySymbol?.get(symbol);
    if (entry && globalEntryReusableForUsCountry(entry)) bySymbol.set(symbol, entry);
    else pending.push(symbol);
  }
  return { bySymbol, pending };
}
...
export async function readCountryRsForSymbols(symbols = [], options = {}) {
  ...
  const reuseGlobalRsBySymbol = options.reuseGlobalRsBySymbol || null;
  const { bySymbol: reusedUs, pending: pendingUs } = pickUsCountryFromReuse(usSymbols, reuseGlobalRsBySymbol);
  for (const [symbol, entry] of reusedUs) bySymbol.set(symbol, entry);

  const engineReads = [];
  if (pendingUs.length) {
    engineReads.push(readGlobalRsForSymbols(pendingUs, {
      engineVersion: US_COUNTRY_RS_ENGINE_VERSION,
      bulkSnapshot: true,
    }));
  }
  for (const [engineVersion, engineSymbols] of byEngine) {
    engineReads.push(readEngineCountryRsForSymbols(engineSymbols, engineVersion));
  }
  const batches = await Promise.all(engineReads);
  for (const batch of batches) {
    for (const [symbol, entry] of batch?.bySymbol || []) {
