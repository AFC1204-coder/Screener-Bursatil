...
const LABELS = {
  ...
  latestVolume: "Vol último",
  avgTurnover: "Importe medio",
  latestTurnover: "Importe último",
...
// Copia local de COMPOSITE_WEIGHTS (lib/scoringEngine.js): este módulo audita
// el score persistido reproduciendo la fórmula por su cuenta, así que debe
// declararla, no importarla — si importara la del motor, un cambio de pesos
// se auto-validaría y la auditoría dejaría de detectar nada. Cuando el motor
// cambie, esta lista tiene que cambiar a mano y el test de equivalencia lo
// exige. El término `ipoScore` (0.02) salió del motor el 2026-08-15.
...
// Los pesos declarados suman 0.98 y el motor renormaliza sobre esa suma.
const SCORE_TOTAL_WEIGHT = SCORE_COMPONENTS.reduce((sum, [, weight]) => sum + weight, 0);
...
function scoreFormulaValue(row = {}, { setupField = "setupQualityScore" } = {}) {
  ...
  const value = SCORE_COMPONENTS.reduce((sum, [key, weight]) => sum + values[key] * weight, 0) / SCORE_TOTAL_WEIGHT;
...
export function addScoredObjectiveMetricAudit(row = {}) {
  ...
  const scoreFormula = "(setup*.17 + rs*.16 + rsQuality*.06 + demand*.10 + ad*.08 + growth*.08 + eps*.08 + sector*.10 + riskReward*.08 + risk*.05 + momentum*.02) / .98";
  const items = [
    ...baseItems,
    metricEntry(row, {
      key: "totalScore",
...
export function objectiveMetricAuditStatusForRow(row = {}) {
  ...
  if (!audit || typeof audit !== "object") {
    return {
      key: "missing",
      label: "Métricas sin auditoría",
      tone: "warn",
      detail: "La fila no trae provenance numérica objetiva.",
      audit: null,
    };
  }
  ...
  if (bad) {
    return {
      key: "bad",
      label: "Métricas objetivas bloqueadas",
      tone: "bad",
      detail: issues.slice(0, 3).map((item) => `${item.label || item.key}: ${item.status}`).join(" · ") || "mismatch numérico",
      audit,
    };
  }
  if (warn) {
