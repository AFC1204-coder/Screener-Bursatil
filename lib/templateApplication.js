...
export function templateSnapshotAssessment(config = {}, analyzedRows = [], context = {}) {
  ...
  const resolved = resolveStoredFilterConfig(safeConfig);
  ...
  const view = rows.length ? filterAnalyzedRows(rows, resolved.effectiveSettings, filterContext) : null;
  return {
    presetKey: resolved.presetKey,
    effectiveSettings: resolved.effectiveSettings,
    familyIntensity: resolved.familyIntensity,
    familyIntensityCustom: resolved.familyIntensityCustom,
