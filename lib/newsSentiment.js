...
export function summarizeNewsSentiment(rows = []) {
  ...
  let regime = "Neutral";
  let contrarianRead = "Ruido equilibrado: el precio y la amplitud mandan más que los titulares.";
  if (pessimismIndex >= 75) {
    regime = "Pesimismo extremo";
    contrarianRead = "Lectura contraria potencial: si los indices recuperan medias y aparecen líderes, el miedo puede estar cerca de agotarse.";
  } else if (pessimismIndex >= 62) {
    regime = "Negatividad dominante";
    contrarianRead = "Entorno emocional defensivo. En analisis técnico por etapas conviene esperar confirmación técnica antes de aumentar riesgo.";
  } else if (pessimismIndex <= 25) {
    regime = "Optimismo/euforia";
    contrarianRead = "Optimismo elevado. Puede acompañar una etapa 2, pero también aumenta riesgo de complacencia si el precio se extiende.";
  ...
  return {
    ...
    regime,
    contrarianRead,
    note: "Indice heurístico basado en titulares recientes; no es analisis semantico propietario ni recomendacion.",
    rows: analyzed,
  };
}
