...
// ─────────────────────────────────────────────────────────────────────────────
// Alineación temporal de series superpuestas.
//
// lightweight-charts fusiona los tiempos de TODAS las series en un solo eje
// indexado: un punto cuyo `time` no coincide con el de ninguna vela añade una
// columna fantasma y separa las velas entre sí (medido en
// docs/analisis-grafico-2026-08-14.md, A2: 58 columnas para 33 velas
// mensuales, porque los snapshots semanales del RS caen en domingo y las
// velas semanales llevan el viernes). REGLA: toda serie que se dibuje junto
// al precio se ancla a la vela vigente en su fecha — la última vela con
// `time <= point.time`. Dos puntos que caen en la misma vela colapsan al
// último (el más reciente manda). El VALOR del punto no se toca: esto es
// coordenada de dibujo, no cálculo de métrica.
function snapPointsToRows(points = [], rows = []) {
  if (!Array.isArray(points) || points.length === 0) return [];
  if (!Array.isArray(rows) || rows.length === 0) return [];
  const out = new Map(); // time de vela → punto (el último gana)
  let rowIndex = 0;
  for (const point of points) {
    const target = Number(point?.time);
    if (!Number.isFinite(target)) continue;
    if (target < rows[0].time) continue;
    // `points` llega ordenado ascendente: rowIndex solo avanza.
    while (rowIndex + 1 < rows.length && rows[rowIndex + 1].time <= target) rowIndex += 1;
    const row = rows[rowIndex];
    if (!row || row.time > target) continue;
    out.set(row.time, { ...point, time: row.time, date: row.date || point.date });
  }
...
export function projectRsRatingSeries(rows = [], series = null, indicators = {}, interval = "D") {
  ...
  // Anclaje a la vela vigente: sin él, los snapshots (domingo) crean columnas
  // fantasma en el eje compartido y separan las velas (análisis A2).
  return snapPointsToRows(projected, rows);
}

// Proyecta la serie de RS país (1-99) con el mismo anclaje temporal que el RS
// global, pero con toggle independiente (`indicators.rsCountryLine`).
export function projectRsCountryRatingSeries(rows = [], series = null, indicators = {}, interval = "D") {
  if (indicators.rsCountryLine === false) return emptyRows();
  return projectRsRatingSeries(rows, series, { ...indicators, rsLine: true }, interval);
}

// Proyecta la serie de RS tema (1-99) con el mismo anclaje temporal que el RS
// global, pero con toggle independiente (`indicators.rsThemeLine`).
export function projectRsThemeRatingSeries(rows = [], series = null, indicators = {}, interval = "D") {
  if (indicators.rsThemeLine === false) return emptyRows();
  return projectRsRatingSeries(rows, series, { ...indicators, rsLine: true }, interval);
}
...
export function projectBenchmarkLineSeries(rows = [], series = null, interval = "D", indicators = {}) {
  ...
  const start = rows[0]?.time || 0;
  const end = rows.at(-1)?.time || Infinity;
  // Anclaje a la vela vigente (sustituye a la agregación por clave temporal
  // propia): cada punto colapsa a la última vela con time <= punto, y el
  // último punto de cada vela manda — la misma semántica de "último cierre
  // del grupo", pero con el `time` de la vela dibujada, que es lo que impide
  // las columnas fantasma del eje compartido (análisis A2/B2).
  const points = snapPointsToRows(
    source
      .map((point) => {
