...
import {
  BREAKOUT_VOL_REFERENCE_THRESHOLD,
...
// Cuando weeklyStageStructure es n/a, el motivo ya vive en weeklyStageStructureDetail
// (lib/weeklyStageStructure.js). Tres familias legibles para la UI — no silencio.
export const STAGE_STRUCTURE_ABSENCE = {
  dubious: {
    word: "Dudoso",
    title: "Sin calificador estructural: ni caja ≤32% ni ruptura con HH/HL.",
  },
  shortHistory: {
    word: "Hist. corto",
    title: "Sin calificador estructural: histórico semanal insuficiente para el techo de 52 semanas.",
  },
  notApplicable: {
    word: "No aplica",
    title: "Sin calificador estructural: solo aplica en etapas 1 y 2.",
  },
};

/**
 * @param {string} structure `weeklyStageStructure`
 * @param {string} detail `weeklyStageStructureDetail`
 * @returns {{word: string, title: string, key: string}|null}
 */
export function stageStructureAbsence(structure = "", detail = "") {
  const normalized = String(structure || "").trim();
  if (normalized !== STRUCTURE_NA && normalized !== "n/a") return null;
  const text = String(detail || "").trim();
  if (!text) return null;
  if (/corto/i.test(text)) {
    return { ...STAGE_STRUCTURE_ABSENCE.shortHistory, key: "short_history" };
  }
  if (/no aplica/i.test(text)) {
    return { ...STAGE_STRUCTURE_ABSENCE.notApplicable, key: "not_applicable" };
  }
  return { ...STAGE_STRUCTURE_ABSENCE.dubious, key: "dubious" };
}

export function formatBreakoutVolMultiplier(ratio) {
...
/**
 * Soporte de volumen en la fuga (STAGE-4). Solo aplica con «Con fuga».
 * Patrón MET-4: «acompaña (1,4×)» — aquí «vol. fuga 1,8×».
 */
export function breakoutVolDisplayForRow(row = {}) {
  const parts = stagePartsFromRow(row);
  if (parts.structure !== STRUCTURE_E2_STRUCTURAL) return null;
  const ratio = finiteBreakoutVol(row);
  const ref = BREAKOUT_VOL_REFERENCE_THRESHOLD;
  if (ratio === null) {
    return {
      available: false,
      phrase: "sin dato vol. fuga",
      multiplier: "",
      title: `Sin ratio de volumen en la ruptura. Referencia visual Weinstein ≈${ref}× la mediana de las ${4} semanas previas (no condiciona el calificador).`,
    };
  }
  const multiplier = formatBreakoutVolMultiplier(ratio);
  const dry = ratio < 1;
  const phrase = dry ? `vol. fuga seco ${multiplier}` : `vol. fuga ${multiplier}`;
  const refHint = ratio >= ref ? "por encima" : "por debajo";
  return {
    available: true,
    phrase,
    multiplier,
    title: `${phrase}. Referencia Weinstein ≈${ref}× (${refHint}); no condiciona «Con fuga».`,
  };
}

function finiteBreakoutVol(row = {}) {
  const weekly = row.stage?.weekly && typeof row.stage.weekly === "object" ? row.stage.weekly : {};
  const raw = row.weeklyBreakoutVolRatio
    ?? weekly.breakoutVolRatio
    ?? weekly.weeklyBreakoutVolRatio
    ?? null;
...
function stagePartsFromRow(row = {}) {
  ...
  return {
    ...
    structureDetail: row.weeklyStageStructureDetail
      || weekly.structureDetail
      || weekly.weeklyStageStructureDetail
      || "",
    breakoutVolRatio: finiteBreakoutVol(row),
  };
}
...
export function stageDisplayForRow(row = {}) {
  ...
  const absenceInfo = qualifierInfo ? null : stageStructureAbsence(parts.structure, parts.structureDetail);
  const breakoutVol = parts.structure === STRUCTURE_E2_STRUCTURAL
    ? breakoutVolDisplayForRow(row)
    : null;
  const titleParts = [];
  ...
  if (qualifierInfo) {
    ...
    if (breakoutVol?.title) titleParts.push(breakoutVol.title);
  } else if (absenceInfo) {
    titleParts.push(STAGE_CODE_VS_OPERATIVE_HINT);
    titleParts.push(absenceInfo.title);
    if (parts.structureDetail) titleParts.push(parts.structureDetail);
  }
  const qualifier = qualifierInfo?.word || absenceInfo?.word || "";
  const qualifierVol = breakoutVol?.multiplier ? `(${breakoutVol.multiplier})` : "";
  const qualifierTitle = qualifierInfo?.title || absenceInfo?.title || "";
  return {
    ...wordInfo,
    confirmation,
    qualifier,
    qualifierVol,
    qualifierDisplay: qualifier && qualifierVol ? `${qualifier} ${qualifierVol}` : qualifier,
    breakoutVol,
    qualifierTitle,
    structureAbsence: absenceInfo?.key || "",
    title: titleParts.filter(Boolean).join(" "),
  };
}

/**
 * Texto plano de etapa Weinstein para superficies sin markup (review, modales).
 * Misma lectura que la columna «Etapa» de la mesa: palabra + calificador.
 *
 * @returns {string|null} null = mostrar ausencia
 */
export function stageSummaryText(row = {}) {
  const display = stageDisplayForRow(row);
  if (!display) return null;
  if (display.qualifierDisplay) return `${display.word} · ${display.qualifierDisplay}`;
