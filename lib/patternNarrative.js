...
function technicalConfidenceForPattern(row = {}) {
  ...
  if (status === "partial_volume" || status === "missing_volume") {
    return {
      ...
      label: "Dato parcial",
      shortLabel: "Parcial",
      detail: "Volumen incompleto: la estructura se puede observar, pero no habilita plan automatico.",
      state: "warn",
      tone: "warn",
      observable: true,
      blocksAction: true,
  ...
  if (status === "ok") {
    return {
      ...
      label: "Dato usable",
      shortLabel: "OK",
      detail: "OHLCV suficiente para validar la metodologia tecnica.",
      state: "pass",
      tone: "good",
      observable: true,
      blocksAction: false,
...
function strictVcpRejectReason(row = {}) {
  ...
  if (!Number.isFinite(pivotPrice) && !Number.isFinite(distanceToPivot)) return "pivot tecnico sin dato";
  ...
  if (Number.isFinite(latestCloseLocation) && latestCloseLocation < 45) return `cierre debil ${pct(latestCloseLocation)} del rango`;
...
function setupStructureForRow(row = {}) {
  ...
  if ((row.pivotSqueeze || family === "pivot_squeeze") && pivotSqueezePass(row)) {
    return {
      key: "pivot_squeeze",
      label: "Compresion de pivot",
      shortLabel: "Compresion de pivot",
      tone: "watch",
