...
function storedDataLimitedReason(row = {}) {
  const reliabilityState = cleanText(row.methodologyReliabilityState).toLowerCase();
  const displayKey = cleanText(row.setupDisplayKey).toLowerCase();
  const displayState = cleanText(row.setupDisplayState).toLowerCase();
  if (boolTrue(row.setupDisplayDataLimited) || reliabilityState === "data_limited" || displayKey === "data_limited" || displayState === "data_limited") {
    return row.methodologyReliabilityReason || row.setupDisplayReason || row.setupVerdictReason || "Datos parciales: no afirmar patrón automáticamente.";
  }
  return "";
}
...
export function methodologyVerdictForRow(input = {}) {
  ...
  const quality = finite(row.patternQualityScore);
  const nearPivot = inNearPivotZone(row);
  const dataLimitedReason = storedDataLimitedReason(row);

  if (dataLimitedReason) {
    return {
      key: "data_issue",
      state: "data",
      label: row.methodologyReliabilityLabel || row.setupDisplayLabel || "Datos parciales",
      shortLabel: row.setupDisplayShortLabel || row.setupVerdictShortLabel || "Datos",
      tone: "muted",
      reason: dataLimitedReason,
      evidence: structure.evidence,
      dataLabel: structure.dataLabel || dataConfidence.label,
      dataConfidence,
      structure,
      tradePlanEligible: false,
      tradePlanReason: gate.reason || dataLimitedReason,
      actionable: false,
      observable: false,
      watch: false,
      strict: false,
