...
function blockedLabelFor(row = {}, structure = {}) {
  ...
  if (structure.key === "pivot_squeeze") return { key: "not_actionable", label: "Compresion no confirmada", shortLabel: "Compresion no confirmada" };
...
export function methodologyVerdictForRow(input = {}) {
  ...
  if (row.patternEligible === false || structure.key === "data") {
    return {
      ...
      shortLabel: "Datos",
      tone: "muted",
      reason: structure.reason || structure.dataLabel || "Cobertura tecnica insuficiente.",
  ...
  if (structure.key === "vcp_strict") {
    return {
      ...
      tone: "watch",
      reason: gate.reason || structure.reason || "La estructura es estricta, pero falta una condicion para plan valido.",
      evidence: structure.evidence,
      dataLabel: structure.dataLabel,
      dataConfidence,
      structure,
  ...
  if (structureWatchEligible) {
    const watchLabel = ["vcp_watch", "constructive_base"].includes(structure.key) ? "Base en vigilancia" : structure.label;
    const watchShortLabel = ["vcp_watch", "constructive_base"].includes(structure.key) ? "Base en vigilancia" : structure.shortLabel;
    return {
      key: structure.key,
      state: "watch",
      label: watchLabel,
      ...
      reason: structure.reason || gate.reason || "Estructura observable, sin plan valido.",
      evidence: structure.evidence,
      dataLabel: structure.dataLabel,
      dataConfidence,
      structure,
