...
/** Ausencia operativa esperada en mesa: sin InfoHint por fila (TABLE-QUIET-1). */
export function themeRsAbsenceIsExpected(row = {}, trs = null) {
  const resolved = trs || themeRs(row);
  ...
  if (!resolved.hydrated) return true;

  const code = String(pick(row, "weeklyThemeRsExclusionReason") || "").trim();
  if (code && THEME_EXCLUSION_TEXT[code]) return true;

  const reason = String(resolved.reason || "").trim();
  if (reason === THEME_RS_NOT_HYDRATED_REASON || reason === THEME_RS_NOT_RANKED_REASON) {
    return true;
  }

  return Object.values(THEME_EXCLUSION_TEXT).some((text) => text && reason.startsWith(text));
}
