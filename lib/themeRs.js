...
export function themeKeyForRow(row = {}) {
  ...
  if (isRankableThemeKey(uiTheme) && hasMinimumThemeProfile(sector, industry, summary)) {
    return uiTheme;
  }
  return null;
}
...
export function themeRsAssignmentForRow(row = {}) {
  ...
  return { themeKey: null, exclusionReason: THEME_RESIDUAL, exclusionDetail: String(pick(row, "theme") || "") };
}
