  ...
  draw(target) {
    ...
    target.useBitmapCoordinateSpace(({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) => {
      ...
      for (const entry of lines) {
        ...
        ctx.lineWidth = Math.max(1, width * horizontalPixelRatio);
        ctx.beginPath();
        ctx.moveTo(entry.x1 * horizontalPixelRatio, entry.y1 * verticalPixelRatio);
        ctx.lineTo(entry.x2 * horizontalPixelRatio, entry.y2 * verticalPixelRatio);
        ctx.stroke();
      }

      if (pending?.p1 && pending?.cursor) {
        ctx.strokeStyle = styles.activa;
        ctx.lineWidth = Math.max(1, 2 * horizontalPixelRatio);
        ctx.setLineDash([4 * horizontalPixelRatio, 4 * horizontalPixelRatio]);
        ctx.beginPath();
        ctx.moveTo(pending.p1.x * horizontalPixelRatio, pending.p1.y * verticalPixelRatio);
        ctx.lineTo(pending.cursor.x * horizontalPixelRatio, pending.cursor.y * verticalPixelRatio);
        ctx.stroke();
        ctx.setLineDash([]);
      }
...
export class TrendlinePrimitive {
  ...
  updateAllViews() {
    ...
    if (!Array.isArray(rows) || rows.length < 2 || !range) {
      ...
      if (emptyHash === this._lastFrameHash) return;
      this._lastFrameHash = emptyHash;
      this._frameState = {
        lines: [],
        pending: null,
    ...
    // Firma barata del contenido que realmente se pinta (no de los inputs:
    // rows/range pueden cambiar de referencia sin cambiar de valor efectivo).
    // Si es idéntica a la del frame anterior, no pintamos de nuevo — corta la
    // cadena updateAllViews→_requestRedraw→(repintado)→updateAllViews aquí,
    // en el origen, en vez de intentar bloquearla a mitad de camino.
    const nextHash = JSON.stringify({
      lines: lines.map((l) => [l.drawing.id, l.x1, l.y1, l.x2, l.y2, l.handles]),
      pending: this._pendingOverlay,
      selectedId: this._selectedId,
      viewportSize,
    });
    if (nextHash === this._lastFrameHash) return;
    this._lastFrameHash = nextHash;
  ...
  getFirstLineCenter() {
    ...
    pickAt(x, y, toleranceOverride) {
    // Tolerancia opcional por tipo de puntero (ADR §7.3): 8px mouse/pen,
    // 16px touch. El default conserva la tolerancia histórica (HIT_TOLERANCE_PX).
    const tolerance = Number.isFinite(toleranceOverride) && toleranceOverride > 0
      ? toleranceOverride
      : HIT_TOLERANCE_PX;
    let best = null;
    // Para handles seguimos exigiendo un mínimo generoso (8px) independientemente
    // del override — si el caller quiere mayor tolerancia para táctil, ya
    // cubre también los handles.
