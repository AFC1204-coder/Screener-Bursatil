...
/**
 * Periodo de media móvil. Vacío / 0 / basura → default (no clamp a min:
 * Number("") === 0 y Math.max(2, 0) dejaba un «2» pegado al borrar el campo).
 */
export function normalizeMaLength(value, { fallback, min, max } = {}) {
  const safeFallback = Number.isFinite(Number(fallback)) ? Number(fallback) : 50;
  const safeMin = Number.isFinite(Number(min)) ? Number(min) : 2;
  const safeMax = Number.isFinite(Number(max)) ? Number(max) : 400;
  if (value === "" || value == null) return safeFallback;
  const n = Number(value);
  if (!Number.isFinite(n) || n <= 0) return safeFallback;
  return Math.min(safeMax, Math.max(safeMin, Math.round(n)));
}
...
export function normalizeChartSettings(value = {}) {
  ...
  return {
    ...
    indicators: {
      ...
      rsCountryLine: indicators.rsCountryLine === true,
      rsThemeLine: indicators.rsThemeLine === true,
      maFast: indicators.maFast !== false,
      maSlow: indicators.maSlow !== false,
      maFastLength: normalizeMaLength(indicators.maFastLength, {
        fallback: DEFAULT_CHART_SETTINGS.indicators.maFastLength,
        min: 2,
        max: 400,
      }),
      maSlowLength: normalizeMaLength(indicators.maSlowLength, {
