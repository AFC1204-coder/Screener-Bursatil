import { setRemotePersistenceConfigured } from "@/lib/localScanPersistence";
import { readFavoriteTombstones, readScanTombstones, writeFavoriteTombstones, writeScanTombstones } from "@/lib/localState";
import { sortedMarketsForLookup } from "@/lib/materializedScanLookup";
import { MARKETS_ANCHOR, NIGHTLY_US_ANCHOR } from "@/lib/scanLocalId";

// Auth: credentials same-origin envía la cookie de sesión HttpOnly; proxy.js la
// valida. No se usa Bearer ni token en localStorage (legacy eliminado en C-08).
...
async function requestJson(url, options = {}) {
  try {
    ...
    if (data.configured === false) setRemotePersistenceConfigured(false);
    else if (data.configured !== false) setRemotePersistenceConfigured(true);
...
// anchor=nightly-us: el último escaneo NOCTURNO ESTADOUNIDENSE, no el último
// escaneo a secas. Pedir "el más reciente" traía el cron europeo de las 22-23h
// —una acción italiana el 16 de agosto de 2026, frente a 3.313
// estadounidenses— porque corre después del nocturno de las 03:57.
// Si no hay nocturno, la respuesta trae `nightly: { found: false, reason }` y
// cero escaneos: nunca el de otro mercado (lib/nightlyUsScan.js).
//
// Solo un escaneo: limit=1 es la única combinación que activa
...
// un escaneo, no un orden global entre varios).
//
// ── Por qué el tope es el universo entero y no unas páginas ───────────────
// rowsLimit=500 venía del diagnóstico del 13 de agosto, cuando el arranque
// pedía 20.000 filas completas (46 KB cada una) para pintar 50. Entonces era
// correcto. Desde entonces el nocturno guarda la población entera en
// proyección ligera y el arranque pide UN escaneo concreto, así que 500 dejó
// de proteger nada y pasó a romper el producto: el usuario filtraba sobre
// 500 de 3.312 acciones, y como el recorte iba por rank_index —que ordena por
// puntuación— los valores débiles no llegaban nunca. Un filtro de etapa 4 o de
// RS bajo devolvía casi nada, y no porque no existieran esos valores.
//
// 6.000 cubre el universo estadounidense entero con margen: el nocturno del
// 17 de agosto de 2026 analizó 5.609 símbolos y guardó 3.312 filas, así que
// 6.000 está por encima incluso del caso en que TODOS los símbolos analizados
// pasaran el filtro base. Y queda por debajo de CACHEABLE_ROWS_LIMIT (8.000),
// el techo que mantiene viva la caché de 15 minutos de la ruta.
//
// Peso medido de esa respuesta el 2026-08-17 (nocturno de 3.312 filas):
// 27,3 MB de JSON, 2,7 MB con gzip, 5,5 s de servidor en frío y 50 ms
// cacheada. Cabe en el transporte; NO cabe en localStorage (presupuesto de
// 4,5 M de caracteres), así que la copia local guarda una muestra repartida y
// lo dice — ver fitScansForBrowser en lib/screenerPipeline.js.
export const STARTUP_ROWS_LIMIT = 6000;

function buildStartupScanUrl({ anchor, markets = [], hydrateRs = "0" } = {}) {
  const params = new URLSearchParams({
    includeRows: "1",
    limit: "1",
    rowsLimit: String(STARTUP_ROWS_LIMIT),
    anchor,
    hydrateRs: String(hydrateRs),
  });
  const normalized = sortedMarketsForLookup(markets);
  if (normalized.length) params.set("markets", normalized.join(","));
  return `/api/scans?${params}`;
}

// Bootstrap core-first (SCREENER-BOOTSTRAP-CORE-FIRST-1): arranque pide
// hydrateRs=0 para pintar la mesa YA; RS país/tema llegan en segunda fase
// (getLatestScanFromCloudExtended*) sin bloquear la UI.
export async function getLatestScanFromCloud() {
  return requestJson(buildStartupScanUrl({ anchor: NIGHTLY_US_ANCHOR, hydrateRs: "0" }));
}

// priority:"low" (Fetch Priority) — no compite con paint/core ni con otros GET
// críticos del arranque (SCANS-HYDRATERS-COLD-1).
export async function getLatestScanFromCloudExtended() {
  return requestJson(buildStartupScanUrl({ anchor: NIGHTLY_US_ANCHOR, hydrateRs: "1" }), {
    priority: "low",
  });
}

/** Último materializado publicable para un conjunto de mercados (anchor=markets). */
export async function getLatestScanFromCloudForMarkets(markets = []) {
  return requestJson(buildStartupScanUrl({ anchor: MARKETS_ANCHOR, markets, hydrateRs: "0" }));
}

export async function getLatestScanFromCloudForMarketsExtended(markets = []) {
  return requestJson(buildStartupScanUrl({ anchor: MARKETS_ANCHOR, markets, hydrateRs: "1" }), {
    priority: "low",
  });
}
...
export async function pullCloudState() {
  ...
  const scanResult = await requestJson(`/api/scans?${scanQuery}`);
