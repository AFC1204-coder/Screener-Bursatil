...
// hydrateRs=1: la mesa de producto necesita RS país/tema (MET-2/3). El default
 // compacto de /api/scans es core (PERF-NAC, sin país/tema) para callers
 // genéricos; el cliente de arranque pide extended de forma explícita.
export async function getLatestScanFromCloud() {
  return requestJson(`/api/scans?includeRows=1&limit=1&rowsLimit=${STARTUP_ROWS_LIMIT}&anchor=${encodeURIComponent(NIGHTLY_US_ANCHOR)}&hydrateRs=1`);
}
...
export async function getLatestScanFromCloudForMarkets(markets = []) {
  ...
  const marketsParam = encodeURIComponent(normalized.join(","));
  return requestJson(
    `/api/scans?includeRows=1&limit=1&rowsLimit=${STARTUP_ROWS_LIMIT}&anchor=${encodeURIComponent(MARKETS_ANCHOR)}&markets=${marketsParam}&hydrateRs=1`,
  );
}
