...
export async function resolveAlertInCloud(alert) {
  if (!alert?.cloudId) return { ok: true, configured: true, skipped: true, message: "Alerta solo local" };
  return requestJson("/api/alerts", { method: "PATCH", body: JSON.stringify({ cloudId: alert.cloudId, status: alert.status || "resolved", payload: alert.payload || {} }) });
}
...
export async function pushCloudState({ scans = [], favorites = [] }) {
  ...
  const alerts = readLocalAlertsForPush();
  const alertResult = await syncAlertsToCloud(alerts.filter((alert) => alert.status !== "resolved" && alert.status !== "dismissed"));
