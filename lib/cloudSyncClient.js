...
const APP_ACCESS_TOKEN = process.env.NEXT_PUBLIC_STATSEDGE_ACCESS_TOKEN || "";

function persistenceAuthHeaders() {
  const token = readPersistenceToken();
  // x-statsedge-token debe llevar el token de app (lo exige middleware.js);
  // el token de persistencia viaja como Bearer, que requirePersistenceAuth también acepta.
  const headers = APP_ACCESS_TOKEN ? { "x-statsedge-token": APP_ACCESS_TOKEN } : {};
  if (token) {
    headers.Authorization = `Bearer ${token}`;
    if (!APP_ACCESS_TOKEN) headers["x-statsedge-token"] = token;
  }
  return headers;
}
...
export async function getLatestScanFromCloud() {
  return requestJson("/api/scans?includeRows=1&limit=1");
}
...
export async function pushCloudState({ scans = [], favorites = [], alerts = null } = {}) {
  ...
  return {
    ...
    message: !configured ? "Supabase no configurado" : ok ? "Datos subidos a Supabase" : scanDeleteResult.message || scanResult.message || favoriteDeleteResult.message || favoriteResult.message || alertResult.message,
