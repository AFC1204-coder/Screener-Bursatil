...
function textFor(item = {}) {
  return norm([item.sector, item.industry, item.theme, item.name, item.country, item.symbol].filter(Boolean).join(" "));
}
...
export function findSimilarStocks(target = {}) {
  ...
  const targetTokens = tokenSet(target.sector, target.industry, target.theme, target.name);
  const targetFamily = activityFamily(target);
  const targetGeo = geoGroup(target.country, target.symbol);
  return PEERS.map((peer) => {
    if (norm(peer.symbol) === targetSymbol) return null;
    if (likelySameIssuer(peer, target)) return null;
    const peerTokens = tokenSet(peer.sector, peer.industry, peer.theme, peer.name);
    const peerFamily = activityFamily(peer);
    const peerGeo = geoGroup(peer.country, peer.symbol);
    if (targetFamily && peerFamily && targetFamily !== peerFamily) return null;

    const reasons = [];
    let score = 0;
    const exactIndustry = target.industry && norm(peer.industry) === norm(target.industry);
    const sameSector = target.sector && norm(peer.sector) === norm(target.sector);
    const sameTheme = target.theme && norm(peer.theme) === norm(target.theme);
