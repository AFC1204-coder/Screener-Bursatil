...
// US usa su ETF (SPY), igual que los índices de referencia del producto
// (decisión 2026-08-16). El resto son índices locales con prefijo para filas
// no-US: no encajan con una versión solo-US del producto y quedan señalados,
// pero su sustitución o retirada es una decisión aparte — hoy solo los usan
// los scans de mercados extranjeros (cron EU/Asia), no el nocturno US.
...
export function scoreRsQuality(row = {}) {
  ...
  return {
    ...
    rsQualityLabel: rs >= 80 && rsQualityScore >= 72 ? "RS limpio" : rs >= 80 && speculationRiskScore >= 55 ? "RS volatil" : rs >= 75 && rsQualityScore >= 62 ? "RS eficiente" : speculationRiskScore >= 70 ? "Momentum especulativo" : rs >= 60 ? "RS constructivo" : "RS débil",
  };
}
