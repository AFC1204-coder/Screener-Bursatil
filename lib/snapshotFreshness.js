...
export function isDismissibleSampleNotice(notice) {
  if (!notice || notice.requiresReauth || notice.stale) return false;
  if (notice.source === "session-sample") return true;
  return notice.source === "supabase" && Boolean(notice.truncated);
}

function truncatedScanDetail(scan, rowsReturned, rowsAvailable) {
  if (scan?.rowsSampled) {
    return `Se muestran ${rowsReturned} de ${rowsAvailable} acciones: muestra repartida por todo el ranking.`;
  }
  return `Se muestran ${rowsReturned} de ${rowsAvailable} acciones; el escaneo llegó recortado.`;
}
...
export function buildSnapshotFreshnessNotice(payload = {}, scan = {}) {
  ...
  if (truncated) {
    ...
    details.push(truncatedScanDetail(scan, rowsReturned, rowsAvailable));
  }
  if (partialRows > 0) {
    details.push(`${partialRows} filas tienen datos incompletos; revísalas antes de decidir.`);
  }

  const truncatedOnly = truncated && !stale;
  return {
    tone: stale ? "warn" : "info",
    label: stale
      ? "Sin actualizar hoy"
      : truncatedOnly
        ? (scan?.rowsSampled ? "Muestra" : "Universo parcial")
        : "Datos parciales",
    detail: details.join(" "),
    stale,
