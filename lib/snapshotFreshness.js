...
export function buildCloudAuthRequiredNotice({ scan = null } = {}) {
  ...
  return {
    ...
    detail: `Tu sesión ya no es válida para cargar el escaneo desde la nube.${populationNote} Vuelve a entrar para recuperar el universo completo.`,
...
export function buildLocalFallbackNotice({ rawMessage = "", configured = true, scan = null } = {}) {
  ...
  return {
    tone: "info",
    label: "Copia local",
    detail: `${reason} Se restaura la última copia local del escaneo nocturno estadounidense.${sampleDetail ? ` ${sampleDetail}` : ""}`,
    source: "local",
    sampled,
    rowsAvailable: sampled ? Number(scan?.rowsAvailable) : null,
    rowsReturned: sampled ? scan?.rows?.length : null,
