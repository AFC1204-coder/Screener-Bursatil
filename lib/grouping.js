...
import { metricValue, rowRsPrimary, weaknessScore } from "@/lib/stockRows";
...
// Promedio de RS de un grupo: solo cuenta el RS semanal del universo, el
// mismo que enseña cada fila (lib/rsCanonical.js). Antes caía al percentil
// del lote para los símbolos sin ranking semanal, con lo que el promedio de
// un grupo mezclaba dos rankings distintos y no cuadraba con la suma de sus
// filas. Los símbolos sin RS quedan fuera del promedio, no dentro con otro
// número: `avg` ya ignora los nulos.
function weeklyRsForGroup(row = {}) {
...
export function groupRows(rows = [], dimension) {
  ...
  return [...map.entries()].map(([key, items]) => {
    ...
    const avgRs = items.map(weeklyRsForGroup).filter(Number.isFinite);
