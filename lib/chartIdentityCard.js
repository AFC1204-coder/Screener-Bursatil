...
/** Mercado US del valor (sin sufijo intl → US por defecto). */
export function isUsStockMarket(data = null, symbol = "") {
  const country = String(data?.country || "").trim();
  if (/^(US|United States|Estados Unidos)$/i.test(country)) return true;
  if (country) return false;
  return countryCode(symbol) === "US";
}

/** RS país en tarjeta: solo si mercado no-US o el número difiere del RS canónico. */
export function shouldShowCountryRsOnIdentityCard({
  rsValue = null,
  countryRsValue = null,
  isUsMarket = true,
} = {}) {
  if (!isUsMarket) return true;
  const fr = Number.isFinite(rsValue) ? Math.round(rsValue) : null;
  const crs = Number.isFinite(countryRsValue) ? Math.round(countryRsValue) : null;
...
export function buildChartIdentityCard({ symbol = "", data = null, rsUniverse = null } = {}) {
  ...
  const stageDisplay = stageDisplayForRow({
    ...
    weeklyStageStructureDetail: weekly.structureDetail || "",
    weeklyBreakoutVolRatio: weekly.breakoutVolRatio ?? data.weeklyBreakoutVolRatio ?? null,
  });
  ...
  const countryRsValue = Number.isFinite(rs.countryRsRating) ? Math.round(rs.countryRsRating) : null;
  const marketIsUs = isUsStockMarket(data, symbol);
  ...
  return {
    ...
    countryRs: {
      value: countryRsValue,
      sampleSize: Number.isFinite(rs.countryRsSampleSize) ? rs.countryRsSampleSize : null,
      weekKey: rs.countryRsWeekKey || "",
      absenceReason: countryRsValue !== null ? "" : (rs.countryRsReason || DESCRIPTIVE_ABSENCE.rsCountry),
      show: shouldShowCountryRsOnIdentityCard({
        rsValue,
        countryRsValue,
        isUsMarket: marketIsUs,
      }),
    },
    structure: {
      distance52w,
