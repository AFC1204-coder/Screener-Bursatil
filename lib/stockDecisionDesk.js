...
function evidenceFocus(item = {}) {
  ...
  return { presetKey: "decision", label: "Enfocar", detail: "Vista metodológica del gráfico" };
}
...
function chartReadItems({ pending, confirmed, chartSettings, showVcpDiagnostics, setupPattern, setupDisplay }) {
  ...
  if (needsSetup && setupPattern && !showVcpDiagnostics) {
    items.push({ key: "vcp", label: "VCP", value: "Apagado", detail: "Pivot y contracciones fuera del gráfico", tone: "warn" });
  }
  if (setupDisplay?.blocksPatternClaim || setupDisplay?.dataLimited) {
    items.push({ key: "methodology", label: "Estructura", value: "Limitada", detail: setupDisplay.reason || setupDisplay.line || "Datos técnicos limitados", tone: "bad" });
  }

  if (!items.length) {
    items.push({ key: "aligned", label: "Lectura gráfico", value: "Alineada", detail: "Marco y capas coherentes con la observación", tone: "good" });
  }
