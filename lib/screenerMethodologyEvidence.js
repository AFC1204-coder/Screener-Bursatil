...
const REVIEW_ACTIONS = {
  ...
  confluence: "Comprobar que liderazgo, confirmación y ejecucion no dependan de un único motor.",
  demand: "Validar acumulación con volumen, A/D, RelVol o UD Vol antes de elevar confianza.",
...
// El número que evalúa esta categoría es el percentil del símbolo dentro de su
// lote de escaneo (rsGlobalPct), que es la entrada real del motor de decisión.
// NO es el RS del ranking semanal del universo que enseña la interfaz
// (lib/rsCanonical.js), y puede diferir. Se etiqueta como lo que es para que
// esta ficha de pruebas no muestre un segundo número llamado "RS": cambiar la
// entrada cambiaría la clasificación de liderazgo, que es cálculo.
function leadershipCategory(row = {}, explanation = {}) {
  ...
  if (rs == null) {
    return category({
      ...
      status: "blocked",
      detail: "Percentil del lote no disponible.",
      metrics: [metric("rs", "Percentil lote", rs), metric("rsQualityScore", "Calidad lote", rsQuality)],
    });
  }
  if (rs >= 72 && qualitySignals >= 1) {
    return category({
      ...
      label: "Liderazgo global",
      status: "confirmed",
      detail: `Percentil del lote ${score(rs)} con ${qualitySignals} señal(es) de calidad relativa.`,
      metrics: [metric("rs", "Percentil lote", rs), metric("rsQualityScore", "Calidad lote", rsQuality), metric("weinsteinScore", "Estructura", weinstein), metric("minerviniScore", "Ruptura", minervini)],
    });
  }
  const neutral = rs >= 55;
  return category({
    ...
    detail: rs < 72
      ? `Percentil del lote ${score(rs)} evaluado por debajo de liderazgo; limita la conviccion del caso.`
      : "Percentil del lote suficiente, pero falta confirmar calidad de liderazgo relativo.",
    metrics: [metric("rs", "Percentil lote", rs), metric("rsQualityScore", "Calidad lote", rsQuality), metric("weinsteinScore", "Estructura", weinstein), metric("minerviniScore", "Ruptura", minervini)],
  });
}
  ...
  return category({
    ...
    detail: supportive
      ...
      : neutral
        ...
        : `${sector || "Grupo"} evaluado débil (${score(value)}${sampleDetail}); limita la conviccion del caso.`,
...
function confluenceCategory(row = {}, explanation = {}) {
  ...
  return category({
    ...
    detail: `liderazgo ${leadershipCount}/3 · confirmación ${confirmationCount}/5 · ejecucion ${executionCount}/1`,
    metrics: [metric("setupQualityScore", "Setup", setup)],
  });
}
...
function demandCategory(row = {}) {
  ...
  if (signals >= 1) {
    return category({
      ...
      status: "confirmed",
      detail: `${signals} señal(es) de demanda: volumen efectivo, A/D, RelVol o UD Vol.`,
  ...
  return category({
    ...
    detail: hasMetric(row, ["demandScore", "volumeEffectScore", "adProxyScore", "relativeVolume", "upDownVolRatio"])
      ? "Demanda evaluada débil: no confirma acumulación o volumen relativo suficiente."
      : "Faltan métricas para evaluar demanda/volumen.",
    metrics: [metric("demandScore", "Demanda", demand), metric("volumeEffectScore", "Volumen", volume), metric("adProxyScore", "A/D", ad), metric("relativeVolume", "RelVol", relVol, { suffix: "x" }), metric("upDownVolRatio", "UD Vol", upDown, { suffix: "x" })],
  });
}
...
function riskCategory(row = {}, explanation = {}) {
  ...
  if (!hasMetric(row, ["riskRewardScore", "extSma50", "maxDrawdown63d", "weaknessScore"])) {
    return category({
      ...
      label: "Riesgo operativo",
      status: "needed",
      detail: "Faltan métricas de extension, drawdown o rentabilidad/riesgo.",
      metrics: [metric("riskRewardScore", "Rent/riesgo", riskReward), metric("extSma50", "Ext SMA50", extension, { suffix: "%" }), metric("maxDrawdown63d", "DD63", drawdown, { suffix: "%" }), metric("weaknessScore", "Deterioro", weakness)],
    });
  }
  return category({
...
function dataCategory(row = {}, dataHealth = {}) {
  ...
  if (status === "ready") {
    return category({
      ...
      label: "Datos fiables",
      status: "confirmed",
      detail: dataHealth.topLine || dataHealth.status?.detail || "Cobertura, frescura y metodología sin bloqueo principal.",
...
function contradictionItems(row = {}, explanation = {}, dataHealth = {}) {
  ...
  if (total != null && total >= 72 && demandSignals < 1) {
    items.push({ key: "high-score-without-demand", severity: "warn", detail: `score ${score(total)} sin confirmación de demanda` });
  }
...
export function buildRowMethodologyEvidence(row = {}, { explanation = {}, settings = {} } = {}) {
  ...
  return {
    ...
    summary: pending.length ? pending.slice(0, 2).map((item) => item.label).join(" · ") : "Pruebas metodológicas confirmadas",
    categories,
    pending,
    blocked,
    reviewFocus,
    ...
    reviewSummary: manualReviewRequired
      ...
      : reviewFocus.length
        ...
        : "Sin brecha metodológica principal.",
