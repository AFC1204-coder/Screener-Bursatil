...
export async function writeScanSymbolHistory({
  ...
  if (!inserts.length) return { saved: 0, candidates: observations.length, notInUniverse: absenceObservations.length };
  ...
  return {
    saved: Array.isArray(saved) ? saved.length : inserts.length,
