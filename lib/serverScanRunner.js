...
// fetch interno a POST /api/scan/continue autenticado con el token del middleware.
...
function resultPayload(row = {}, scanId, ownerId, rankIndex) {
  return {
    owner_id: ownerId,
    scan_id: scanId,
    symbol: textOrNull(row.symbol) || "-",
    company_name: textOrNull(row.companyName || row.symbol),
...
async function chainNextLink({ baseUrl, scanId, linkToken }) {
  const token = process.env.STATSEDGE_ACCESS_TOKEN || "";
  ...
  try {
    ...
    const flushBatches = async (force = false) => {
      while (state.buffer.length >= RESULT_BATCH_SIZE || (force && state.buffer.length)) {
        const batch = state.buffer.splice(0, RESULT_BATCH_SIZE);
        await supabaseRequest("scan_results", {
          method: "POST",
          prefer: "return=minimal",
          body: batch.map((row, index) => resultPayload(row, scanId, ownerId, state.insertedCount + index + 1)),
        });
    ...
    if (state.completed >= symbols.length) {
      await patchScan(scanId, ownerId, {
        row_count: state.insertedCount,
        settings: progressPayload("done", { finishedAt: new Date().toISOString() }),
      });
      return;
    }
