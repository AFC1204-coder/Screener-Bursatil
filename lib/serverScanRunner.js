  ...
  try {
    ...
    const workers = Promise.all(Array.from({ length: Math.min(SCAN_CONCURRENCY, Math.max(chunkEnd - startCursor, 1)) }, async () => {
      while (!state.cancelRequested && cursor < chunkEnd) {
        ...
        try {
          ...
          // Persiste en daily_bars las barras que este scan ya descargó, para que
          // dejen de congelarse en la última pasada del cron (docs/barras-desfasadas-2026-08-09.md).
          // Reutiliza el mismo escritor que usa el cron (writeDailyBarsCache, vía
          // withDailyBarsCache en materializedScanner.js) — no se duplica lógica de
          // caché ni de caducidad. Deliberadamente SIN awaitear: el usuario está
          // esperando la respuesta del scan (para scans <=20 símbolos el eslabón
          // corre inline, no en after() — ver INLINE_SCAN_SYMBOL_LIMIT en
          // app/api/scan/route.js), así que esta escritura no debe sumarse a esa
          // latencia. writeDailyBarsCache ya no lanza (atrapa sus propios errores
          // y devuelve {status:"error"}), pero el .catch() queda como cinturón y
          // tirantes por si esa garantía cambia — un fallo aquí nunca debe tumbar
          // el scan.
          writeDailyBarsCache(symbol, chart, { interval: "D" }).catch(() => {});
    ...
    if (state.completed >= symbols.length) {
      ...
      try {
        ...
        await patchScan(scanId, ownerId, {
          row_count: state.insertedCount,
          settings: progressPayload(completeness.status, {
