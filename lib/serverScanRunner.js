...
export async function runScanChunk({ scanId, ownerId, baseUrl }) {
  ...
  try {
    ...
    // ===== INSTRUMENTACIÓN TEMPORAL scan-timing — INICIO =====
    // Medición puntual para docs/timeout-tres-minutos-2026-08-10.md: se
    // sospecha que readCancelRequested o patchScan (RPC scan_progress_patch)
    // se cuelgan ~20-23s dentro de este bucle, pero no se ha medido cuál.
    // Cronometra las tres operaciones del bucle sin tocar su lógica ni su
    // orden. Retirar este bloque completo (INICIO/FIN) tras la medición.
    let scanTimingLoop = 0;
    const scanTimingLog = (op, ms) => {
      console.log(`[scan-timing] vuelta=${scanTimingLoop} op=${op} ms=${ms}`);
    };
    const scanTimingWrap = async (op, fn) => {
      const startedAtMs = Date.now();
      const result = await fn();
      scanTimingLog(op, Date.now() - startedAtMs);
      return result;
    };
    // ===== INSTRUMENTACIÓN TEMPORAL scan-timing — FIN (declaración) =====
    while (!state.workersDone) {
      scanTimingLoop += 1; // INSTRUMENTACIÓN TEMPORAL scan-timing
      ...
      state.cancelRequested = state.cancelRequested || await scanTimingWrap("readCancel1", () => readCancelRequested(scanId, ownerId)); // INSTRUMENTACIÓN TEMPORAL scan-timing
      if (state.cancelRequested) break;
      await scanTimingWrap("flushBatches", () => flushBatches(false)); // INSTRUMENTACIÓN TEMPORAL scan-timing
      ...
      state.cancelRequested = state.cancelRequested || await scanTimingWrap("readCancel2", () => readCancelRequested(scanId, ownerId)); // INSTRUMENTACIÓN TEMPORAL scan-timing
      if (state.cancelRequested) break;
      await scanTimingWrap("patchProgress", () => patchScan(scanId, ownerId, { rowCount: state.insertedCount, progress: progressPayload("running"), fallbackSettings: settings })); // INSTRUMENTACIÓN TEMPORAL scan-timing
    ...
    if (state.completed >= symbols.length) {
      ...
      try {
        ...
        const result = await finalizeScanResultsInDb(scanId, ownerId, { onBatchProgress: onFinalizeBatchProgress });
