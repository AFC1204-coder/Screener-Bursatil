...
function weeklyContextFromRow(row = {}, weeklyFromBars = null) {
  if (row.stage?.weekly) return row.stage.weekly;
  if (row.weeklyStage && typeof row.weeklyStage === "object") return row.weeklyStage;
  if (row.weeklyStageState) {
    return {
      state: row.weeklyStageState,
      slowMaSlopePct: row.weeklySlowMaSlope ?? row.slowMaSlopePct,
      flatPct: row.weeklyFlatPct,
      fastWeeks: row.weeklyFastWeeks,
      slowWeeks: row.weeklySlowWeeks,
    };
  }
  return weeklyFromBars;
}

function persistenceFromRowFields(row = {}, slowWeeks, fastWeeks) {
  const slowCount = finite(row.weeksAboveSma30w);
  if (slowCount === null) return { slow: null, fast: null };
  const slowAbove = row.weeksAboveSma30wAbove ?? true;
  const fastCount = finite(row.weeksAboveSma10w);
  const fastAbove = row.weeksAboveSma10wAbove ?? true;
  return {
    slow: { count: slowCount, above: slowAbove },
    fast: fastCount === null ? null : { count: fastCount, above: fastAbove },
  };
}

function persistenceLine(weeks, weekly, config, row = {}) {
  ...
  let slow = consecutiveWeeksRelativeToMa(weeks, slowWeeks);
  let fast = consecutiveWeeksRelativeToMa(weeks, fastWeeks);
  if (slow.count === null) {
    const fromRow = persistenceFromRowFields(row, slowWeeks, fastWeeks);
    if (fromRow.slow) slow = fromRow.slow;
    if (fast.count === null && fromRow.fast) fast = fromRow.fast;
  }
...
function accelerationLine(bars, row = {}) {
  ...
  const recent = finite(row.advanceRecentPct)
    ?? finite(row.perf3m)
    ?? perf(bars, 63);
 ...
 */
/**
 * Normaliza una fila de scan/screener para buildTrendSupportLines sin barras
 * en modal (vista rápida): reutiliza campos ya proyectados en el nocturno.
 */
export function trendSupportInputFromScanRow(scanRow = {}) {
  return {
    stage: scanRow.stage,
    weeklyStage: scanRow.weeklyStage,
    weeklyStageState: scanRow.weeklyStageState,
    weeklySlowMaSlope: scanRow.weeklySlowMaSlope,
    weeklyFlatPct: scanRow.weeklyFlatPct,
    weeklyFastWeeks: scanRow.weeklyFastWeeks,
    weeklySlowWeeks: scanRow.weeklySlowWeeks,
    perf6m: scanRow.perf6m,
    perf3m: scanRow.perf3m,
    upDownVolRatio: scanRow.upDownVolRatio,
    advanceRecentPct: scanRow.advanceRecentPct,
    advancePriorPct: scanRow.advancePriorPct,
    weeksAboveSma30w: scanRow.weeksAboveSma30w,
    weeksAboveSma30wAbove: scanRow.weeksAboveSma30wAbove,
    weeksAboveSma10w: scanRow.weeksAboveSma10w,
    weeksAboveSma10wAbove: scanRow.weeksAboveSma10wAbove,
  };
}
...
export function buildTrendSupportLines(bars = [], row = {}, options = {}) {
  const config = normalizeWeeklyStageSettings(options);
  const weeks = weeklyBarsFromDaily(bars);
  const weeklyFromBars = weeklyStageForBars(bars, options);
  const weekly = weeklyContextFromRow(row, weeklyFromBars);
  return {
    title: "Sostén de la tendencia",
    lines: [
      persistenceLine(weeks, weekly, config, row),
