...
const SOCIAL_BEARISH_RULES = [
  ...
  [/\$?(spy|qqq|iwm|nasdaq|spx).{0,32}\b(red|dump|selloff|breakdown|lower|crash)\b/i, -9, "indices debiles"],
];
...
export function summarizeSocialSentiment(rows = []) {
  ...
  let regime = "Neutral";
  let contrarianRead = "Ruido social equilibrado: conviene dar mas peso a precio, volumen y amplitud.";
  if (pessimismIndex >= 78) {
    regime = "Miedo social extremo";
    contrarianRead = "Lectura contraria potencial: si el mercado deja de caer y aparecen lideres, el pesimismo social puede señalar agotamiento vendedor.";
  } else if (pessimismIndex >= 63) {
    regime = "Negatividad social dominante";
    contrarianRead = "La conversacion esta defensiva. En analisis tecnico por etapas, mejor exigir confirmacion de medias y amplitud antes de aumentar riesgo.";
    ...
    contrarianRead = "Tono social constructivo. Es util si acompaña a liderazgo real, no si solo hay entusiasmo sin precio.";
  }
  return {
    ...
    regime,
    contrarianRead,
    note: "Indice heuristico basado en posts publicos recientes; no es analisis semantico propietario ni recomendacion.",
    rows: analyzed,
  };
}
...
export async function fetchXMarketPosts(options = {}) {
  ...
  if (!token) {
    return {
      ...
      query,
      rows: [],
      error: "X no configurado: define X_BEARER_TOKEN o TWITTER_BEARER_TOKEN en el entorno.",
    };
  }
