...
// Whitespace a la izquierda del primer bar (fixLeftEdge:false): desviación
// manual aunque el último dato siga visible en el borde derecho.
export function hasLeftWhitespaceLogicalRange(logicalRange = null) {
  const from = Number(logicalRange?.from);
  return Number.isFinite(from) && from < -0.5;
}
