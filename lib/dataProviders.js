...
export function providerStatus() {
  return PROVIDER_PLAN.map((provider) => ({
    ...provider,
    configured: provider.id === "jquants" ? jquantsStatus().configured : (provider.optionalKey ? true : (provider.envKey ? hasEnvValue(provider.envKey) : true)),
    keyConfigured: provider.id === "jquants" ? jquantsStatus().configured : (provider.envKey ? hasEnvValue(provider.envKey) : null),
    runtime: provider.id === "openfigi" ? openFigiStatus() : (provider.id === "jquants" ? jquantsStatus() : null),
  }));
}
