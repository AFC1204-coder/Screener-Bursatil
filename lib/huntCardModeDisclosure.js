 ...
 * }}
 */
/** Familias de la ficha activa (orden diario del aside SHELL-C). */
export function huntCardSheetFamilyKeys({ cardId = "", presetKey = "", markets = [] } = {}) {
  const card = cardId
    ? huntCardById(cardId)
    : resolveActiveHuntCard(presetKey, markets);
  if (!card) return PRESET_DOOR_PRIORITY.balanced;
  return PRESET_DOOR_PRIORITY[card.presetKey] || PRESET_DOOR_PRIORITY.balanced;
}

export function huntCardModeDisclosure({ cardId = "", presetKey = "", markets = [], activeSettings = null } = {}) {
  const card = cardId
    ? huntCardById(cardId)
    : resolveActiveHuntCard(presetKey, markets);
  if (!card) return null;
  ...
  const values = activeSettings || settingsForPreset(effectivePresetKey);

  if (effectivePresetKey === "ipoDiscovery") {
    const maxMonths = Number.isFinite(Number(values.maxIpoAgeMonths))
      ? Number(values.maxIpoAgeMonths)
      : 24;
    const doors = [
      {
        label: `Salida ≤ ${maxMonths}m`,
        familyKey: "ipo",
        familyLabel: familyLabel("ipo"),
      },
      {
        label: "Liquidez mínima (precio, cap, volumen)",
        familyKey: "liquidity",
        familyLabel: familyLabel("liquidity"),
      },
    ];
    return {
      cardId: card.id,
      ...
      mode: "lente",
      modeLabel: "Lente",
      modeDesc: "Cohort por edad de salida, sin puertas de tendencia ni setup.",
      modeBadgeLabel: "Lente",
      doors,
      summaryLine: `Lente · 1 puerta: salida ≤ ${maxMonths} m · liquidez mínima · sin puertas de tendencia`,
    };
  }
