...
const DATA_HEALTH_FILTERS = [
  ...
  { key: "unknown", label: "Sin auditoria", shortLabel: "Sin audit", tone: "muted" },
];
...
function issueItems(row = {}, gate = {}, freshness = {}) {
  ...
  if (rowValue(row, "decisionProjectionPartial") === true) {
    items.push({
      ...
      tone: "bad",
      detail: missing.length ? missing.join(", ") : "faltan campos de decision",
    });
  }
...
function statusFor({ gate, freshness, coverage, technical, fundamental, issues }) {
  if (issues.some((item) => item.tone === "bad") || gate?.passed === false) {
    return {
      ...
      label: "Bloqueado",
      tone: "bad",
      detail: "No usar como decision sin revisar datos base.",
    };
  }
  if (freshness.stale) {
    return {
  ...
  if ((Number.isFinite(coverage) && coverage < 60) || (Number.isFinite(technical) && technical < 65) || (Number.isFinite(fundamental) && fundamental < 30) || issues.length) {
    return {
      ...
      label: "Parcial",
      tone: "warn",
      detail: "Cobertura suficiente para ranking, limitada para decision final.",
    };
  }
  if (Number.isFinite(coverage) || freshness.ok || gate?.passed === true) {
    return {
      ...
      label: "Datos aptos",
      tone: "good",
      detail: "Base tecnica fresca y cobertura util.",
    };
  }
  return {
    key: "unknown",
...
export function buildScreenerDataHealth(row = {}, settings = {}) {
  ...
  const metrics = [
    ...
    metricItem("technical", "Tec", technical, { detail: "precio, grafico, volumen y RS", good: 70, warn: 55 }),
...
function dataHealthSummaryVerdict(counts = new Map(), total = 0) {
  ...
  return { key: "ready", label: "Base apta", tone: "good", detail: `${ready} filas listas para investigacion.` };
}
