...
function missingDecisionEvidence(row = {}) {
  return [
    ...
    ["technicalCoverageScore", "cobertura técnica"],
    ["fundamentalCoverageScore", "cobertura fundamental"],
    ["rsQualityScore", "Calidad lote"],
...
function normalModeDrivers(row, drivers) {
  ...
  if (rowValue(row, "setupDisplayPlanValid") === true) {
    addUnique(drivers, { key: "setup-plan", label: "Plan válido", tone: "strong", text: rowText(row, "setupDisplayReason") || "Setup/VCP con plan válido" });
  ...
  // "RS universo" era engañoso: el número es el percentil del símbolo dentro
  // de su lote de escaneo (rsGlobalPct), el mismo que consume el score, NO el
  // RS del ranking semanal que enseña la interfaz (lib/rsCanonical.js). Con la
  // etiqueta antigua la vista rápida mostraba un segundo número llamado "RS"
  // que contradecía al del panel. Cambiar la entrada sería cambiar la
  // evaluación de liderazgo del motor de decisión; cambiar la etiqueta no.
  addScoreDriver(drivers, "rs-global", "Percentil lote", rowNumber(row, "rsGlobalPct") ?? rowNumber(row, "rsRating"), 72, 82);
  addScoreDriver(drivers, "weinstein", "Estructura", rowNumber(row, "weinsteinScore"), 72, 82);
  addScoreDriver(drivers, "minervini", "Ruptura", rowNumber(row, "minerviniScore"), 72, 82);
  addScoreDriver(drivers, "rs-group", "RS grupo", rowNumber(row, "rsSectorPct"), 70, 80);
  addScoreDriver(drivers, "rs-quality", "Calidad lote", rowNumber(row, "rsQualityScore"), 68, 78);
...
function addFallbackDrivers(row, drivers) {
  ...
  if (drivers.length) return;
  addScoreDriver(drivers, "risk", "Riesgo técnico", rowNumber(row, "riskScore"), 78, 90);
...
function buildRankAction({ weaknessMode, drivers, watch, displayWatch, tone }) {
  ...
  if (hasKey(drivers, "weakness-dominates")) {
    return rankAction("avoid-long", "bad", "La debilidad técnica domina y no hay tesis larga clara.");
  }
  ...
  if (hasKey(drivers, "setup-strict") || hasKey(drivers, "setup-watch")) {
    return rankAction("watch-setup", displayWatch.length ? "warn" : "neutral", "Estructura observable; esperar confirmación o mejor punto.");
  }
  if (tone === "good") {
...
export function buildDecisionBrief(row = {}, settingsOrExplanation = {}) {
  ...
  const action = explanation.action || rankAction("unclear", "muted", "Sin acción definida.");
  ...
  const riskDetail = primaryWatch
    ...
    : readiness.key === "operable"
      ? "La cola no detecta bloqueos relevantes; validar precio y volumen en el gráfico."
