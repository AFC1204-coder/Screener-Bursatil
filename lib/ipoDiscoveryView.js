...
// ── Desde salida (IPO-UX-D) ───────────────────────────────────────────────
// % desde la salida: precio actual vs primer cierre usable ≥ ipoDate en la
// serie ya presente en la fila (`chartPreview`, ascendente por fecha).
//
// Convención:
//   - Solo IPO ya cotizada (`ipoDate` verificada); pre-IPO / fecha estimada → ausencia.
//   - Ancla: primera barra de chartPreview con date ≥ ipoDate y close > 0.
//   - Actual: row.price; si falta, último close de chartPreview.
//   - Si la ventana de chartPreview (~48 sesiones en fila ligera) no alcanza
//     hasta ipoDate, no se inventa ancla ni se pide serie extra aquí.
