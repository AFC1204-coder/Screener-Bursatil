...
async function fetchOfficialMarketUniverse(code) {
  ...
  if (code === "TW") return fetchTwseUniverse();
  if (code === "GB") return fetchFcaFirdsUniverse(code);
  if (FIRDS_MARKETS.has(code)) return fetchFirdsUniverse(code);
  return [];
}
...
export async function getUniverse(market) {
  ...
  if (code === "US") {
    const dynamic = await fetchUSUniverse();
