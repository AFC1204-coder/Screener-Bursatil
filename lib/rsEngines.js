...
// RS tema (MET-3b): un engine_version por theme curado (12 keys), USD + FX.
export const THEME_RS_ENGINE_PREFIX = "statsedge-private-theme-rs-usd-";
export const THEME_RS_ENGINE_SUFFIX = "-v1";

/** Slug ASCII para engine_version: minúsculas, espacios y / → guión. */
export function themeRsSlug(themeKey = "") {
...
/** engine_version del ranking tema para una de las 12 keys curadas. */
export function themeRsEngineVersion(themeKey = "") {
  const slug = themeRsSlug(themeKey);
  if (!slug || !rankableThemeKeys().includes(String(themeKey || "").trim())) return "";
  return `${THEME_RS_ENGINE_PREFIX}${slug}${THEME_RS_ENGINE_SUFFIX}`;
}

export function isThemeRsKeySupported(themeKey = "") {
  return Boolean(themeRsEngineVersion(themeKey));
}

export function themeRsDisclosure(themeKey = "", sampleSize = null, weekKey = "") {
  const label = String(themeKey || "").trim() || "ocupación";
  const parts = [`RS tema · ${label} · universo privado curado`];
