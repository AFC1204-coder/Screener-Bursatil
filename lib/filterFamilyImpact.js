...
// impacto falso). Activa y recorta → «recorta −N» (RS: «deja fuera ~N»).
// Activa y no recorta → «sin recorte» (tono neutro/gris).
export function filterFamilyImpactCardLabel(impact) {
  if (!impact || !impact.layerOn) return null;
  if (impact.cut > 0) {
    // P9: RS en lenguaje de decisión, no jerga de «recorta −N».
    if (impact.familyKey === "relativeStrength") {
      return { text: `deja fuera ~${impact.cut}`, tone: "cut" };
    }
    return { text: `recorta −${impact.cut}`, tone: "cut" };
  }
...
export function filterFamilyImpactModalLine(impact) {
  ...
  if (impact.cut > 0) {
    if (impact.familyKey === "relativeStrength") {
      return `RS deja fuera ~${impact.cut} · ${impact.remaining} de ${impact.total} con dato`;
    }
