...
function regimeRejectReason(row, marketHealth, enabled, set = {}) {
  ...
  if (s >= 75) return null;
  if (s >= 55) return objectiveScore >= 60 && row.riskScore >= 45 && row.weinsteinScore >= 55 ? null : rejectReason("regime", "Régimen exige score compuesto >= 60, risk >= 45 y estructura de tendencia >= 55");
  if (s >= 40) return objectiveScore >= 72 && row.riskScore >= 55 && row.weinsteinScore >= 65 && row.minerviniScore >= 55 ? null : rejectReason("regime", "Régimen exige score compuesto >= 72, risk >= 55, estructura de tendencia >= 65 y estructura de ruptura >= 55");
  return objectiveScore >= 82 && row.riskScore >= 65 && row.weinsteinScore >= 75 && row.minerviniScore >= 65 ? null : rejectReason("regime", "Régimen exige score compuesto >= 82, risk >= 65, estructura de tendencia >= 75 y estructura de ruptura >= 65");
}
...
function objectiveStage(row = {}) {
  ...
  if (Number.isFinite(price) && Number.isFinite(sma200) && price > sma200) return "Precio > SMA200";
  return userFacingSearchError("Histórico insuficiente");
}
