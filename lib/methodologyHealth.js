...
function detailFor(status, totals = {}, calibration = {}) {
  if (status === "pass") {
    return `${totals.passed ?? "-"} de ${totals.cases ?? "-"} casos pasan; ${calibration.mismatches ?? "-"} desajustes y ${calibration.buckets?.plan ?? "-"} planes automaticos.`;
  }
  if ((totals.failed ?? 0) > 0) return `${totals.failed} casos fallan en la matriz metodologica.`;
  ...
  if ((calibration.buckets?.plan ?? 0) > 0) return "Hay casos marcados como plan antes de que esa senal este habilitada con confianza.";
  return "La matriz metodologica necesita revision.";
}
...
export function parseMethodologyValidationMatrix(markdown = "", options = {}) {
  ...
  return {
    ...
    note: "Valida la coherencia de VCP, vigilancia y bloqueo de Plan valido; no genera recomendaciones de entrada.",
  };
}
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      return {
        ...
        detail: `No se encontro ${sourcePath}. Ejecuta la auditoria metodologica para publicar el estado.`,
        sourcePath,
        parsedAt: new Date().toISOString(),
        ...
        failedRows: [],
        note: "Sin matriz no se debe mostrar la metodologia como validada.",
      };
    }
    throw error;
  }
