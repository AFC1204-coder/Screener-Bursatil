...
function normalizeMarkets(markets = []) {
  const list = Array.isArray(markets) ? markets : String(markets || "US").split(",");
  const clean = list.map((item) => cleanText(item).toUpperCase()).filter(Boolean);
  return [...new Set(clean.length ? clean : ["US"])];
}

function cacheKey(markets = []) {
  return `universe:${normalizeMarkets(markets).sort().join(",")}`;
}
...
async function readSupabaseSnapshot(key, maxAgeHours = DEFAULT_MAX_AGE_HOURS) {
  ...
  const symbols = await supabaseRequest("universe_snapshot_symbols", {
    query: `snapshot_id=eq.${encodeURIComponent(snapshot.id)}&select=*&order=symbol.asc`,
  });
  return dbSnapshotToApi(snapshot, symbols || [], {
    hit: true,
    status: "supabase",
