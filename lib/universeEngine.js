...
function qualityGate(entry = {}) {
  ...
  const reasons = [];
  if (!symbol) reasons.push("sin símbolo");
  if (!/^[A-Z0-9.-]{1,18}$/.test(symbol)) reasons.push("símbolo no soportado");
...
function coverageForUniverseEntry(entry = {}, gate = qualityGate(entry)) {
  ...
  return {
    ...
    universeCoverageLabel: score >= 85 ? "alta" : score >= 65 ? "útil" : score >= 45 ? "parcial" : "baja",
  };
}

function hkExchangeMetadata(entry = {}) {
  const out = {};
  const exchangeCategory = cleanText(entry.exchangeCategory);
  const exchangeSubCategory = cleanText(entry.exchangeSubCategory);
  const isin = cleanText(entry.isin);
  const currency = cleanText(entry.currency);
  if (exchangeCategory) out.exchangeCategory = exchangeCategory;
  if (exchangeSubCategory) out.exchangeSubCategory = exchangeSubCategory;
  if (entry.shortSellEligible === true || entry.shortSellEligible === false) {
    out.shortSellEligible = entry.shortSellEligible;
  }
...
function normalizeEntry(entry = {}, market = "") {
  ...
  return {
    ...
    market,
    micCode: cleanText(entry.micCode || entry.mic_code || entry.tradingVenue).toUpperCase(),
    source: cleanText(entry.source || "universe"),
    qualityGate: gate,
    ...coverage,
    ...hkExchangeMetadata(entry),
  };
}
...
function dbSnapshotRowToApi(row = {}) {
  return {
    symbol: row.symbol,
    name: row.name || row.symbol,
    country: row.country,
    market: row.market,
    micCode: cleanText(row.raw?.micCode || row.raw?.mic_code || row.raw?.tradingVenue).toUpperCase(),
    source: row.source,
    qualityGate: row.quality_gate || {},
    ...(row.coverage || {}),
    ...hkExchangeMetadata(row.raw || row),
  };
}
...
function dbSnapshotToApi(snapshot = {}, symbols = [], cache = {}) {
  const universe = symbols
    ...
    .map((row) => dbSnapshotRowToApi(row));
