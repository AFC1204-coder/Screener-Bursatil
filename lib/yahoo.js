...
function chartRequestOptions({ range = "", interval = "" } = {}) {
  const intraday = Object.hasOwn(YAHOO_INTERVAL_MAP, interval);
  let yahooRange = YAHOO_RANGE_MAP[range] || "2y";
  const yahooInterval = intraday ? YAHOO_INTERVAL_MAP[interval] : "1d";
  if (interval === "1m") yahooRange = range === "1D" ? "1d" : "5d";
  if (["5m", "15m", "30m"].includes(interval) && !["1d", "5d", "1mo"].includes(yahooRange)) yahooRange = "1mo";
  if (["1h", "4h"].includes(interval) && yahooRange === "5y") yahooRange = "2y";
  return { intraday, yahooInterval, yahooRange, requestedRange: range, requestedInterval: interval };
}
