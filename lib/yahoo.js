...
export async function fetchYahooChart(symbol) {
  try {
    ...
    return { ...fallback, meta: { ...fallback.meta, fallbackReason: "Yahoo historico insuficiente" } };
  } catch (error) {
    try {
      const fallback = await fetchStooqChart(symbol);
      return { ...fallback, meta: { ...fallback.meta, fallbackReason: error.message || "Yahoo no disponible" } };
    } catch (fallbackError) {
      throw new Error(`${error.message || "Yahoo no disponible"} · ${fallbackError.message || "Stooq no disponible"}`);
    }
  }
}
