...
import { firstTradeDateFromChartMeta, ipoDateFromEpochSeconds, IPO_DATE_SOURCES } from "@/lib/ipoDate";
...
const STOOQ_API_KEY = envValue("STOOQ_API_KEY");

let yahooAuthCache = null;

function raw(value) {
  if (value === null || value === undefined || value === "") return null;
  if (value && typeof value === "object" && "raw" in value) return raw(value.raw);
  return Number(value);
}
...
function fallbackProfile(symbol = "", quote = {}, search = {}) {
  ...
  return {
    ...
    marketCap: firstFinite(safeNumber(quote.marketCap), search.marketCap),
...
async function getYahooAuth() {
  ...
  if (!crumb || crumb.includes("<")) throw new Error("Yahoo ownership auth no válido");
...
function mergeGrowthMetrics(timeseriesMetrics = {}, quoteMetrics = {}) {
  ...
  if (!Number.isFinite(merged.shortPercentOfFloat)) {
    merged.shortPercentOfFloat = ratioPct(merged.sharesShort, merged.floatShares);
  }
  if (!Number.isFinite(merged.sharesPercentSharesOut)) {
    merged.sharesPercentSharesOut = ratioPct(merged.sharesShort, merged.sharesOutstanding);
  }
  return merged;
}
...
async function fetchStooqChart(symbol) {
  ...
  if (!bars.length) throw new Error("Stooq sin histórico");
  return {
    bars,
    meta: {
...
function chartRequestOptions({ range = "", interval = "" } = {}) {
  const normalizedInterval = normalizeChartInterval(interval);
  const intraday = Object.hasOwn(YAHOO_INTERVAL_MAP, normalizedInterval);
  let yahooRange = YAHOO_RANGE_MAP[range] || "2y";
  const yahooInterval = intraday ? YAHOO_INTERVAL_MAP[normalizedInterval] : "1d";
  if (normalizedInterval === "1m") yahooRange = range === "1D" ? "1d" : "5d";
  if (["5m", "15m", "30m"].includes(normalizedInterval) && !["1d", "5d", "1mo"].includes(yahooRange)) yahooRange = "1mo";
  if (["1H", "4H"].includes(normalizedInterval) && yahooRange === "5y") yahooRange = "2y";
  return { intraday, yahooInterval, yahooRange, requestedRange: range, requestedInterval: normalizedInterval };
}
...
async function fetchYahooChartDirect(symbol, options = {}) {
  ...
  if (!r) throw new Error("Sin histórico Yahoo");
  ...
  const bars = ts.map((t, i) => {
    ...
    return {
      time: t,
      date: request.intraday ? new Date(t * 1000).toISOString() : new Date(t * 1000).toISOString().slice(0, 10),
  ...
  return {
    ...
    meta: {
      ...
      requestedInterval: request.requestedInterval || "D",
      requestedRange: request.requestedRange || "2A",
      yahooInterval: request.yahooInterval,
      yahooRange: request.yahooRange,
      splitEvents,
    },
  };
}
...
export async function fetchYahooChart(symbol, options = {}) {
  ...
  const runFallbacks = async (reason) => {
    if (request.intraday) throw new Error(`${reason || "Yahoo no disponible"} · intradia sin fallback configurado`);
    try {
  ...
  try {
    ...
    return runFallbacks("Yahoo histórico insuficiente");
  } catch (error) {
    return runFallbacks(error.message || "Yahoo no disponible");
  }
}

// Primera cotización según el endpoint v8 de gráficos (`meta.firstTradeDate`,
// en segundos epoch). Es la ÚNICA fuente de Yahoo que devuelve el dato hoy:
// `defaultKeyStatistics.firstTradeDateEpochUtc` ya no viene en quoteSummary y
// el endpoint v7 `finance/quote` responde 401 sin crumb (ver la cabecera de
// lib/ipoDate.js con la medición de las tres). Se pide `range=5d` porque solo
// interesa el meta; las barras van al contenedor de la basura.
export async function fetchYahooFirstTradeDate(symbol) {
  const yahooSymbol = canonicalYahooSymbol(symbol);
  ...
  const res = await fetch(url, { headers: YAHOO_HEADERS, next: { revalidate: 604800 } });
  if (!res.ok) throw new Error(`Yahoo chart HTTP ${res.status}`);
  const data = await res.json();
  const meta = data?.chart?.result?.[0]?.meta;
  if (!meta) throw new Error(data?.chart?.error?.description || "Yahoo chart sin meta");
  return firstTradeDateFromChartMeta(meta);
}

export async function fetchYahooProfile(symbol) {
  const yahooSymbol = canonicalYahooSymbol(symbol);
  const [summaryResult, fundamentalsResult, searchResult, quoteResult, firstTradeResult] = await Promise.allSettled([
    ...
    fetchYahooFirstTradeDate(yahooSymbol),
  ]);
  ...
  // Orden de preferencia, mismo que resolveIpoDate (lib/ipoDate.js): el meta
  // del gráfico primero porque es el único que responde hoy — medido el
  // 2026-08-28 sobre 300 símbolos US (94,7% de cobertura, toda de esta fuente)
  // y 18 internacionales (100%). Los otros tres se conservan porque no cuestan
  // una petición extra (ya venían en respuestas que se piden igual) y porque
  // un proveedor de reserva podría traerlos, pero hoy los cuatro casos medidos
  // los devolvieron vacíos. Cada candidato lleva su procedencia para que
  // `ipoDateSource` diga la verdad sobre de dónde salió el dato.
  const firstTradeCandidates = [
    [firstTradeResult.status === "fulfilled" ? firstTradeResult.value : null, IPO_DATE_SOURCES.chartMeta],
    [stats.firstTradeDateEpochUtc?.raw, IPO_DATE_SOURCES.keyStatistics],
    [price.firstTradeDateMilliseconds ? Math.floor(price.firstTradeDateMilliseconds / 1000) : null, IPO_DATE_SOURCES.price],
    [quote.firstTradeDateMilliseconds ? Math.floor(quote.firstTradeDateMilliseconds / 1000) : null, IPO_DATE_SOURCES.quote],
  ];
  const [firstTradeRaw, firstTradeSource] = firstTradeCandidates.find(([value]) => Number(value) > 0) || [null, null];
  ...
  const sourceErrors = [
    ...
    // firstTradeResult NO entra aquí a propósito. `profileProviderError` es lo
    // que la ficha usa para avisar de "fundamentales no disponibles", y un 404
    // del endpoint de gráficos no dice nada sobre el perfil: sector, industria
    // y crecimiento pueden haber llegado enteros. La ausencia de la fecha se
    // declara en su propio canal (`ipoDateReason`, lib/ipoDate.js), que es
    // donde el filtro IPO la busca.
  ].filter(Boolean);
  ...
  if (!Number.isFinite(growthMetrics.shortRatio)) {
    growthMetrics.shortRatio = ratio(growthMetrics.sharesShort, valuationMetrics.averageVolume3m);
  }

  return {
    name: firstText(price.longName, price.shortName, fallback.name, yahooSymbol),
    marketCap: firstFinite(safeNumber(price.marketCap), fallback.marketCap, quoteSnapshot.marketCap),
    ...
    ipoDate: ipoDateFromEpochSeconds(firstTradeRaw),
    ipoDateSource: firstTradeRaw ? firstTradeSource : null,
