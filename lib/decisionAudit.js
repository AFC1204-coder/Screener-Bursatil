...
export function decisionPriorityBreakdown(row = {}, settingsOrExplanation = {}, knownIssues = null) {
  const explanation = settingsOrExplanation?.action && settingsOrExplanation?.readiness
    ? settingsOrExplanation
    : explainScreenerRank(row, settingsOrExplanation || {});
  // knownIssues: el annotate O(pasan) ya auditó una vez; no re-auditar en el mismo gesto.
  ...
  const components = [
    ...
    {
      // OJO con la etiqueta: este componente NO es el RS que el producto
      // enseña. Es el percentil del símbolo dentro de su lote de escaneo
      // (rsGlobalPct), que es lo que el score compuesto y esta prioridad
      // consumen de verdad. El RS de la interfaz es el ranking semanal del
      // universo (lib/rsCanonical.js) y puede ser otro número.
      // Se llamaba "RS" a secas y por eso este panel enseñaba un tercer valor
      // bajo el mismo nombre en la vista rápida. Cambiar la ENTRADA sería
      // cambiar el cálculo de la prioridad; cambiar la ETIQUETA no.
      key: "rs",
      label: "Percentil lote",
      value: rs * 1.2,
      raw: rs,
      detail: Number.isFinite(rs) ? `${Math.round(rs)} x1.2 · percentil del lote, no el RS semanal` : "",
    },
    {
      key: "riskReward",
      label: "Rent/riesgo",
...
export function compactErrorMessage(value = "") {
  const text = String(value || "")
    ...
    .trim();
  if (!text) return "";
  // Sin nombres de servicio ni de proveedor de red: esta función la comparten
  // los scripts de auditoría y cualquier superficie que muestre su salida, y
  // no hay forma de saber desde aquí en cuál de las dos acabará el texto. El
  // detalle técnico sigue disponible en el error original que recibe el caller.
  if (/aborted|timeout|timed out/i.test(text)) return "Tiempo de espera agotado al consultar los datos.";
  if (/Error code\s*\d+/i.test(text)) return "El servidor de datos devolvió un error temporal.";
