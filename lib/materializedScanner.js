...
import { stageHealthFieldsFromBars } from "@/lib/stageHealth";
import { trendSupportFieldsFromBars } from "@/lib/trendSupport";
...
function buildResearchRow(symbol, chart = {}, profile = {}, benchmarks = {}, options = {}) {
  ...
  const weeklyStageStructure = weeklyStageStructureForBars(calcBars, {
    ...options,
    weeklyStageState: weeklyStage.state,
  });
  const setupPattern = setupPatternForBars(calcBars, { ...options, rawBars: chart.bars || [] });
  const trendSupport = trendSupportFieldsFromBars(calcBars, options);
  const stageHealth = stageHealthFieldsFromBars(calcBars, {
    weeklyStageState: weeklyStage.state,
    perf3m,
    perf6m,
    upDownVolRatio: udVol(calcBars, 50),
    distanceSma30w: weeklyStage.distanceSlowMaPct,
    weeklyDistanceSlowMa: weeklyStage.distanceSlowMaPct,
    ...trendSupport,
  }, options);
  const row = {
    symbol,
    ...
    ...weeklyStageStructureFields(weeklyStageStructure),
    ...trendSupport,
    ...stageHealth,
    ...setupPattern,
