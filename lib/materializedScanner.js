...
import { DEFAULT_SCAN_MARKETS, EUROPE_PRIORITY_MARKETS, normalizeMarketList } from "@/lib/markets";
...
const CURATED_CORE_SCAN_MARKETS = new Set(["HK", "AU", "KR", "IN", "CA", ...EUROPE_PRIORITY_MARKETS]);
...
function selectUniverseRows(snapshot = {}, options = {}) {
  ...
  const curatedCoreMarket = curatedCoreMarketFromList(markets);
  let curatedCount = 0;
  if (curatedCoreMarket) {
    const core = prependCuratedCoreRows(rows, curatedCoreMarket);
    rows = core.rows;
    curatedCount = core.curatedCount;
  }

  // HK/AU/KR/IN/CA/Europa priority: conservar orden curado→extra. El sort de
  // materialización volvería a mezclar el dump oficial por encima del núcleo líquido.
  const orderedRows = curatedCoreMarket
    ? rows
  ...
  const selection = {
    ...
    priorityMode: curatedCoreMarket ? "curated-core" : (prioritizeMaterialization ? "materialization-priority" : "cursor"),
    selectedReasons: {},
  ...
  if (!perMarket) {
    ...
    const start = curatedCoreSelectionStart(
  ...
  const marketStarts = Object.fromEntries(markets.map((market) => [market, curatedCoreSelectionStart(
