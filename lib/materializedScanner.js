...
function selectUniverseRows(snapshot = {}, options = {}) {
  ...
  if (!perMarket) {
    ...
    const result = scanSelectionCursor(rows, start, limit, excludedSymbols);
    ...
    selection.nextMarketOffsets[key] = rows.length && result.cursor < rows.length ? result.cursor : 0;
...
async function readRecentlyScannedSymbols(options = {}) {
  ...
  const maxRows = Math.min(Math.max(Number(options.recentScanMaxRows || DEFAULT_RECENT_SCAN_MAX_ROWS), 1), 50000);
  const markets = normalizeMarketList(options.markets || DEFAULT_MATERIALIZED_MARKETS, DEFAULT_MATERIALIZED_MARKETS);
  ...
  const rows = await supabaseRequestAll("scan_results", {
    query: `owner_id=eq.${encodeURIComponent(config.ownerId)}&created_at=gte.${encodeURIComponent(since)}&select=symbol,country,created_at&order=created_at.desc`,
  }, {
    maxRows,
  });
  ...
  for (const row of rows || []) {
    ...
    if (!byMarketSets[key]) byMarketSets[key] = new Set();
...
async function resolveSymbols(options = {}) {
  ...
  if (options.skipRecentlyScanned) {
    try {
  ...
  const selection = selectUniverseRows(snapshot, { ...options, markets, excludedSymbols: recentScanExclusion?.symbols });
