...
function scoreObjectiveSetupQuality(row) {
  let score = 0;
...
function scorePatternContribution(row) {
  return methodologyPatternEvidenceBonus(row);
}

function scorePatternQuality(row) {
  const contribution = scorePatternContribution(row);
  if (!contribution) return 0;
  const quality = firstFinite(row.patternQualityScore, row.baseQualityScore, row.contractionScore);
  return Number.isFinite(quality) ? clamp(quality) : clamp(contribution * 4);
}

function scoreSetupQuality(row) {
  let score = scoreObjectiveSetupQuality(row);
  score += scorePatternContribution(row);
...
function sectorize(rows = []) {
  ...
  return enrichRelativePercentiles(rows).map((row) => {
    ...
    const objectiveSetupScore = scoreObjectiveSetupQuality(row);
    const patternContributionScore = scorePatternContribution(row);
    const patternScore = scorePatternQuality(row);
    ...
    const compositeScore = scoreCompositeValue({ setupQualityScore, rsAnchor, rsQualityScore, demandScore, adProxyScore, growthScore, epsAnchor, sectorScore, riskRewardScore, riskScore: row.riskScore, momentumScore: row.momentumScore });
