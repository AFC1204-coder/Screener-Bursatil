...
export const SCAN_CRON_GROUPS = [
  ...
  // Secondary EU (DK/NO/FI/BE/PT/AT/IE): deeper curated scan batch per night.
  // limit/perMarket here is DB-snapshot read depth only — not ESMA_FIRDS inline
  // download (see header comment on EU1/EU2 exclusion above).
...
// Shadow-europe thin cohorts rotate resolve/price/scan on the DB snapshot
// (no FIRDS re-download). They extend mesa yield for light secondary markets
// (AT/BE/IE/PT) that SCAN_CRON_GROUPS already touches but shadow-europe did
// not. SHADOW_FIRDS_CRON_GROUPS is a separate path: daily ESMA seed refresh
// that downloads FIRDS ZIP per market — same IE+PT / AT+BE pairings, different
// caps and purpose (reference seeding vs priced scan materialization).
export const SHADOW_EUROPE_CRON_GROUPS = [
  ...
  {
    key: "shadow-europe-pair-ie-pt",
    title: "Shadow Europe pair — IE+PT",
    markets: ["IE", "PT"],
    resolvePerMarket: 3,
    pricePerMarket: 6,
    scanPerMarket: 6,
  ...
  {
    key: "shadow-europe-pair-at-be",
    title: "Shadow Europe pair — AT+BE",
    markets: ["AT", "BE"],
    resolvePerMarket: 3,
    pricePerMarket: 6,
    scanPerMarket: 6,
