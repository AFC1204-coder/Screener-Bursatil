...
function weeklyRsRowWeekKey(row = {}) {
  const weekKey = String(row.week_key || "").trim();
  if (weekKey) return weekKey;
  return toDate(row.snapshot_date) || "";
}

function countDistinctWeeksForEngine(rows = [], engineVersion = "") {
  const engine = String(engineVersion || "").trim();
  if (!engine) return 0;
  const keys = new Set();
  for (const row of rows) {
    if (String(row?.engine_version || "") !== engine) continue;
    if (!Number.isFinite(finiteOrNull(row?.rs_rating))) continue;
    const key = weeklyRsRowWeekKey(row);
    if (key) keys.add(key);
  }
  return keys.size;
}

function latestSnapshotTimeForEngine(rows = [], engineVersion = "") {
  const engine = String(engineVersion || "").trim();
  if (!engine) return 0;
  let latest = 0;
  for (const row of rows) {
    if (String(row?.engine_version || "") !== engine) continue;
    if (!Number.isFinite(finiteOrNull(row?.rs_rating))) continue;
    latest = Math.max(latest, dateTime(row.snapshot_date));
  }
  return latest;
}

/** Elige un solo engine_version para la serie RS global de ficha/chart. */
export function selectGlobalRsSeriesEngineVersion(
  rows = [],
  {
    pinnedEngine = canonicalRsEngineVersion(),
    minWeeks = RS_LINE_MIN_WEEKS,
  } = {},
) {
  const list = Array.isArray(rows) ? rows : [];
  const pinnedWeeks = countDistinctWeeksForEngine(list, pinnedEngine);
  if (pinnedWeeks >= minWeeks) return pinnedEngine;

  const engines = [...new Set(list.map((row) => String(row?.engine_version || "").trim()).filter(Boolean))];
  let bestEngine = "";
  let bestWeeks = -1;
  let bestLatest = -1;
  for (const engine of engines) {
    if (engine === pinnedEngine) continue;
    const weeks = countDistinctWeeksForEngine(list, engine);
    if (weeks < minWeeks) continue;
    const latest = latestSnapshotTimeForEngine(list, engine);
    if (weeks > bestWeeks || (weeks === bestWeeks && latest > bestLatest)) {
      bestEngine = engine;
      bestWeeks = weeks;
      bestLatest = latest;
    }
  ...
  if (pinnedWeeks > 0) return pinnedEngine;
  return list[0]?.engine_version || "";
}

function mapWeeklyRowsToGlobalSeries(sameEngineRows = []) {
  return dedupeWeeklyRsSeriesByWeekKey(
    (sameEngineRows || [])
      .map((row) => ({
...
export async function readGlobalRsSeriesForSymbol(symbol = "", options = {}) {
  ...
  if (!config.configured || !clean) {
    return { configured: config.configured, series: [], latest: null, ratingLatest: null };
  }
  ...
  // existe). El PIN manda para el rating de mesa (readGlobalRsForSymbols); aquí
  // solo caemos a otro motor si el pin tiene menos de RS_LINE_MIN_WEEKS semanas
  // distintas y otro engine del mismo símbolo sí alcanza el umbral. Sin pin con
  // historia usable, se enseña la mejor serie alternativa etiquetada; si ninguna
  // alcanza el mínimo pero el pin tiene filas, se conserva el pin (mensaje «Sin
  // línea RS» honesto).
  const selectedEngineVersion = selectGlobalRsSeriesEngineVersion(rows || []);
  ...
  const sameEngineRows = selectedEngineVersion
    ...
    : (rows || []);
  const series = mapWeeklyRowsToGlobalSeries(sameEngineRows);
  const latest = series.at(-1) || null;
  const ratingLatest = pinnedEngine === selectedEngineVersion
    ? latest
    : (mapWeeklyRowsToGlobalSeries((rows || []).filter((row) => row.engine_version === pinnedEngine)).at(-1) || null);
  return {
    configured: true,
    symbol: clean,
    series,
...
export async function hydrateRowsWithWeeklyRs(rows = []) {
  ...
  const { hydrateRowsWithWeeklyThemeRs } = await import("@/lib/themeRsHydrate");
  ...
  const withCountry = await hydrateRowsWithWeeklyCountryRs(withGlobal);
  return hydrateRowsWithWeeklyThemeRs(withCountry);
}
