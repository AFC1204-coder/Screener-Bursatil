...
export function companyFaviconUrl(domain = "") {
  return domain ? `https://www.google.com/s2/favicons?domain=${encodeURIComponent(domain)}&sz=128` : "";
}

export function companyClearbitUrl(domain = "") {
  return domain ? `https://logo.clearbit.com/${domain}` : "";
}

/** Mesa: favicon primero, Clearbit si falla. */
export function companyMarkLogoCandidates(row = {}) {
  const domain = companyLogoDomain(row);
  return [companyFaviconUrl(domain), companyClearbitUrl(domain)].filter(Boolean);
}

/** Ficha /stock: Clearbit primero, favicon del brief si falla. */
export function stockHeroLogoCandidates(visual = {}) {
  return [visual?.clearbitLogoUrl, visual?.logoUrl].filter(Boolean);
}
