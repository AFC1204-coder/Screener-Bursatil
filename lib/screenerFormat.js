...
import { MULTI_MARKET_COMPACT_THRESHOLD } from "@/lib/marketAvailability";
...
function compactScanStatusLists(text = "", threshold = MULTI_MARKET_COMPACT_THRESHOLD) {
  const multiMaterialized = text.match(/^Cargando materializados \((.+)\)…$/);
  if (multiMaterialized) {
    const parts = multiMaterialized[1].split(" + ").filter(Boolean);
    if (parts.length > threshold) {
      return `Cargando ${parts.length} materializados…`;
    }
  }
  const selectionLoading = text.match(/^Cargando datos de la selección \((.+)\)…$/);
  if (selectionLoading) {
    const codes = selectionLoading[1].split("+").filter(Boolean);
    if (codes.length > threshold) {
      return `Cargando ${codes.length} mercados…`;
    }
  }
  return text;
}
...
export function investorStatusLabel(text = "") {
  ...
  return compactScanStatusLists(normalized);
}

/** Alias histórico: misma compactación que investorStatusLabel (desktop + móvil). */
export function compactMobileScanStatus(text = "") {
  return investorStatusLabel(text);
}
...
/** Empty Negocio en Vista rápida — mismo tono que FICHA (`resolveStockCompanyBriefSurface`). */
export const QUICK_BUSINESS_EMPTY = "Sin descripción de negocio disponible para este valor.";

/**
 * Resumen de negocio del modal. Solo texto usable del proveedor; no inventa
 * «opera en {industria}…» a partir de Activity/sector (parece brief lleno).
 */
export function quickBusinessDescription(row = {}) {
  const summary = compactBusinessSummary(row.businessSummary, 300);
  if (summary) return summary;
  return QUICK_BUSINESS_EMPTY;
}

/** Actividad (industria · sector · tema): copy corto, sin guion mudo. */
export function quickBusinessActivity(row = {}) {
  return shortBusiness(row) || "Sin actividad";
}

export function quickBusinessMarket(row = {}) {
  return [marketName(row.country), row.exchange]
    .filter((value, index, arr) => value && value !== "-" && value !== "Sin país" && arr.indexOf(value) === index)
    .join(" · ") || "Sin mercado";
}
...
/** Google favicon primero; Clearbit si esa URL falla. Vacío sin dominio. */
export function companyMarkLogoCandidates(row = {}) {
  const domain = companyLogoDomain(row);
  if (!domain) return [];
  return [googleFaviconUrl(domain), clearbitLogoUrl(domain)].filter(Boolean);
}
