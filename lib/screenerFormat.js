...
/** Status bar móvil: una línea sin volcar nombres de 10+ mercados en el fold. */
export function compactMobileScanStatus(text = "") {
  const normalized = investorStatusLabel(text);
  const multiMaterialized = normalized.match(/^Cargando materializados \((.+)\)…$/);
  ...
  const selectionLoading = normalized.match(/^Cargando datos de la selección \((.+)\)…$/);
  if (selectionLoading) {
