...
import { aggregateVolumeIndicators } from "@/lib/marketVolume";
import { applyThreshold } from "@/lib/volumeDivergence";
...
// NH/NL 52s (MH-FILL-4): distancia al extremo, no cruce intradía.
// nearHigh: distance52w ≥ umbral (precio cerca del máximo 52s).
// nearLow: lowAdvance52w ≤ umbral (precio cerca del mínimo 52s).
export const BREADTH_NEAR_HIGH_52W_PCT = -1;
export const BREADTH_NEAR_LOW_52W_ADVANCE_PCT = 1;
...
export function aggregateUniverseBreadth(rows = []) {
  ...
  for (const row of rows) {
    ...
    const lowAdvance52w = finiteOrNull(row.lowAdvance52w);
    if (lowAdvance52w !== null) measured.lowAdvance52w.push(lowAdvance52w);
  ...
  const indicators = [
    ...
    indicator({
      key: "nearHigh52w",
      label: `Nuevos máximos 52s (≤${Math.abs(BREADTH_NEAR_HIGH_52W_PCT)}% del techo)`,
      count: measured.distance52w.filter((value) => value >= BREADTH_NEAR_HIGH_52W_PCT).length,
      measured: measured.distance52w.length,
      population,
    }),
    indicator({
      key: "nearLow52w",
      label: `Nuevos mínimos 52s (≤${BREADTH_NEAR_LOW_52W_ADVANCE_PCT}% sobre el suelo)`,
      count: measured.lowAdvance52w.filter((value) => value <= BREADTH_NEAR_LOW_52W_ADVANCE_PCT).length,
      measured: measured.lowAdvance52w.length,
      population,
    }),
    indicator({
...
// enuncia como hecho. Acepta un umbral opcional (sin él, comportamiento
// retrocompatible usado por los tests existentes) — el umbral del briefing
// se aplica desde la capa superior vía applyThreshold().
...
export async function readNightlyScanRows(options = {}) {
  const config = supabaseConfig();
  if (!config.configured) return { configured: false, rows: [], scan: null, error: "supabase-disabled" };
  const scan = await readLatestNightlyScan();
  if (!scan?.id) {
    return {
      configured: true,
      rows: [],
      scan: null,
      error: nightlyAbsenceReasonText({ reason: scan?.absenceReason }),
    };
  }
  const rows = await readNightlyRows(config, scan.id, options);
  return {
    configured: true,
    rows,
...
async function readNightlyRows(config, scanId, options = {}) {
  ...
  const select = [
    ...
    "lowAdvance52w:metrics->lowAdvance52w",
    "upDownVolRatio:metrics->upDownVolRatio",
    "volumeDryUpRatio:metrics->volumeDryUpRatio",
    "volumeSurgePct:metrics->volumeSurgePct",
  ...
  return supabaseRequestAll("scan_results", {
    ...
    timeoutMs: options.timeoutMs || ROWS_TIMEOUT_MS,
...
async function computeParticipationSeries(config, latestSnapshot) {
  ...
  return {
    ...
    summary: applyThreshold(participationSummary(weeks)),
  };
}
