...
import { CORE_LAYER_KEYS, OPTIONAL_LAYER_KEYS } from "@/lib/screenerFilterCatalog";
const MARKET_META = {
  ...
  TW: {
    ...
    region: "Asia",
    selectable: false,
    unavailabilityReason: "Materializado TW fallido (cron); sin filas publicables.",
  },
...
// Universo UI seleccionable (sin TW). Presets multi-mercado / tests de cobertura.
const ALL_SELECTABLE_MARKETS = ["US", ...EUROPE, "CA", ...ASIA, "AU", "ZA", "BR", "MX"].filter((code) => code !== "TW");
// Decisión producto A (2026-09-16): sesión diaria fresca / reset abre en EE. UU.
// La última selección persistida en screenerSession prevalece si ya existe.
...
const DEFAULT_STATUS = "Listo · Arranque con escaneo nocturno US; elige un mercado concreto para cargar su materializado";
const SCREENER_FILTER_SETTING = { type: "screener_filters", key: "default" };
const USER_TEMPLATE_LIMIT = 18;
// ipo:false — capa «IPO categoría» retirada de la UI (IPO-UX-C); clave queda
// por compat de sesiones/plantillas. verifiedIpoCategory sigue en CSV/export.
const DEFAULT_VIEW_LAYERS = { country: true, theme: true, sector: true, industry: true, sectorStrength: true, ipo: false };
const VIEW_LAYERS = [
...
// Los índices US van por su ETF (SPY/QQQ/IWM), no por el símbolo con prefijo:
// mismos datos que el resto del sistema y con histórico acumulable en
// daily_bars (decisión 2026-08-16). Los índices extranjeros de esta cinta
// siguen con prefijo (^GSPTSE, ^FTSE, ^N225...): no encajan con una versión
// solo-US y quedan señalados, pero fuera del alcance de ese cambio.
const GLOBAL_INDEX_TAPE = [
  { market: "Global", symbol: "ACWI", label: "MSCI ACWI" },
  { market: "US", symbol: "SPY", label: "S&P 500" },
  { market: "US", symbol: "QQQ", label: "Nasdaq 100" },
  { market: "US", symbol: "IWM", label: "Russell 2000" },
...
// CORE_LAYER_KEYS y OPTIONAL_LAYER_KEYS: fuente única en screenerFilterCatalog.js
