...
//
// ESTRATEGIA DE INVALIDACIÓN (toda la app sigue estas cuatro reglas):
// 1. TTL por tipo de dato (TTL.* abajo): techo de vida de cualquier entrada.
// 2. Corte por día UTC en datos de sesión: las claves de barras diarias incluyen
//    el día (dayStamp en lib/marketData), así el cambio de sesión invalida solo.
// 3. Bypass explícito: ?refresh=1 / ?cache=0 atraviesan TODAS las capas (la de
//    Supabase en dailyBarsCache y esta de memoria) y re-pueblan la entrada.
// 4. Clear-on-write en caches de respuesta API (p. ej. scansCache de /api/scans
//    se vacía en cada POST/DELETE): una lectura posterior a una escritura propia
//    nunca devuelve datos anteriores a esa escritura.
// Observabilidad: stats() por caché, expuestas en GET /api/data-providers.
...
export function createTtlCache({ maxEntries = 2000, name = "cache" } = {}) {
  ...
  function peek(key, { allowExpired = false } = {}) {
    const entry = store.get(key);
    if (!entry) return undefined;
    const expired = entry.expiresAt <= Date.now();
    if (expired && !allowExpired) return undefined;
    return {
      value: entry.value,
      expired,
      expiresAt: entry.expiresAt,
      cachedAt: entry.cachedAt ?? entry.expiresAt,
      staleForMs: expired ? Date.now() - (entry.cachedAt ?? entry.expiresAt) : 0,
    };
  }
  ...
  return { cached, get, peek, set, stats, clear };
}
