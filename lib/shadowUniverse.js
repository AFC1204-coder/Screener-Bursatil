...
export async function buildShadowUniverseReport({ markets = [], refresh = false, maxAgeHours = 24 } = {}) {
  ...
  const snapshot = await getUniverseEngineSnapshot({ markets: normalizedMarkets, refresh, maxAgeHours, allowCuratedFallback: true });
  ...
  return {
    ...
    focus: {
      ...
      rule: "Priorizar usuarios occidentales: EEUU, Canada, Europa, Reino Unido, Japón, Hong Kong y Australia. India/Israel quedan solo como core curado o futuro proveedor.",
    },
    persistence: {
    ...
    legalGuardrails: [
      ...
      "Una acción solo pasa a scannable cuando tiene ticker resuelto, OHLCV fresco y Quality Gate aprobado.",
    ],
    rows,
  };
}
