...
const DEFAULT_SCAN_TIMEOUT_MS = Number(process.env.COVERAGE_SCAN_TIMEOUT_MS || 2500);
...
async function buildScanCoverage({ sinceDays = DEFAULT_SCAN_SINCE_DAYS, maxPriceFreshnessDays = DEFAULT_MAX_PRICE_FRESHNESS_DAYS, minCoverageScore = DEFAULT_MIN_SCAN_COVERAGE, maxRows = DEFAULT_SCAN_MAX_ROWS, timeoutMs = DEFAULT_SCAN_TIMEOUT_MS } = {}) {
...
export async function buildCoverageReport({ markets = CORE_COVERAGE_MARKETS, refresh = false, maxAgeHours = 24, scanMaxRows = DEFAULT_SCAN_MAX_ROWS, scanTimeoutMs = DEFAULT_SCAN_TIMEOUT_MS } = {}) {
  ...
  const [snapshot, scanCoverage] = await Promise.all([
    ...
    buildScanCoverage({ maxRows: scanMaxRows, timeoutMs: scanTimeoutMs }),
  ]);
  const byMarket = snapshot.coverage?.byMarket || {};
