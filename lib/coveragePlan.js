...
import { coverageReadinessForMarket } from "@/lib/coverageReadiness";
...
const DEFAULT_SCAN_MAX_ROWS = Number(process.env.COVERAGE_SCAN_MAX_ROWS || 4000);
const DEFAULT_SCAN_TIMEOUT_MS = Number(process.env.COVERAGE_SCAN_TIMEOUT_MS || 7000);
...
export const MARKET_COVERAGE_TARGETS = {
  ...
  AU: { region: "Australia", investableTarget: 1200, priority: 1, status: "partial", source: "ASIC short reports + curated", nextAction: "Añadir ASX master list oficial/licenciada; mantener ASIC solo para short interest." },
  ...
  TW: { region: "Asia", investableTarget: 900, priority: 3, status: "active", source: "TWSE ISIN listed equities + curated fallback", nextAction: "Monitorizar disponibilidad TWSE y añadir TPEx/.TWO solo si el price mapping queda fiable." },
...
export const IMPLEMENTATION_PHASES = [
  ...
  },
  {
    ...
    status: "in-progress",
    steps: ["Hong Kong via HKEX listados integrado", "Taiwán via TWSE ISIN listados integrado", "Japón via J-Quants V2 al configurar API key", "Europa/EEA via ESMA FIRDS opt-in para referencia ISIN/MIC/CFI y resolución OpenFIGI por lotes", "UK via FCA FIRDS opt-in separado de ESMA", "Europa via ESEF para fundamentales anuales", "Canada/Singapur/Sudáfrica como cores curados legal-safe", "India/Israel diferidos y manuales", "Australia con ASX master list si licencia lo permite"],
  },
  {
    id: "daily-bars-cache",
  ...
  },
  {
    ...
    status: "planned",
    steps: ["SEC EDGAR US", "ESEF anual Europa", "J-Quants statements Japón", "FMP/EODHD como capa premium si la cobertura gratuita no alcanza"],
  },
];
...
export function scanRowShape(row = {}, { maxPriceFreshnessDays = DEFAULT_MAX_PRICE_FRESHNESS_DAYS, minCoverageScore = DEFAULT_MIN_SCAN_COVERAGE, nowMs = Date.now() } = {}) {
  const freshness = priceFreshness(row, maxPriceFreshnessDays, nowMs);
  const coverage = metric(row, "dataCoverageScore");
  const totalScore = firstFinite(row.raw?.totalScore, row.metrics?.totalScore, row.total_score);
  const objectiveScore = firstFinite(row.raw?.objectiveScore, row.metrics?.objectiveScore, totalScore);
...
export function summarizeScanCoverageRows(rows = [], { maxPriceFreshnessDays = DEFAULT_MAX_PRICE_FRESHNESS_DAYS, minCoverageScore = DEFAULT_MIN_SCAN_COVERAGE, nowMs = Date.now() } = {}) {
  const shaped = latestBySymbol((rows || []).map((row) => scanRowShape(row, { maxPriceFreshnessDays, minCoverageScore, nowMs })));
  const byMarket = {};
  let latestScanAt = 0;
  for (const row of shaped) {
    const market = row.country || "-";
    if (!byMarket[market]) byMarket[market] = { uniqueSymbols: 0, fresh: 0, qualityOk: 0, rankingEligible: 0, actionable: 0 };
    ...
    if (row.rankingEligible) {
      byMarket[market].rankingEligible += 1;
      byMarket[market].actionable += 1;
    }
  ...
  return {
    ...
    rankingEligible: shaped.filter((row) => row.rankingEligible).length,
    actionable: shaped.filter((row) => row.rankingEligible).length,
...
export async function buildScanCoverage({ sinceDays = DEFAULT_SCAN_SINCE_DAYS, maxPriceFreshnessDays = DEFAULT_MAX_PRICE_FRESHNESS_DAYS, minCoverageScore = DEFAULT_MIN_SCAN_COVERAGE, maxRows = DEFAULT_SCAN_MAX_ROWS, timeoutMs = DEFAULT_SCAN_TIMEOUT_MS, signal } = {}) {
  const config = supabaseConfig();
  if (!config.configured) return emptyScanCoverage("supabase-disabled", { missing: config.missing });
  const since = new Date(Date.now() - sinceDays * 86400 * 1000).toISOString();
  const now = new Date();
  try {
    const payload = await supabaseRpc("coverage_scan_summary", {
    ...
    const aggregate = Array.isArray(payload) ? payload[0] || {} : payload || {};
    return {
      status: "supabase",
      sinceDays,
      maxPriceFreshnessDays,
      minCoverageScore,
      rowsRead: Number(aggregate.rowsRead || 0),
      uniqueSymbols: Number(aggregate.uniqueSymbols || 0),
      fresh: Number(aggregate.fresh || 0),
      qualityOk: Number(aggregate.qualityOk || 0),
      rankingEligible: Number(aggregate.rankingEligible || 0),
      actionable: Number(aggregate.actionable ?? aggregate.rankingEligible ?? 0),
      latestScanAt: aggregate.latestScanAt || "",
...
function blockersFor(snapshot) {
  ...
  if (snapshot.cache?.status === "supabase-skip") blockers.push({ severity: "high", area: "cache", message: "Las tablas de universe cache no existen todavía en Supabase. Aplica supabase/schema.sql." });
  if (!hasEnvValue("SUPABASE_ACCESS_TOKEN")) blockers.push({ severity: "medium", area: "ops", message: "Falta SUPABASE_ACCESS_TOKEN; no puedo aplicar schema automáticamente desde el script del repo." });
  if (!hasEnvValue("JQUANTS_API_KEY") && !hasEnvValue("JQUANTS_REFRESH_TOKEN")) blockers.push({ severity: "medium", area: "japan", message: "Falta credencial J-Quants; Japón seguira curado hasta configurar cuenta/plan." });
  if (snapshot.markets?.includes("GB") && !/^(1|true|yes|on)$/i.test(envValue("FCA_FIRDS_ENABLED"))) blockers.push({ severity: "low", area: "uk", message: "FCA FIRDS esta implementado pero desactivado para carga automática; UK seguira con core curado salvo jobs shadow manuales." });
...
export async function buildCoverageReport({ markets = CORE_COVERAGE_MARKETS, refresh = false, maxAgeHours = 24, scanMaxRows = DEFAULT_SCAN_MAX_ROWS, scanTimeoutMs = DEFAULT_SCAN_TIMEOUT_MS, signal } = {}) {
  const normalizedMarkets = normalizeMarketList(markets, CORE_COVERAGE_MARKETS);
  const [snapshot, scanCoverage] = await Promise.all([
    getUniverseEngineSnapshot({ markets: normalizedMarkets.length ? normalizedMarkets : CORE_COVERAGE_MARKETS, refresh, maxAgeHours, signal }),
    buildScanCoverage({ maxRows: scanMaxRows, timeoutMs: scanTimeoutMs, signal }),
  ]);
  signal?.throwIfAborted();
  const byMarket = snapshot.coverage?.byMarket || {};
  const bySource = snapshot.coverage?.bySource || {};
  const bySourceByMarket = snapshot.coverage?.bySourceByMarket || {};
  const providerByMarket = snapshot.providerDiagnostics?.byMarket || {};
  const rows = snapshot.markets.map((market) => {
    ...
    const rankingEligible = Number(scan.rankingEligible ?? scan.actionable ?? 0);
    ...
    const readiness = coverageReadinessForMarket({
      ...
      provider: providerByMarket[market] || null,
    });
    return {
      market,
      ...
      scan: {
        ...
        rankingEligiblePct: clampPct((rankingEligible / targetCount) * 100),
  ...
  const rankingEligibleTotal = rows.reduce((sum, row) => sum + row.scan.rankingEligible, 0);
  const actionableTotal = rankingEligibleTotal;
  const regionCoverage = rows.reduce((map, row) => {
    const key = row.region;
    if (!map[key]) map[key] = { region: key, current: 0, target: 0, gap: 0, scanned: 0, fresh: 0, rankingEligible: 0, actionable: 0 };
    map[key].current += row.current;
    map[key].target += row.target;
    map[key].gap += row.gap;
    map[key].scanned += row.scan.uniqueSymbols;
    map[key].fresh += row.scan.fresh;
    map[key].rankingEligible += row.scan.rankingEligible;
    map[key].actionable += row.scan.rankingEligible;
    return map;
  }, {});
  const regions = Object.values(regionCoverage).map((row) => ({
    ...
    rankingEligibleCoveragePct: clampPct((row.rankingEligible / Math.max(row.target, 1)) * 100),
  ...
  const degraded = scanCoverage.status !== "supabase";
  return {
    generatedAt: new Date().toISOString(),
    degraded,
    status: degraded ? "partial-scan-coverage" : "complete",
    objective: "Cobertura maxima razonable de acciones ordinarias/vehiculos listados utiles para el analisis técnico por etapas y estructura, excluyendo instrumentos basura, microcaps e iliquidas.",
    measurementModel: {
      ...
      rankingEligible: "Símbolos escaneados, frescos, con cobertura mínima y puntuación suficiente para aparecer en rankings/listas. No implica setup ni plan válido.",
      actionable: "Alias legacy de rankingEligible; no representa accionabilidad metodológica.",
    ...
    summary: {
      ...
      rankingEligibleSymbols: rankingEligibleTotal,
      rankingEligibleCoveragePct: clampPct((rankingEligibleTotal / Math.max(targetTotal, 1)) * 100),
    ...
    scanCoverage: {
      ...
      rankingEligible: scanCoverage.rankingEligible ?? scanCoverage.actionable,
    ...
    providerDiagnostics: snapshot.providerDiagnostics || null,
    blockers: [
      ...blockersFor(snapshot),
      ...(degraded ? [{
        severity: "high",
        area: "scan-coverage",
        message: `Cobertura de scans incompleta (${scanCoverage.status}): ${scanCoverage.error || "RPC no disponible"}. No usar ceros como cobertura completa.`,
      }] : []),
      ...rows
        .filter((row) => row.readiness?.blocksCoverageClaim)
        .sort((a, b) => {
          const aw = a.readiness.state === "official_source_missing" ? 0 : a.readiness.state === "coverage_gap" ? 1 : 2;
          const bw = b.readiness.state === "official_source_missing" ? 0 : b.readiness.state === "coverage_gap" ? 1 : 2;
          return aw - bw || a.priority - b.priority || b.gap - a.gap;
        })
        .slice(0, 6)
        .map((row) => ({
          severity: row.readiness.state === "official_source_missing" ? "high" : "medium",
          area: `coverage-${row.market.toLowerCase()}`,
          message: `${row.market}: ${row.readiness.label}. ${row.readiness.detail}`,
        })),
    ...
    legalLimits: [
      ...
      "Los listados oficiales suelen permitir consulta publica, pero no siempre redistribucion masiva; cache interno de investigación primero.",
      "Short interest no es comparable entre países sin etiquetar metodología, lag y denominador.",
    ],
  };
}
