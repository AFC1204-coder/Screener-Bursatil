...
export function scanDecisionMetrics(row = {}, settingsOrExplanation = {}) {
  ...
  return {
    ...
    // el scan (percentiles recalculados sobre el universo completo). Ausente
    // se preserva como null — no inventar "batch" en filas sin scope en DB.
    percentileScope: row.percentileScope ?? null,
    ...
    // Los cuatro campos de la fecha de salida a bolsa. Van en `metrics`, no
    // solo en `raw`, porque `requireRecentIpo` / `maxIpoAgeMonths` los leen con
    // metricText/firstFinite (lib/screenerFilters.js) y la proyección
    // `?projection=decision` no pide la columna `raw`: sin esto el filtro IPO
    // no muerde en la superficie que más se consulta. La proyección ligera ya
    // llevaba ipoDate/ipoAgeMonths (lib/scanLightProjection.js), así que hasta
    // ahora las dos formas de la misma tabla no coincidían.
    ipoDate: row.ipoDate ?? null,
    ipoAgeMonths: row.ipoAgeMonths ?? null,
    ipoDateSource: row.ipoDateSource ?? null,
    ipoDateReason: row.ipoDateReason ?? null,
    ipoAnchorClose: row.ipoAnchorClose ?? null,
    ipoAnchorDate: row.ipoAnchorDate ?? null,
    ...
    weeksAboveSma30w: row.weeksAboveSma30w ?? null,
    weeksAboveSma30wAbove: row.weeksAboveSma30wAbove ?? null,
    weeksAboveSma10w: row.weeksAboveSma10w ?? null,
    weeksAboveSma10wAbove: row.weeksAboveSma10wAbove ?? null,
    advanceRecentPct: row.advanceRecentPct ?? null,
    advancePriorPct: row.advancePriorPct ?? null,
    weeklyStageStructure: row.weeklyStageStructure ?? null,
    weeklyStageStructureLabel: row.weeklyStageStructureLabel ?? null,
    weeklyStageStructureDetail: row.weeklyStageStructureDetail ?? null,
    weeklyResistance: row.weeklyResistance ?? null,
    weeklyResistanceDate: row.weeklyResistanceDate ?? null,
    weeklyDistResistancePct: row.weeklyDistResistancePct ?? null,
    weeklyRng26Pct: row.weeklyRng26Pct ?? null,
    weeklyRuptura: row.weeklyRuptura ?? null,
    weeklyHhHl: row.weeklyHhHl ?? null,
    weeklyBreakoutVolRatio: row.weeklyBreakoutVolRatio ?? null,
    weeklyBreakoutVolWeek: row.weeklyBreakoutVolWeek ?? null,
    weeklyBreakoutVolDetail: row.weeklyBreakoutVolDetail ?? null,
    stageHealthScore: row.stageHealthScore ?? null,
    stageHealthAbsenceCode: row.stageHealthAbsenceCode ?? null,
