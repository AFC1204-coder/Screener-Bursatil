...
import { evaluateUnifiedVcpFromDailyBars, isVcpUnifiedEnabled } from "@/lib/vcpEngine";
...
export function setupPatternForBars(bars = [], options = {}) {
  ...
  let contractionCount = depths.filter(Number.isFinite).length;
  const contractionStructureOk = contractionResult.contractionStructureStatus === "ok";
  let contractionsDecreasing = contractionStructureOk && decreasingSequence(depths);
  let contractionReductionPct = depths.length >= 2 && Number.isFinite(depths[0]) && depths[0] > 0 ? ((depths[0] - depths.at(-1)) / depths[0]) * 100 : null;
  let lastContractionDepthPct = depths.at(-1) ?? null;
  ...
  let contractionsTighten = contractionsDecreasing && contractionCount >= 3;
  let vcpCandidate = Boolean(dataGate.patternEligible && context.consolidationCandidate && contractionsTighten && Number.isFinite(volumeDryUpRatio) && volumeDryUpRatio <= 0.85 && Number.isFinite(base.depthPct) && base.depthPct <= 35);
  if (isVcpUnifiedEnabled(options)) {
    const dailyAsc = [...rows].reverse();
    const stage = options.weeklyStage ?? weeklyStageForBars(dailyAsc);
    const structure = options.weeklyStageStructure ?? weeklyStageStructureForBars(dailyAsc, { weeklyStageState: stage.state });
    const vcpUnified = evaluateUnifiedVcpFromDailyBars(rows, {
      stage: stage.state,
      structure: structure.structure,
      distResistancePct: structure.distResistancePct,
    });
    vcpCandidate = vcpUnified.vcpCandidate === true;
    if (Array.isArray(vcpUnified.v7?.contracciones) && vcpUnified.v7.contracciones.length) {
      depths.length = 0;
      depths.push(...vcpUnified.v7.contracciones);
      contractionCount = depths.filter(Number.isFinite).length;
      contractionsDecreasing = vcpUnified.v7.base === true && contractionCount >= 2;
      contractionReductionPct = depths.length >= 2 && Number.isFinite(depths[0]) && depths[0] > 0
        ? ((depths[0] - depths.at(-1)) / depths[0]) * 100
        : null;
      lastContractionDepthPct = depths.at(-1) ?? null;
      contractionsTighten = vcpUnified.v7.base === true && contractionCount >= 2;
    }
  }
