...
function patternDataGate(rows = [], options = {}) {
  ...
  const recentRows = rows.slice(0, Math.min(90, rows.length));
  const volumeRows = rows.slice(0, Math.min(50, rows.length));
  const priceAgeDays = daysSince(rows[0]?.date);
  const issues = [];
  ...
  if (coverageRatio(recentRows, "high") < .95 || coverageRatio(recentRows, "low") < .95 || coverageRatio(recentRows, "close") < .98) issues.push("sparse_ohlc");
  if (coverageRatio(volumeRows, "volume") < .8) issues.push("missing_volume");
...
function consolidationContext(rows = [], { base = {}, pivotPrice, price } = {}) {
  ...
  const establishedCeiling = Number.isFinite(pivotAgeBars) && pivotAgeBars >= 8 || nearPivotDays >= 10;
...
function structuralContractions(rows = [], context = {}) {
  if (!context.consolidationCandidate) return [];
  ...
  const ceiling = Math.max(anchor.high, Number.isFinite(context.pivotPrice) ? context.pivotPrice : anchor.high) * 1.055;
  const floor = anchor.low * 0.985;
  return meaningful
    .filter((item) => item === anchor || item.highDescIndex < anchor.highDescIndex)
    .filter((item) => {
      if (item === anchor) return true;
      const sameCeiling = !Number.isFinite(item.high) || item.high <= ceiling;
      const sameFloor = !Number.isFinite(item.low) || item.low >= floor;
      return sameCeiling && sameFloor;
    })
    .slice(0, 4);
}
...
export function setupPatternForBars(bars = []) {
  ...
  const dataGate = patternDataGate(rows);
  const context = consolidationContext(rows, { base, pivotPrice, price });
  const contractions = dataGate.patternEligible ? structuralContractions(rows, context) : [];
  const depths = contractions.map((item) => item.depthPct);
  const contractionCount = depths.filter(Number.isFinite).length;
  const contractionsDecreasing = decreasingSequence(depths);
  ...
  const verdict = methodologyVerdictForRow({ ...result, setupStructureKey: structure.key, setupStructureStrict: structure.strict, setupStructureReason: structure.reason });
  return {
    ...result,
