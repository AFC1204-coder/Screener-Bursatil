...
export const LEADERBOARD_STRATEGIES = {
  ...
  stage2: {
    ...
    description: "Tendencia alcista estructural confirmada por medias de largo plazo alineadas.",
  },
  nearPivot: {
  ...
  minervini: {
    label: "Ruptura",
    description: "Estructura de tendencia, momentum y proximidad a máximos con score de estructura de ruptura alto.",
  },
  ipo: {
...
export const DEFAULT_LEADERBOARD_SPECS = [
  ...
  { key: "global-stage2", title: "Top Etapa 2 Global", strategy: "stage2", scopeType: "global", scopeValue: "" },
...
function priceFreshness(row = {}, maxDays = DEFAULT_MAX_PRICE_FRESHNESS_DAYS) {
  ...
  return {
    days,
    ok: days <= limit,
    label: days <= 2 ? "fresco" : days <= limit ? "útil" : "viejo",
...
export function buildLeaderboard(rows = [], params = {}) {
  ...
  return {
    ...
    source: "Señales derivadas de los resultados de escaneo",
    input: {
...
// ─── Lectura anclada al escaneo nocturno estadounidense ───────────────────
//
// readScanRows coge las N filas más recientes de scan_results VENGAN DEL
// ESCANEO QUE VENGAN. Con maxRows=80, un escaneo interactivo o una corrida
// del cron por mercados desplazan al nocturno o se cuelan a medias: el
// 13 de agosto de 2026 eso metía 8328.HK/8329.HK/8326.HK en "Deterioro
// técnico" y 8321.HK en "Pullback to SMA50", con cierre del día 10 mientras
// el resto de la pantalla iba a cierre del 12 — otro mercado y otra fecha
// sin decirlo (docs/migracion-listas-2026-08-13.md §13.3).
//
// El lanzamiento es solo Estados Unidos, así que la fuente de las listas es
// UN escaneo concreto: el último nocturno. Se identifica por el prefijo de
// local_id que escribe scripts/scan-universe.mjs, definido en
// lib/scanLocalId.js — antes estaba escrito a mano aquí y en dos scripts más.
//
// Las corridas de PRUEBA quedan fuera por construcción: llevan el prefijo
// "test:" delante, así que no casan con este `like` y no pueden convertirse
// en la fuente de las Listas. Antes sí podían, y el 14 de agosto de 2026 una
// corrida --limit=300 lo hizo.
//
// CAVEAT heredado, no resuelto aquí: app/api/jobs/scan-refresh/route.js
// llama a la misma runMaterializedScan y, si alguien lo invoca a mano con
// ?markets=US, produciría un local_id con este mismo prefijo. El propio
// scan-universe.mjs ya lo documenta en su cabecera y decide no resolverlo
// con una segunda señal; se mantiene ese criterio.
const PUBLISHABLE_PARENT_STATUS = ["complete", "partial", "done"];

function parentStatusOf(scan = {}) {
  return cleanText(scan?.settings?.progress?.status || "");
}

export async function readNightlyUsScan({ timeoutMs = DEFAULT_SCAN_READ_TIMEOUT_MS } = {}) {
  const config = supabaseConfig();
  if (!config.configured) return { configured: false, ...disabledPayload(), scan: null };
  const scans = await supabaseRequest("scans", {
    query: [
      ...
      "select=id,local_id,created_at,settings",
  ...
  if (!scan) {
    return { configured: true, scan: null, reason: "no-nightly-scan" };
  }
  const status = parentStatusOf(scan);
  // Mismo contrato terminal que la RPC leaderboard_publishable_rows: un
  // escaneo que no terminó en un estado publicable no publica nada. Y no se
  // busca "el anterior que sí valga": si el nocturno de esta noche falló, lo
  // honesto es decirlo, no servir el de anteayer como si fuera de hoy.
  if (!PUBLISHABLE_PARENT_STATUS.includes(status)) {
    return { configured: true, scan: null, reason: "nightly-not-publishable", rejectedScan: { id: scan.id, localId: scan.local_id, createdAt: scan.created_at, status } };
  }
  return { configured: true, scan: { id: scan.id, localId: scan.local_id, createdAt: scan.created_at, status } };
}

export async function readNightlyUsScanRows({ maxRows = DEFAULT_SCAN_ROWS, timeoutMs = DEFAULT_SCAN_READ_TIMEOUT_MS } = {}) {
  const found = await readNightlyUsScan({ timeoutMs });
  if (!found.configured) return { configured: false, message: found.message, rows: [], nightly: { found: false, reason: "supabase-disabled" } };
  if (!found.scan) {
    return {
      configured: true,
      rows: [],
      ownerId: supabaseConfig().ownerId,
      nightly: { found: false, reason: found.reason, rejectedScan: found.rejectedScan || null },
    };
  }
  const limit = Math.min(Math.max(Number(maxRows || DEFAULT_SCAN_ROWS), 1), 10000);
  const rows = await supabaseRequestAll("scan_results", {
    query: [
      `scan_id=eq.${encodeURIComponent(found.scan.id)}`,
      "select=id,owner_id,scan_id,symbol,company_name,country,sector,industry,theme,rank_index,total_score,weinstein_score,minervini_score,risk_score,rs_rating,metrics,raw,created_at",
  ...
  return {
    ...
    rowsPublished: list.length,
    rowsExcluded: 0,
    // rows vacías con el escaneo presente no es lo mismo que no tener
    // escaneo, y quien pinte la pantalla necesita poder distinguirlo.
    nightly: { found: true, empty: list.length === 0, ...found.scan, rows: list.length },
  };
}
