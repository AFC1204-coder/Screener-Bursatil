...
// local_id que escribe scripts/scan-universe.mjs — "materialized:US:".
//
...
const NIGHTLY_US_LOCAL_ID_PREFIX = "materialized:US:";
...
export async function readNightlyUsScan({ timeoutMs = DEFAULT_SCAN_READ_TIMEOUT_MS } = {}) {
  ...
  const scans = await supabaseRequest("scans", {
    query: [
      ...
      `local_id=like.${encodeURIComponent(`${NIGHTLY_US_LOCAL_ID_PREFIX}*`)}`,
