import { disabledPayload, finiteOrNull, supabaseConfig, supabaseRequest, supabaseRequestAll, textOrNull } from "@/lib/supabaseServer";
...
export const LEADERBOARD_STRATEGIES = {
  ...
  nearPivot: {
    ...
    description: "Lideres fuertes en zona de pivot observable; no implica plan automatico.",
  },
  rs: {
    label: "RS",
    description: "Mayor fuerza relativa del universo disponible; el benchmark queda como senal secundaria y no sustituye al RS.",
  },
  rsQuality: {
    label: "RS Quality",
  ...
  weakness: {
    label: "Deterioro tecnico",
    description: "Debilidad observable para evitar largos o estudiar cortos sin mezclarla con listas de oportunidad larga.",
  },
  minervini: {
    label: "Minervini",
    description: "Trend template, momentum y proximidad a maximos con score Minervini alto.",
  },
  ipo: {
    label: "IPO / New Leaders",
    description: "Companias recientes con edad IPO verificable y fuerza derivada suficiente.",
  },
  extended: {
    label: "Extended but strong",
    description: "Lideres fuertes pero extendidos sobre SMA50; vigilancia, no entrada automatica.",
  },
  pullback: {
    label: "Pullback to SMA50",
    description: "Lideres de calidad cerca de SMA50 y por encima de SMA200 para vigilancia.",
  },
  liquidity: {
    label: "Liquidez",
...
function methodologyReliability(row = {}, maxPriceFreshnessDays = DEFAULT_MAX_PRICE_FRESHNESS_DAYS) {
  ...
  return {
    ...
    setupPlanValid: Boolean(verdict.actionable && verdict.tradePlanEligible),
    setupActionable: Boolean(verdict.actionable),
    setupWatch: Boolean(verdict.watch),
    setupStrict: Boolean(verdict.strict),
    ...
    setupDisplayPlanValid: display.actionable === true && display.tradePlanEligible === true && display.blocksPatternClaim !== true,
    setupDisplayActionable: display.actionable === true,
    setupDisplayObservable: display.observable === true,
    setupDisplayWatch: display.watch === true,
    setupDisplayStrict: display.strict === true,
    setupDisplayTradePlanEligible: display.tradePlanEligible === true,
    setupDisplayConfidenceKey: display.confidence?.key || verdict.dataConfidence?.key || "",
    setupDisplayConfidenceLabel: display.confidence?.label || verdict.dataConfidence?.label || "",
  };
}
...
function strategyPasses(row = {}, strategy = "momentum", options = {}) {
  ...
  if (strategy === "nearPivot") {
    const reliability = methodologyReliability(row, options.maxPriceFreshnessDays);
    if (reliability.blocksPatternClaim) return false;
    return total >= 55 && rs >= 55 && methodologyPivotWatchEligible({ ...row, ...reliability }, { min: -8, max: 3 });
  }
  if (strategy === "rs") return Number.isFinite(rsUniverseValue(row)) && rsUniverseValue(row) >= 60;
  if (strategy === "rsQuality") return (metric(row, "rsQualityScore") || 0) >= 55 && (rsUniverseValue(row) || 0) >= 55;
  ...
  if (strategy === "minervini") {
    if ((metric(row, "minerviniScore") || 0) < 60 || total < 50 || rs < 55) return false;
    if (Number.isFinite(perf3m) && perf3m < 0) return false;
    if (Number.isFinite(distance52w) && distance52w < -25) return false;
    return true;
  }
  if (strategy === "ipo") {
    const age = monthsSince(metricText(row, "ipoDate"));
    return Number.isFinite(age) && age >= 0 && age <= 60 && ((metric(row, "ipoScore") || 0) >= 45 || total >= 50 || rs >= 55);
  }
  if (strategy === "extended") return (metric(row, "extSma50") || 0) >= 15 && total >= 70 && (!Number.isFinite(distance52w) || distance52w >= -20);
  if (strategy === "pullback") {
    const extSma50 = metric(row, "extSma50");
    const price = metric(row, "price");
    const sma200 = metric(row, "sma200");
    return Number.isFinite(extSma50) && extSma50 >= -3 && extSma50 <= 8 && (!Number.isFinite(price) || !Number.isFinite(sma200) || price > sma200) && total >= 50;
  }
...
export async function readScanRows({ maxRows = DEFAULT_SCAN_ROWS, sinceDays = 45 } = {}) {
  ...
  const rows = await supabaseRequestAll("scan_results", {
    query: `owner_id=eq.${encodeURIComponent(config.ownerId)}&created_at=gte.${encodeURIComponent(since)}&select=*&order=created_at.desc`,
  }, {
    maxRows: limit,
  });
