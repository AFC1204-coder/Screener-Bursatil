...
// La lista "Top Etapa 2 Global" comprueba LA ETAPA, la misma que la columna.
//
// Antes tenía su propia definición —las medias diarias de 50/150/200, y si
// faltaban, un umbral sobre dos puntuaciones que no miran ninguna media— así
// que la lista podía incluir valores que la tabla no marcaba como etapa 2 y
// al revés. Era la séptima implementación de etapa del repo
// (docs/auditoria-etapas-2026-08-16.md, A.1, caso 7). Sin etapa no hay lista:
// no se sustituye por puntuaciones.
function isStage2(row = {}) {
  // Se lee también de `raw` porque las filas del nocturno que pasaron el
  // cribado guardan ahí sus métricas, y las que no, en `metrics`
  // (lib/scanLightProjection.js). `metric`/`metricText` sólo miran `metrics`.
  const state = row.weeklyStageState ?? row.metrics?.weeklyStageState ?? row.raw?.weeklyStageState;
  return cleanText(state) === "stage2";
}
...
//
// Quién es ese nocturno lo decide lib/nightlyUsScan.js, compartido con la
// pantalla principal (app/api/scans/route.js) y con la amplitud del universo
// (lib/marketBreadth.js): las tres tenían el mismo fallo y ahora preguntan al
// mismo sitio. Aquí solo se leen SUS filas.
export { readNightlyUsScan };
