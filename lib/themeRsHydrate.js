...
const THEME_RS_CHUNK_CONCURRENCY = 6;
...
async function readEngineThemeRsForSymbols(symbols = [], engineVersion = "", themeKey = "") {
  ...
  // Chunk al tope seguro PostgREST (3 filas/símbolo × 333 ≤ 1000). Con ~12 engines
  // en paralelo, chunks de 80 generaban ~49 HTTP; 333 baja a ~16 sin cambiar semántica.
  const rowsPerSymbolCap = 3;
  const chunkSize = Math.max(1, Math.floor(POSTGREST_MAX_ROWS / rowsPerSymbolCap));
  const chunks = [];
  for (let i = 0; i < cleanSymbols.length; i += chunkSize) chunks.push(cleanSymbols.slice(i, i + chunkSize));
  const concurrency = THEME_RS_CHUNK_CONCURRENCY;
  for (let i = 0; i < chunks.length; i += concurrency) {
    const batch = chunks.slice(i, i + concurrency);
    const pages = await Promise.all(batch.map((chunk) => supabaseRequest("rs_weekly_items", {
      query: [
...
export async function readThemeRsForSymbols(symbols = [], options = {}) {
  ...
  const engineReads = await Promise.all(
    [...byEngine.entries()].map(([engineVersion, { themeKey, symbols: engineSymbols }]) =>
      readEngineThemeRsForSymbols(engineSymbols, engineVersion, themeKey)),
  );
  for (const themed of engineReads) {
