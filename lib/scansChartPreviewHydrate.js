...
/** Lectura síncrona de miniaturas ya hidratadas en esta sesión (mesa → /review). */
export function peekCachedChartPreviews(scanId = "", symbols = []) {
  const map = cacheByScan.get(scanCacheKey(scanId));
  if (!map?.size) return {};
  const wanted = Array.isArray(symbols) && symbols.length
    ? symbols.map((symbol) => String(symbol || "").trim().toUpperCase()).filter(Boolean)
    : [...map.keys()];
  const out = {};
  for (const symbol of wanted) {
    const chartPreview = map.get(symbol);
    if (Array.isArray(chartPreview) && chartPreview.length >= 2) out[symbol] = chartPreview;
  }
  return out;
}
...
/** Alias usado por tests de Review (#31). */
