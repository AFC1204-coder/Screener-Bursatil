const SCORE_COMPONENTS = [
  ...
  { key: "rs", label: "RS", fields: ["rsGlobalPct", "rsRating"], weight: .16 },
  { key: "rs-quality", label: "RS calidad", field: "rsQualityScore", weight: .06 },
  ...
  { key: "ipo", label: "IPO", field: "ipoScore", weight: .02, optional: true },
];
...
export function buildScreenerScoreAudit(row = {}) {
  const components = SCORE_COMPONENTS.map((item) => {
  ...
  return {
    schemaVersion: 1,
    formula: "setup .17 + RS .16 + RS calidad .06 + demanda .10 + A/D .08 + growth .08 + EPS .08 + grupo .10 + rent/riesgo .08 + riesgo .05 + momentum .02 + IPO .02",
