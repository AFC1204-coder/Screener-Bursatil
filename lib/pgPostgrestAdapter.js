...
export function buildUpsertAppSettingNewerWinsSql(
  ownerId,
  settingType,
  settingKey,
  value,
  ...
  // Delega a public.upsert_app_setting_newer_wins (schema.sql): newer-wins por
  // updated_at; null p_value → set vacío; conflict pierde si existing es más nuevo
  // y aun así RETURNING devuelve la fila vigente (settingSyncSummary → skippedStale).
  if (value === null || value === undefined) {
    return { sql: null, values: null, empty: true };
  }
  const sql = `
    SELECT *
    FROM public.upsert_app_setting_newer_wins(
      $1::text,
      $2::text,
      $3::text,
      $4::jsonb,
  ...
  return {
    ...
    values: [
      ...
      settingType == null ? null : String(settingType),
      settingKey == null ? null : String(settingKey),
      jsonbValue,
      updatedAt == null || updatedAt === "" ? null : String(updatedAt),
    ],
    empty: false,
  };
}
...
export async function pgRpc(pool, functionName, payload = {}) {
  ...
  if (name === "upsert_app_setting_newer_wins") {
    const { sql, values, empty } = buildUpsertAppSettingNewerWinsSql(
      payload.p_owner_id,
      payload.p_setting_type,
      payload.p_setting_key,
      payload.p_value,
      payload.p_updated_at ?? null,
    );
    if (empty) return [];
    const result = await pool.query(sql, values);
    return result.rows;
