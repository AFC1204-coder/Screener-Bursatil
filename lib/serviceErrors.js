...
export const CLOUD_AUTH_FAILURE_PATTERN = /\bHTTP 401\b|\bHTTP 403\b|\bunauthorized\b|\bforbidden\b|\bno autorizado\b|token no autorizado/i;

export function isCloudAuthFailure(rawMessage) {
  const text = String(rawMessage == null ? "" : rawMessage).trim();
  return Boolean(text) && CLOUD_AUTH_FAILURE_PATTERN.test(text);
}
