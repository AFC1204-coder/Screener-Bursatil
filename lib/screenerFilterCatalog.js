...
export const SCREENER_FILTER_QUERY_KEYS = [
  ...
  "vcpMinDistanceToPivotPct",
  "vcpMaxDistanceToPivotPct",
  "vcpMaxVolumeDryUpRatio",
  "vcpWeeklyStageStructure",
  "minRiskRewardScore",
  "minReturnToVol3m",
...
export const STRING_FILTER_KEYS = new Set(["filterStrictness", "setupMode", "vcpWeeklyStageStructure"]);
...
export const QUALITY_DEFAULTS = {
  ...
  vcpRequireStage2: false,
  vcpMinContractionCount: 0,
  vcpMaxLastContractionDepthPct: 999,
  vcpMinDistanceToPivotPct: -999,
  vcpMaxDistanceToPivotPct: 999,
  vcpMaxVolumeDryUpRatio: 999,
  vcpWeeklyStageStructure: "",
};

export const SCREENER_FILTER_PRESETS = {
  // FILTER-SESSION-BACK-1: liquidez/tendencia algo más amplias en frío (US
  // primero se mantiene vía DEFAULT_MARKETS). Capas momentum/RS/proximidad
  // off en v4 neutralizan perfs/distancias aunque el preset las declare.
  balanced: { name: "Balanceado", desc: "Tendencia alcista amplia con sesgo profesional", v: { ...QUALITY_DEFAULTS, minMarketCap: 150000000, minPrice: 2, minAvgVolume: 100000, minAvgTurnover: 1000000, minWeinsteinScore: 40, minMinerviniScore: 30, maxDistance20dHigh: 25, maxDistance50dHigh: 35, maxDistance52w: 45, maxDistanceATH: 75, maxHighsSpreadPct: 30, minPerf3m: 0, minPerf6m: 5, minPerf12m: 8, maxExtensionSma50: 50, maxSymbols: SCREENER_ALL_SYMBOLS_LIMIT } },
  ...
  // Ficha IPO recientes (IPO-UX-A): lente cohort por edad; receta intl + requireRecentIpo; no setup ipoRecent.
  ipoDiscovery: { name: "IPO recientes", desc: "Cohort por edad de salida, sin puertas de tendencia", v: { ...QUALITY_DEFAULTS, filterStrictness: "discovery", setupMode: "any", requireStage2: false, requireSma200Up: false, requirePriceAboveSma50: false, requireRecentIpo: true, maxIpoAgeMonths: 24, minDataCoverageScore: 20, minTechnicalCoverageScore: 30, minFundamentalCoverageScore: 0, minMarketCap: 50000000, minPrice: 1, minAvgVolume: 25000, minAvgTurnover: 100000, minLatestVolume: 0, minLatestTurnover: 0, minRelativeVolume: 0, minVolumeSurgePct: -999, minUpDownVolRatio: 0, minVolumeEffectScore: 0, maxDistance20dHigh: 30, maxDistance50dHigh: 40, maxDistance52w: 50, maxDistanceATH: 80, maxHighsSpreadPct: 40, minPerf3m: -100, minPerf6m: -100, minPerf12m: -100, maxExtensionSma50: 55, maxDailyMove20dPct: 999, maxDailyRange20dPct: 999, maxRange63dPct: 999, maxVolatility63d: 999, maxDrawdown63d: 999, minWeinsteinScore: 0, minMinerviniScore: 0, minMomentumScore: 0, minRiskScore: 0, minVolumeScore: 0, minLiquidityScore: 0, minRsRating: 0, minRsBenchmarkRating: 0, minRsCountryPct: 0, minRsSectorPct: 0, minRsQualityScore: 0, minSectorScore: 0, minTotalScore: 0, minRiskRewardScore: 0, minReturnToVol3m: -999, minReturnToDrawdown3m: -999, maxPriceFreshnessDays: 14, maxSymbols: SCREENER_ALL_SYMBOLS_LIMIT } },
...
// Núcleo (mesa) vs adicionales (opt-in). Mismo contrato que lib/screenerConfig.js.
// v4 (FILTER-SESSION-BACK-1): núcleo frío más corto — liquidez + tendencia +
// score + cobertura. Momentum/RS/proximidad/volatilidad pasan a opt-in (no
// reabre pattern/vcp/volumen+/RR/SI/IPO, que siguen en OPTIONAL).
export const CORE_LAYER_KEYS = [
  "liquidity",
  "trend",
  "score",
  "coverage",
];
export const OPTIONAL_LAYER_KEYS = [
  "momentum",
  "relativeStrength",
  "proximity",
  "volatility",
...
export const FILTER_FAMILIES = {
  ...
  trend: {
    ...
    fields: [
      ...
      { key: "minWeeksAboveSma30w", label: "Semanas sobre MA 30s min", unit: "", step: 1, hint: "Semanas consecutivas con cierre semanal sobre la media de 30 semanas. Si el precio está bajo la media o falta el dato, no pasa con umbral >0." },
    ],
    actions: [
      ...
      { label: "≥8 sem sobre MA 30s", detail: "Exige sostén acumulado sobre la media lenta.", settings: { minWeeksAboveSma30w: 8 }, fieldRules: { minWeeksAboveSma30w: true } },
    ],
    coverageMetric: null,
    modes: null,
  },
  ...
  relativeStrength: {
    ...
    fields: [
      ...
      { key: "minThemeRsRating", label: "RS tema min", unit: "", step: 5 },
    ...
    actions: [
      ...
      { label: "RS abierto", detail: "No corta por ranking relativo.", settings: { minRsRating: 0, minRsBenchmarkRating: 0, minRsCountryPct: 0, minThemeRsRating: 0, minRsQualityScore: 0, minSectorScore: 0 } },
    ],
    coverageMetric: null,
    modes: null,
  },
  ...
  vcp: {
    ...
    label: "VCP",
    title: "VCP",
    detail: "compresiones Minervini",
    intro: "Contracciones decrecientes, pivot y candidato del motor unificado. Sin puntuación de patrón ni hunt paralelo.",
    settingKeys: ["requireVcpCandidate", "vcpRequireStage2"],
    fields: [
      { key: "vcpMinContractionCount", label: "Contracciones min", unit: "", step: 1, hint: "Número mínimo de pullbacks medidos en la base reciente." },
      { key: "vcpMaxLastContractionDepthPct", label: "Última contracción max", unit: "%", step: 1 },
      { key: "vcpMinDistanceToPivotPct", label: "Distancia pivot min", unit: "%", step: 0.5, hint: "Negativo = por debajo del pivot estimado." },
      { key: "vcpMaxDistanceToPivotPct", label: "Distancia pivot max", unit: "%", step: 0.5 },
      { key: "vcpMaxVolumeDryUpRatio", label: "Volumen seco max", unit: "x", step: 0.05, hint: "Volumen medio 10d / 50d en la pata final." },
      { key: "vcpWeeklyStageStructure", label: "Estructura semanal", unit: "", step: 1, hint: "Opcional: E2_ma_only (pre-fuga) o E2_structural (con fuga). Vacío = cualquiera." },
    ],
    actions: [
      { label: "VCP Minervini", detail: "2C+, tight, etapa 2 y candidato motor ON.", filterLayers: { vcp: true }, settings: { requireVcpCandidate: true, vcpRequireStage2: true, vcpMinContractionCount: 2, vcpMaxLastContractionDepthPct: 12, vcpMinDistanceToPivotPct: -8, vcpMaxDistanceToPivotPct: 3, vcpMaxVolumeDryUpRatio: 1 } },
      { label: "VCP en formación", detail: "2C+ sin exigir candidato del motor (bases en curso).", filterLayers: { vcp: true }, settings: { requireVcpCandidate: false, vcpRequireStage2: false, vcpMinContractionCount: 2, vcpMaxLastContractionDepthPct: 12, vcpMinDistanceToPivotPct: -8, vcpMaxDistanceToPivotPct: 3, vcpMaxVolumeDryUpRatio: 1.2 } },
      { label: "Apagado", detail: "Desactiva esta familia opcional.", filterLayers: { vcp: false }, settings: { requireVcpCandidate: false, vcpRequireStage2: false, vcpMinContractionCount: 0, vcpMaxLastContractionDepthPct: 999, vcpMinDistanceToPivotPct: -999, vcpMaxDistanceToPivotPct: 999, vcpMaxVolumeDryUpRatio: 999, vcpWeeklyStageStructure: "" } },
    ],
    coverageMetric: null,
    modes: null,
  },
  ...
  ipo: {
    ...
    label: "IPO",
    title: "IPO",
    detail: "edad de salida a bolsa",
    // Fallback estático; el modal genera copy desde maxIpoAgeMonths (filterFamilyIpoIntro).
    intro: "Cotizadas con salida reciente (edad máxima configurable).",
    intensityPilot: true,
    ...
    actions: [
      { label: "IPO real", detail: "Solo cotizadas con salida reciente confirmada.", settings: { setupMode: "ipoRecent", requireRecentIpo: true, maxIpoAgeMonths: 60, minDataCoverageScore: 35, minTechnicalCoverageScore: 45, minPerf3m: 10, minPerf6m: 0, minPerf12m: -100 } },
...
/** Copy honesto de la familia IPO desde la regla de edad (IPO-UX-C). */
export function filterFamilyIpoIntro(settings = {}) {
  const months = Number(settings?.maxIpoAgeMonths);
  if (Number.isFinite(months) && months > 0 && months < 999) {
    return `Cotizadas con salida ≤ ${Math.round(months)} m.`;
  }
  return FILTER_FAMILIES.ipo.intro;
}
...
//
// v2 (2026-08-14): todas las capas encendidas de fábrica (alineación pantalla/cron).
// v3 (2026-09-01, UX-FILTERS-8): familias opcionales off en frío; sólo núcleo on.
//   Los presets balanced/nearPivot/leader no activan pattern sin acción explícita.
//   El cron sigue aplicando el preset crudo; la pantalla neutraliza capas apagadas.
// v4 (2026-09-25, FILTER-SESSION-BACK-1): núcleo frío = liquidity/trend/score/coverage.
//   momentum/RS/proximity/volatility pasan a opt-in. Sesiones v3 se respetan
//   (MIN_RESTORABLE=3); el frío nuevo usa DEFAULT_FILTER_LAYERS v4.
export const FILTER_LAYERS_CONTRACT_VERSION = 4;
/** Primera versión cuyo estado de capas guardado se respeta (no se descarta). */
export const FILTER_LAYERS_MIN_RESTORABLE_VERSION = 3;

// Núcleo on, opcionales off. Encender pattern/volumen+/IPO/etc. es decisión del usuario.
export const DEFAULT_FILTER_LAYERS = Object.fromEntries(
  FILTER_FAMILY_ORDER.map((key) => [key, CORE_LAYER_KEYS.includes(key)]),
);
// Alias de «todas encendidas» para scripts de contrato y pruebas de neutralización.
export const ALL_FILTER_LAYERS = Object.fromEntries(FILTER_FAMILY_ORDER.map((key) => [key, true]));
// Sólo presets cuyo propósito exige una familia opcional concreta (no pattern en hunt E2).
export const PRESET_LAYER_OVERRIDES = {
  ipo: { ipo: true },
  ipoDiscovery: { ipo: true },
};

export function filterLayersForPreset(key = "balanced") {
  const presetKey = SCREENER_FILTER_PRESETS[key] ? key : "balanced";
  const setupMode = SCREENER_FILTER_PRESETS[presetKey]?.v?.setupMode || "leader";
  return {
    ...DEFAULT_FILTER_LAYERS,
    ...(PRESET_LAYER_OVERRIDES[presetKey] || {}),
    // Capas que el setupMode del preset exige encendidas (p. ej. nearPivot →
    // proximity+momentum). Sin esto, effectiveSettingsFromLayers degrada el
    // modo a «any» cuando esas familias son opt-in (contrato v4).
    ...setupModeLayerRequirements(setupMode),
  };
}
...
export const SETTING_LAYER_DEPENDENCIES = {
  ...
  requireVcpCandidate: { layer: "vcp", label: "VCP" },
  vcpRequireStage2: { layer: "vcp", label: "VCP" },
};
...
export const FIELD_RULES = {
  ...
  vcpMinContractionCount: { op: "min", metric: "contractionCount", label: "contracciones VCP" },
  vcpMaxLastContractionDepthPct: { op: "max", metric: "lastContractionDepthPct", label: "última contracción VCP" },
  vcpMinDistanceToPivotPct: { op: "min", metric: "distanceToPivotPct", label: "distancia pivot VCP" },
  vcpMaxDistanceToPivotPct: { op: "max", metric: "distanceToPivotPct", label: "distancia pivot VCP" },
  vcpMaxVolumeDryUpRatio: { op: "max", metric: "volumeDryUpRatio", label: "volumen seco VCP" },
