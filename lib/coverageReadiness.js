...
export function coverageReadinessForMarket({ market = "", byMarket = {}, bySource = {}, target = null, scan = null, provider = null } = {}) {
  ...
  if (policy?.officialSources?.length && officialCount < Number(policy.minOfficialCount || 1)) {
    return {
      ...
      detail: withProviderDetail(`${policy.sourceClaim || "Fuente oficial"} no alcanzo el minimo esperado (${officialCount}/${policy.minOfficialCount || 1}); no presentar este mercado como cobertura oficial.`, provider),
      operational: false,
      officialCount,
  ...
  if (policy?.mode === "curated_core" || policy?.mode === "deferred_curated") {
    return {
      ...
      detail: withProviderDetail(policy.mode === "deferred_curated"
        ...
        : "Cobertura util por liquidez, pero no equivale a universo completo de exchange.", provider),
      operational: false,
      officialCount,
      curatedCount: curated,
      coveragePct,
  ...
  if (hasScan && activationPct < 20) {
    return {
      ...
      label: "Inventario sin materializar",
      tone: "warn",
      sourceClaim: policy?.sourceClaim || "Inventario disponible",
      detail: withProviderDetail("Hay inventario, pero pocos simbolos estan escaneados/frescos; no usar como cobertura operativa plena.", provider),
      operational: false,
      officialCount,
      curatedCount: curated,
      coveragePct,
