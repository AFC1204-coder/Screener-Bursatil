...
export function setupTagsForRow(row = {}) {
  ...
  if (verdict.key === "actionable_vcp") tags.push({ key: "setup_plan_valid", label: "VCP plan válido" });
...
export function methodologyEvents(row = {}, previous = null) {
  ...
  if (!previous) {
    ...
    if (verdict.key === "actionable_vcp") events.push(event("setup_plan_valid", "positive", "VCP plan válido observado", verdict.reason, "setupVerdict", verdict.key));
    ...
    else if (verdict.watch) events.push(event("setup_watch", "neutral", verdict.label || "Setup en vigilancia", verdict.reason || "Estructura observable sin plan válido.", "setupVerdict", verdict.key));
  ...
  if (verdict.key !== prevVerdict?.key && verdict.key === "actionable_vcp") events.push(event("setup_plan_valid", "positive", "Pasa a VCP plan válido", verdict.reason, "setupVerdict", verdict.key, prevVerdict?.key));
