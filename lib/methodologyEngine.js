...
export function setupTagsForRow(row = {}) {
  ...
  if (row.vcpCandidate || row.snapshot?.vcpCandidate) tags.push({ key: "vcp_candidate", label: "Contraccion VCP" });
...
export function methodologyEvents(row = {}, previous = null) {
  ...
  if (!previous) {
    ...
    if (vcpCandidate) events.push(event("vcp_candidate", "neutral", "VCP candidato", "Base con contracciones decrecientes y volumen medio por debajo del tramo previo.", "vcpCandidate", true));
  ...
  if (vcpCandidate && !prevVcpCandidate) events.push(event("vcp_candidate", "neutral", "VCP candidato", "Aparece una base con contracciones decrecientes y volumen seco.", "vcpCandidate", true, false));
