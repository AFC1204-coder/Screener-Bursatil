...
// La etapa la calcula lib/weeklyStage.js sobre la media de 30 semanas, y solo
// él. Aquí se LEE.
//
// Antes esta función tenía su propio árbol de decisión sobre las medias
// diarias de 50/150/200 para las filas sin etapa semanal, y emitía las mismas
// palabras ("stage2", "base"…) con un criterio distinto: la misma fila podía
// leerse "Etapa 2" en la ficha y otra cosa en la tabla. Era una de las siete
// implementaciones que encontró docs/auditoria-etapas-2026-08-16.md (A.1,
// caso 2). Una fila sin etapa semanal ahora declara la ausencia en vez de
// inventar una fase del ciclo con datos diarios (principio 3 de
// docs/principios-producto.md).
export function stageStateForRow(row = {}) {
  const key = row.weeklyStageState || row.weeklyStage?.state || "";
  if (key) {
    const label = row.weeklyStageLabel || row.weeklyStage?.label || "Etapa semanal";
    const detail = row.weeklyStage?.detail || `Clasificación semanal sobre la media de ${row.weeklySlowWeeks || row.weeklyStage?.slowWeeks || 30} semanas.`;
    return { key, label, detail, confirmation: row.weeklyStageConfirmation || row.weeklyStage?.confirmation || "" };
  }
  return {
    key: "insufficient_history",
    label: userFacingSearchError("Sin etapa"),
    detail: "No hay barras semanales suficientes para clasificar la etapa de este valor.",
    confirmation: "",
  };
}
...
export function methodologyEvents(row = {}, previous = null) {
  ...
  if (!previous) {
    if (currentMethodology.stageState.key === "stage2") events.push(event("stage2_observed", "positive", "Etapa 2 observada", currentMethodology.stageState.detail));
    ...
    if (rs >= 80) events.push(event("rs_leader", "positive", "RS líder", "Percentil/fuerza relativa alta dentro del universo analizado.", "rs", rs));
