...
export function setupTagsForRow(row = {}) {
  ...
  if (gte(distance20d, -5) && gte(distance52w, -15) && lte(extSma50, 15) && lte(highsSpreadPct, 10)) tags.push({ key: "near_pivot", label: "Zona de pivot" });
  if (
    Number.isFinite(distanceToPivotPct)
    && between(distanceToPivotPct, -5, 3)
    && (
      verdict.observable
      || row.consolidationCandidate === true
        && !["not_vcp", "failed_breakout", "trend_no_base"].includes(structure.key)
        && gte(patternQualityScore, 55)
    )
  ) tags.push({ key: "pivot_zone", label: "Pivot cercano" });
  if (verdict.key === "actionable_vcp") tags.push({ key: "setup_plan_valid", label: "VCP plan valido" });
  else if (verdict.strict) tags.push({ key: "vcp_strict", label: "VCP estricto" });
  else if (verdict.watch) tags.push({ key: "setup_watch", label: verdict.label || "Vigilancia" });
  if (verdict.observable && Number.isFinite(patternQualityScore) && patternQualityScore >= 65) tags.push({ key: "pattern_quality", label: "Estructura medible" });
...
export function methodologyEvents(row = {}, previous = null) {
  ...
  if (!previous) {
    ...
    if (verdict.key === "actionable_vcp") events.push(event("setup_plan_valid", "positive", "VCP plan valido observado", verdict.reason, "setupVerdict", verdict.key));
    else if (verdict.strict) events.push(event("vcp_strict", "neutral", "VCP estricto observado", verdict.reason, "setupVerdict", verdict.key));
    else if (verdict.watch) events.push(event("setup_watch", "neutral", verdict.label || "Setup en vigilancia", verdict.reason || "Estructura observable sin plan valido.", "setupVerdict", verdict.key));
  ...
  if (verdict.key !== prevVerdict?.key && verdict.key === "actionable_vcp") events.push(event("setup_plan_valid", "positive", "Pasa a VCP plan valido", verdict.reason, "setupVerdict", verdict.key, prevVerdict?.key));
  else if (verdict.key !== prevVerdict?.key && verdict.strict) events.push(event("vcp_strict", "neutral", "Pasa a VCP estricto", verdict.reason, "setupVerdict", verdict.key, prevVerdict?.key));
  else if (verdict.key !== prevVerdict?.key && verdict.watch) events.push(event("setup_watch", "neutral", verdict.label || "Pasa a vigilancia", verdict.reason, "setupVerdict", verdict.key, prevVerdict?.key));
