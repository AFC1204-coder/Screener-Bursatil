...
export function createViewportLifecycle({
  ...
  function deriveFromLogicalRange(logicalRange) {
    ...
    if (nextView.isManual && timeRange) {
      state.manualView = {
        from: timeRange.from,
        to: timeRange.to,
        logical: { from: Number(logicalRange.from), to: Number(logicalRange.to) },
      };
    } else if (hasLeftWhitespaceLogicalRange(logicalRange)) {
      // Pan con whitespace: el rango temporal no captura el offset lógico.
      state.manualView = {
        from: timeRange?.from,
        to: timeRange?.to,
        logical: { from: Number(logicalRange.from), to: Number(logicalRange.to) },
    ...
    } else if (!nextView.isManual && !hasLeftWhitespaceLogicalRange(logicalRange)) {
      state.manualView = null;
    }
    schedulePublish();
  }
  ...
  function resolveManualLogicalRange(manualView) {
    const stored = manualView?.logical;
    const from = Number(stored?.from);
    const to = Number(stored?.to);
    const rows = getRowTimes().length;
    if (Number.isFinite(from) && Number.isFinite(to) && to > from && rows >= 2) {
      if (hasLeftWhitespaceLogicalRange(stored)) {
        const maxTo = rows - 0.5;
        return { from, to: Math.min(maxTo, to) };
      }
    ...
    if (Number.isFinite(Number(manualView?.from)) && Number.isFinite(Number(manualView?.to))) {
      return timeWindowLogicalRange({
        rowTimes: getRowTimes(),
        timeRange: manualView,
        minSpan: 8,
      });
    }
    return null;
  ...
  function applyContractWindow() {
    ...
    if (!timeScale) return;
    if (state.manualView) {
      const logical = resolveManualLogicalRange(state.manualView);
      if (logical) {
