...
const SHADOW_COUNT_TIMEOUT_MS = Number(process.env.SHADOW_COUNT_TIMEOUT_MS || 1500);
const SHADOW_READ_TIMEOUT_MS = Number(process.env.SHADOW_READ_TIMEOUT_MS || 2000);
...
export async function readCachedSymbolResolutions({ market = "", provider = SHADOW_RESOLUTION_PROVIDER, isins = [], statuses = ["resolved", "priced"], timeoutMs = SHADOW_READ_TIMEOUT_MS } = {}) {
  const config = supabaseConfig();
...
export async function readShadowInstrumentsForResolution({ market = "", provider = SHADOW_REFERENCE_PROVIDER, status = "reference", limit = 10, timeoutMs = SHADOW_READ_TIMEOUT_MS } = {}) {
  const config = supabaseConfig();
  if (!config.configured) return { ...supabaseDisabled(), rows: [] };
  const clean = cleanMarket(market);
...
export async function readSymbolResolutionsForPricing({ market = "", provider = SHADOW_RESOLUTION_PROVIDER, status = null, statuses = null, limit = 10 } = {}) {
  ...
  if (!clean) return { status: "empty", configured: true, rows: [] };
  // Resolve the effective status filter. Prefer the explicit `statuses` array
  // when provided; otherwise fall back to a single `status`; otherwise default
  // to ["resolved", "priced"] so existing callers (which historically passed
  // status="resolved") continue to work but the cron paths that pass
  // statuses=["resolved", "priced", "price-unavailable"] get the full pool.
  const requested = Array.isArray(statuses) && statuses.length
    ? statuses
    : (status ? [status] : ["resolved", "priced"]);
  const statusValues = Array.from(new Set(requested.map(cleanText).filter(Boolean)));
  if (!statusValues.length) return { status: "empty", configured: true, rows: [] };
...
export async function readShadowReferenceCounts({ markets = [], provider = SHADOW_REFERENCE_PROVIDER, maxAgeHours = 0 } = {}) {
  ...
  const byMarket = {};
  try {
    const counts = await Promise.all((markets || []).map(async (rawMarket) => {
      ...
      return [market, await supabaseCount("shadow_instruments", { query, timeoutMs: SHADOW_COUNT_TIMEOUT_MS })];
    }));
    for (const item of counts) {
      if (item) byMarket[item[0]] = item[1];
    }
    return {
      status: "supabase",
      configured: true,
...
export async function readSymbolResolutionCounts({ markets = [], provider = SHADOW_RESOLUTION_PROVIDER, maxAgeHours = 0 } = {}) {
  ...
  try {
    const counts = await Promise.all((markets || []).map(async (rawMarket) => {
      ...
      return [market, await supabaseCount("symbol_resolutions", { query, timeoutMs: SHADOW_COUNT_TIMEOUT_MS })];
    }));
    for (const item of counts) {
      if (item) byMarket[item[0]] = item[1];
    }
