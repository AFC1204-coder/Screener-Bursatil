...
/**
 * RS canónico del objeto `relativeStrength` del brief de ficha.
 * El pin (`rating`) es el corte semanal del ranking; la cola de
 * `globalRsSeries` alimenta solo el overlay histórico del gráfico.
 */
export function canonicalBriefRs(rs = {}) {
  const seriesTail = Array.isArray(rs.globalRsSeries) ? rs.globalRsSeries.at(-1) : null;
  const value = [rs.rating, seriesTail?.rsRating].find(Number.isFinite);
  return value ?? null;
}
