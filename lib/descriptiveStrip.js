...
import { canonicalRsEngineVersion } from "@/lib/rsEngines";
...
// ── RS «desde N» en la tarjeta de identidad ──────────────────────────────
// Tras RS-CHART-HISTORY-1 el FR (pin) y la serie del overlay pueden ser de
// motores distintos. El delta solo tiene sentido sobre la misma metodología
// que el rating mostrado; si la serie visible es de otro engine y no hay al
// menos dos puntos del motor pin, no se afirma «desde N».
export function rsWeeklyDeltaForIdentityCard(rs = {}, rsValue = null) {
  const fullSeries = Array.isArray(rs.globalRsSeries) ? rs.globalRsSeries : [];
  if (rsValue === null || !fullSeries.length) {
    return { from: null, fromDate: "", weeksBack: 13 };
  }

  const seriesEngine = fullSeries[0]?.engineVersion || "";
  const hasPinRating = Number.isFinite(rs.rating);
  const ratingEngine = hasPinRating ? canonicalRsEngineVersion() : seriesEngine;

  let deltaSeries = fullSeries;
  if (ratingEngine && seriesEngine && ratingEngine !== seriesEngine) {
    deltaSeries = fullSeries.filter((point) => point.engineVersion === ratingEngine);
    if (deltaSeries.length < 2) {
      return { from: null, fromDate: "", weeksBack: 13 };
    }
  } else if (fullSeries.length < 2) {
    return { from: null, fromDate: "", weeksBack: 13 };
  }

  const delta = rsWeeklyDelta(deltaSeries);
  return { from: delta.from, fromDate: delta.fromDate, weeksBack: delta.weeksBack };
}
...
export const DESCRIPTIVE_ABSENCE = {
  ...
  rsCountry: "Sin RS de país: el ranking semanal de país aún no está disponible para este mercado o este valor no entra en el universo curado de su mercado.",
  ...
  supportInsufficientHistory: "Sin lectura de medias: el histórico semanal no alcanza para la media de 30 semanas.",
  accelInsufficientHistory: "Sin lectura de aceleración: histórico inferior a 6 meses.",
  accelDiscontinuous: "Sin lectura de aceleración: salto sin ajustar en la serie de precios.",
  volumeCoverage: "Sin reparto de volumen: cobertura de volumen insuficiente en 50 sesiones.",
  healthStageMissing: "Sin salud de etapa: el histórico semanal no alcanza para clasificar la etapa.",
  healthNonTrendingStage: "Sin salud de etapa: la etapa actual no es una tendencia en curso; el índice mide etapas 2 y 4.",
  healthAccelHistory: "Sin salud de etapa: histórico inferior a 6 meses para la lectura de aceleración.",
  healthDiscontinuous: "Sin salud de etapa: salto sin ajustar en la serie de precios.",
  healthVolumeCoverage: "Sin salud de etapa: cobertura de volumen insuficiente en 50 sesiones.",
};
