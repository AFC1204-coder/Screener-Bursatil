...
export function inferTradingViewSymbol(symbol = "", exchange = "") {
  ...
  for (const [suffix, tvExchange] of TV_SUFFIXES) {
    if (s.endsWith(suffix)) return `${tvExchange}:${stripYahooSuffix(s, suffix)}`;
  }
...
export function externalLinks(symbol = "", exchange = "") {
  ...
  return {
    ...
    googleFinance: `https://www.google.com/finance/quote/${encodeURIComponent(s.replace(".", "/"))}`,
  };
}

export function benchmarkForFavorite(favorite = {}) {
  const text = `${favorite.snapshot?.theme || ""} ${favorite.theme || ""} ${favorite.sector || ""}`.toLowerCase();
  if (/software|semis|fot|ia|ai|cloud|cyber|chip|tech/.test(text)) return "QQQ";
  if (favorite.country && favorite.country !== "US") return "ACWI";
  return "SPY";
}
