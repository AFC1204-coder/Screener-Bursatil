...
function normalizedLists(lists = []) {
  return (Array.isArray(lists) ? lists : []).map((list) => {
    ...
    return {
      ...
      count: Number(list.count ?? rows.length) || 0,
      rows,
    };
  });
}
...
function noteForAudit(audit = {}) {
  ...
  if (audit.listHealth.thinLists.length || audit.groupHealth.thinGroups.length) return "Cobertura util, con algunas listas o grupos de muestra pequena.";
...
export function buildCoverageAudit({
  ...
  const groupSummaries = groupRows.map((group) => {
    const count = Number(group.count ?? group.items?.length ?? 0) || 0;
  ...
  const audit = {
    state,
    label: state === "empty" ? "Sin auditoria" : state === "warn" ? "Revisar sesgo" : state === "watch" ? "Cobertura parcial" : "Cobertura OK",
...
export function auditIssueLabels(audit = {}, limit = 4) {
  const labels = [
    ...
    ...(audit.listHealth?.emptyLists || []).map((item) => `${item.title} vacia`),
  ];
