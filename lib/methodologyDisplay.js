...
const EMPTY_CHART_PATTERN_SHORT_LABEL = "Sin validar";
const EMPTY_CHART_PATTERN_EVIDENCE = "Estructura sin dato";

export function shouldShowChartPatternBadge(display = null) {
  if (!display || typeof display !== "object") return false;
  const shortLabel = cleanText(display.shortLabel);
  const evidence = cleanText(display.evidence);
  return !(shortLabel === EMPTY_CHART_PATTERN_SHORT_LABEL && evidence === EMPTY_CHART_PATTERN_EVIDENCE);
}

export function chartPatternBadgeForRow(row = null) {
  if (row == null) return null;
  const display = methodologyDisplayForRow(row);
  return shouldShowChartPatternBadge(display) ? display : null;
}
