...
function buildResearchRow(symbol, chart, profile = {}, requireLongHistoryOrOptions = false, benchmarks = {}) {
  ...
  const b = chart.bars || [];
  if (b.length < (requireLongHistory ? 180 : 20)) throw new Error("Histórico insuficiente");
  ...
  if (row.ipoDate) {
    const anchor = ipoAnchorFromBars(calcBars, row.ipoDate);
    if (anchor) Object.assign(row, anchor);
  }
