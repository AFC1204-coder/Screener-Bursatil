...
const FMP_API_KEY = envValue("FMP_API_KEY");
...
// Fallback de fecha de salida a bolsa: UNA petición a /profile, no las nueve
// de fetchFmpCompanyData. Existe aparte precisamente para poder usarla como
// red de seguridad por símbolo dentro del escaneo sin pagar el perfil entero.
// Devuelve `{configured:false}` si no hay clave — el llamador decide, no se
// inventa una ausencia de dato cuando lo que falta es la credencial.
export async function fetchFmpIpoDate(symbol) {
  if (!FMP_API_KEY) return { configured: false, ipoDate: "" };
  const clean = canonicalSymbol(symbol);
  const data = await fmpJson(`/profile/${encodeURIComponent(clean)}`);
  const profile = Array.isArray(data) ? data[0] || {} : {};
  return { configured: true, ipoDate: firstText(profile.ipoDate) };
}
