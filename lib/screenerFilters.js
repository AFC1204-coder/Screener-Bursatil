import { dailyLongBiasIssue, isConfirmedStage2, pulsoRejectDetail, stage2RejectDetail } from "@/lib/trendStructure";
...
export function screenerFiltersFromParams(params = {}) {
  ...
  const base = preset ? { ...WEB_FILTER_PRESETS[preset] } : {};
  const explicit = [];
  ...
  for (const key of SCREENER_FILTER_QUERY_KEYS) {
    ...
    explicitValues[key] = BOOLEAN_KEYS.has(key)
      ? boolValue(raw)
      : STRING_KEYS.has(key)
  ...
  // Coherencia preset ↔ modo de setup.
  //
  // Pedir un preset POR NOMBRE y a la vez un modo de otro preset producía un
  // screener etiquetado con el primero y filtrando con la semántica del
  // segundo. El caso concreto: `?filterPreset=balanced&setupMode=weakness`
  // dejaba activo el minWeaknessScore=50 que "Balanceado" hereda de
  // QUALITY_DEFAULTS y que el motor sólo aplica en modo weakness, de modo que
  // el preset "Balanceado" devolvía únicamente valores deteriorados. Ver
  // docs/auditoria-filtros-2026-08-13.md, sección B.3.
  //
  // Se resuelve como ya lo resolvía la pantalla (applySetupMode en
  // app/page.jsx): un modo distinto al del preset trae consigo los ajustes de
  // ese modo. Sólo aplica cuando hay preset con nombre — sin él no hay etiqueta
  // que contradecir, y los llamantes que componen params a mano
  // (lib/leaderboards.js con strategy=weakness) siguen igual.
  const requestedMode = cleanText(explicitValues.setupMode || "");
  const coherence = preset && requestedMode && requestedMode !== base.setupMode && SETUP_MODE_DEFAULTS[requestedMode]
    ? setupModeDefaults(requestedMode)
    : {};
  const values = { ...base, ...coherence, ...explicitValues };
...
function neutralForField(field, direction = "min") {
  ...
  if (["minVolumeSurgePct", "minReturnToVol3m", "minReturnToDrawdown3m", "vcpMinDistanceToPivotPct"].includes(field)) return -999;
  return 0;
}
...
const VCP_FILTER_FIELDS = new Set([
...
const VCP_VOLUME_FILTER_FIELDS = new Set(["vcpMaxVolumeDryUpRatio"]);
...
function activeVcpFilterKeys(set = {}) {
  const active = [];
  if (set.requireVcpCandidate === true) active.push("requireVcpCandidate");
  if (set.vcpRequireStage2 === true) active.push("vcpRequireStage2");
  const structure = cleanText(set.vcpWeeklyStageStructure);
  if (structure && structure !== "any") active.push("vcpWeeklyStageStructure");
  for (const field of VCP_FILTER_FIELDS) {
    const rule = FIELD_RULES[field];
    if (!rule) continue;
    const direction = rule.op === "max" ? "max" : "min";
    if (thresholdActive(set[field], neutralForField(field, direction), direction)) active.push(field);
...
function vcpValidityGate(row = {}, set = {}) {
  const activeKeys = activeVcpFilterKeys(set);
  if (!activeKeys.length) return null;
  ...
  const needsVolumeEvidence = activeKeys.some((key) => VCP_VOLUME_FILTER_FIELDS.has(key));
  if (needsVolumeEvidence && (row.patternVolumeEligible === false || dataStatus === "partial_volume" || dataStatus === "missing_volume")) {
    return {
      field: "patternDataStatus",
      label: "volumen VCP",
...
function compactNumber(value) {
  ...
  if (abs >= 10_000) return `${Math.round(n).toLocaleString("es-ES")}`;
...
function minWeeksAboveSma30wReject(row = {}, threshold) {
  const minWeeks = finite(threshold);
  if (!Number.isFinite(minWeeks) || minWeeks <= 0) return null;
  const weeks = metric(row, "weeksAboveSma30w");
  if (!Number.isFinite(weeks) || row.weeksAboveSma30wAbove == null) {
    return reject("minWeeksAboveSma30w", "semanas sobre MA 30s sin dato");
  }
  if (row.weeksAboveSma30wAbove !== true) {
    return reject("minWeeksAboveSma30w", "bajo la media de 30 semanas");
  }
  if (weeks < minWeeks) {
    return reject("minWeeksAboveSma30w", `semanas sobre MA 30s ${weeks} < ${minWeeks}`);
  }
  return null;
}

function addMinWeeksAboveSma30wRule(bucket, row = {}, set = {}) {
  const minWeeksAboveSma30w = finite(set.minWeeksAboveSma30w);
  if (!Number.isFinite(minWeeksAboveSma30w) || minWeeksAboveSma30w <= 0) return;
  const weeks = metric(row, "weeksAboveSma30w");
  if (!Number.isFinite(weeks) || row.weeksAboveSma30wAbove == null) {
    addRule(bucket, {
      field: "minWeeksAboveSma30w",
      label: "semanas sobre MA 30s",
      status: "fail",
      detail: "semanas sobre MA 30s sin dato",
    });
    return;
  }
  if (row.weeksAboveSma30wAbove !== true) {
    addRule(bucket, {
      ...
      label: "semanas sobre MA 30s",
      status: "fail",
      detail: "bajo la media de 30 semanas",
    });
    return;
  }
...
export function buildScreenerFilterExplainPlan(row = {}, filters = {}) {
  ...
  if (set.requireStage2 === true) {
    ...
    addRule(bucket, { field: "requireStage2", label: "Etapa 2", status: issue ? "fail" : "pass", detail: issue || "En etapa 2" });
  }
  if (set.requirePulso === true) {
    const issue = pulsoRejectDetail(row);
    addRule(bucket, { field: "requirePulso", label: "Pulso", status: issue ? "fail" : "pass", detail: issue || "Cumple las seis condiciones diarias" });
  }
  ...
  if (mode !== "weakness" && mode !== "ipoRecent" && set.requireStage2 !== true && set.filterStrictness !== "discovery") {
  ...
  // minRsRating filtra por el RS semanal (rs_weekly_items, universo US
  // completo), no por rsGlobalPct (percentil del lote del escaneo — con
  // lotes <20 sale null y rechazaba TODO, el bug que motivó este cambio).
  // Cuando el símbolo no está en el ranking semanal (row.weeklyRsAvailable
  // !== true), el criterio se OMITE por completo: no se añade ninguna regla,
  // ni siquiera como "sin dato" — la fila no se penaliza por un criterio que
  ...
  if (Number.isFinite(minRsRating) && minRsRating > 0 && row.weeklyRsAvailable === true) {
    addRule(bucket, explainThresholdRule({
      field: "minRsRating",
      label: "RS semanal",
      value: row.weeklyRsRating,
      threshold: minRsRating,
      direction: "min",
    }));
  }

  const minRsCountryPct = finite(set.minRsCountryPct);
  if (Number.isFinite(minRsCountryPct) && minRsCountryPct > 0 && row.weeklyCountryRsAvailable === true) {
    addRule(bucket, explainThresholdRule({
      field: "minRsCountryPct",
      label: "RS país semanal",
      value: row.weeklyCountryRsRating,
      threshold: minRsCountryPct,
      direction: "min",
    }));
  }

  const minThemeRsRating = finite(set.minThemeRsRating);
  if (Number.isFinite(minThemeRsRating) && minThemeRsRating > 0 && row.weeklyThemeRsAvailable === true) {
    addRule(bucket, explainThresholdRule({
      field: "minThemeRsRating",
      label: "RS tema semanal",
      value: row.weeklyThemeRsRating,
      threshold: minThemeRsRating,
      direction: "min",
    }));
  }

  addMinWeeksAboveSma30wRule(bucket, row, set);
...
export function screenerFilterRejectReason(row = {}, filters = {}) {
  ...
  if (set.requirePulso === true) {
    const pulsoIssue = pulsoRejectDetail(row);
    if (pulsoIssue) return reject("requirePulso", pulsoIssue);
  }
  const price = metric(row, "price");
  const sma50 = metric(row, "sma50");
  const sma200 = metric(row, "sma200");
  ...
  const vcpGate = vcpValidityGate(row, set);
  if (vcpGate) return reject(vcpGate.field, vcpGate.detail);
  if (set.requireVcpCandidate === true && row.vcpCandidate !== true) {
    return reject("requireVcpCandidate", "no es candidato VCP del motor unificado");
  }
  if (set.vcpRequireStage2 === true) {
    const stage2Issue = stage2RejectDetail(row, set);
    if (stage2Issue) return reject("vcpRequireStage2", stage2Issue);
  }
  const vcpStructure = cleanText(set.vcpWeeklyStageStructure);
  if (vcpStructure && vcpStructure !== "any") {
    const rowStructure = metricText(row, "weeklyStageStructure");
    if (rowStructure !== vcpStructure) {
      return reject("vcpWeeklyStageStructure", `estructura ${rowStructure || "sin dato"} ≠ ${vcpStructure}`);
    }
  }
  ...
  // Mismo criterio que en screenerFilterExplanation: RS semanal, omitido
  // (no rechazado) cuando el símbolo no está en el ranking.
  const minRsRating = finite(set.minRsRating);
  if (Number.isFinite(minRsRating) && minRsRating > 0 && row.weeklyRsAvailable === true) {
  ...
  if (Number.isFinite(minRsCountryPct) && minRsCountryPct > 0 && row.weeklyCountryRsAvailable === true) {
    ...
    if (!crs.available || crs.value === null || crs.value < minRsCountryPct) {
      return reject("minRsCountryPct", `RS país ${crs.available ? crs.value.toFixed(0) : "sin dato"} < ${minRsCountryPct}`);
    }
  }
  ...
  if (Number.isFinite(minThemeRsRating) && minThemeRsRating > 0 && row.weeklyThemeRsAvailable === true) {
    ...
    if (!trs.available || trs.value === null || trs.value < minThemeRsRating) {
      return reject("minThemeRsRating", `RS tema ${trs.available ? trs.value.toFixed(0) : "sin dato"} < ${minThemeRsRating}`);
    }
  }

  const persistenceReject = minWeeksAboveSma30wReject(row, set.minWeeksAboveSma30w);
  if (persistenceReject) return persistenceReject;
