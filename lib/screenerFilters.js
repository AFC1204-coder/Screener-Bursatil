...
const FIELD_RULES = {
  ...
  maxContraction1DepthPct: { op: "max", metric: "contraction1DepthPct", label: "contraccion 1" },
  maxContraction2DepthPct: { op: "max", metric: "contraction2DepthPct", label: "contraccion 2" },
  maxContraction3DepthPct: { op: "max", metric: "contraction3DepthPct", label: "contraccion 3" },
  maxLastContractionDepthPct: { op: "max", metric: "lastContractionDepthPct", label: "ultima contraccion" },
...
export function screenerFilterRejectReason(row = {}, filters = {}) {
  ...
  if (set.requireContractionsDecreasing === true && row.contractionsDecreasing !== true) return reject("requireContractionsDecreasing", "contracciones no decrecientes");
  ...
  if (mode === "nearPivot") {
    ...
    if (!([dist20, spread, ext].every(Number.isFinite) && dist20 >= -maxDist && spread <= maxSpread && ext <= maxExt)) return reject("setupMode", "no encaja en vigilancia pivot");
  }
  if (mode === "pullback") {
    const ext = metric(row, "extSma50");
    const distance52w = metric(row, "distance52w");
