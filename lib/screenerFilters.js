...
function setupModeGate(row = {}, set = {}, mode = cleanText(set.setupMode || "")) {
  if (mode === "nearPivot") {
    ...
    return setupGateItem({
      ...
      detail: `Vigilancia pivot: dist20 ${compactNumber(dist20)} >= -${compactNumber(maxDist)}, spread ${compactNumber(spread)} <= ${compactNumber(maxSpread)}, ext ${compactNumber(ext)} <= ${compactNumber(maxExt)}`,
      missing,
      ok: missing.length === 0 && dist20 >= -maxDist && spread <= maxSpread && ext <= maxExt,
  ...
  if (mode === "pullback") {
    ...
    return setupGateItem({
      ...
      detail: `Pullback SMA50: precio ${compactNumber(price)} > SMA200 ${compactNumber(sma200)}, ext ${compactNumber(ext)} entre -4 y 9, 52w ${compactNumber(distance52w)} >= -30, 6M ${compactNumber(perf6m)} >= ${compactNumber(minPerf6m)}`,
      missing,
      ok: missing.length === 0 && price > sma200 && ext >= -4 && ext <= 9 && distance52w >= -30 && perf6m >= minPerf6m,
      near: isNearBoundary(ext, -4, "min") || isNearBoundary(ext, 9, "max") || isNearBoundary(distance52w, -30, "min") || isNearBoundary(perf6m, minPerf6m, "min"),
    });
  }
  if (mode === "early") {
    const price = metric(row, "price");
  ...
  if (mode === "ipoRecent") {
    ...
    return setupGateItem({
      label: "IPO reciente",
      detail: `IPO reciente: edad ${compactNumber(age)}m <= ${compactNumber(maxIpoAgeMonths)}m, 52w ${compactNumber(distance52w)} >= -35, ext ${compactNumber(ext)} <= ${compactNumber(maxExt)}, momentum ${compactNumber(momentumScore)} >= ${compactNumber(minMomentum)}`,
      missing,
      ok: missing.length === 0 && age >= 0 && age <= maxIpoAgeMonths && distance52w >= -35 && ext <= maxExt && momentumScore >= minMomentum,
  ...
  if (mode === "extended") {
    ...
    return setupGateItem({
      ...
      detail: `Extendida fuerte: ext ${compactNumber(ext)} entre 12 y ${compactNumber(maxExt)}, momentum ${compactNumber(momentumScore)} >= ${compactNumber(minMomentum)}`,
      missing,
      ok: missing.length === 0 && ext >= 12 && ext <= maxExt && momentumScore >= minMomentum,
      near: isNearBoundary(ext, 12, "min") || isNearBoundary(ext, maxExt, "max") || isNearBoundary(momentumScore, minMomentum, "min"),
    });
  }
  return null;
}
...
export function buildScreenerFilterExplainPlan(row = {}, filters = {}) {
  ...
  if (set.setupMode === "weakness") {
    const weak = metric(row, "weaknessScore");
    const minWeakness = Math.max(35, finite(set.minWeaknessScore) ?? 0);
    addRule(bucket, explainThresholdRule({ field: "minWeaknessScore", label: "deterioro", value: weak, threshold: minWeakness, direction: "min" }));
  } else {
