...
const WEB_FILTER_PRESETS = {
  balanced: {
    ...
    minWeinsteinScore: 55,
    minMinerviniScore: 45,
    minMomentumScore: 25,
    minRiskScore: 30,
    minVolumeScore: 15,
    minLiquidityScore: 15,
    minRsRating: 55,
    minDataCoverageScore: 50,
    minTechnicalCoverageScore: 70,
    minFundamentalCoverageScore: 0,
    minAvgTurnover: 5000000,
    minMarketCap: 300000000,
    minPrice: 3,
    minAvgVolume: 300000,
    maxDistance20dHigh: 10,
    maxDistance50dHigh: 18,
    maxDistance52w: 25,
    maxDistanceATH: 40,
    maxHighsSpreadPct: 15,
    minPerf3m: 10,
    minPerf6m: 20,
    minPerf12m: 30,
    maxExtensionSma50: 30,
  },
  strict: {
    setupMode: "leader",
    requireStage2: true,
  ...
  nearPivot: {
    ...
    minPerf6m: 15,
    minPerf12m: 20,
    maxExtensionSma50: 15,
    maxDailyMove20dPct: 10,
    maxDailyRange20dPct: 14,
    maxRange63dPct: 45,
    maxVolatility63d: 55,
    maxDrawdown63d: 18,
    minRiskScore: 55,
    minWeinsteinScore: 65,
    minMinerviniScore: 55,
    minRsRating: 62,
    minTotalScore: 58,
  },
  ipo: {
...
function isStage2(row = {}) {
  if (metricText(row, "weeklyStageState")) return metricText(row, "weeklyStageState") === "stage2";
  ...
  return [price, sma50, sma150, sma200, slope].every(Number.isFinite)
    && price > sma50
    && price > sma150
    && price > sma200
    && sma50 > sma150
...
export function screenerFiltersFromParams(params = {}) {
  ...
  for (const key of SCREENER_FILTER_QUERY_KEYS) {
    ...
    values[key] = BOOLEAN_KEYS.has(key) ? boolValue(raw) : key === "setupMode" ? cleanText(raw) : finite(raw);
    explicit.push(key);
  }
...
export function screenerFilterRejectReason(row = {}, filters = {}) {
  const set = filters.values || filters || {};
  ...
  if (set.requireStage2 === true && !isStage2(row)) return reject("requireStage2", "no cumple Stage 2");
  const price = metric(row, "price");
  const sma50 = metric(row, "sma50");
  const sma200 = metric(row, "sma200");
