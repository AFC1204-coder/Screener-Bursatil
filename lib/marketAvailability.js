...
import { countRowsByMarket } from "@/lib/lideresIntlGuardrail";
...
/** Mínimo de mercados en mesa para listar conteo por mercado en truth line. */
export const MULTI_MARKET_ROW_COUNT_MIN_MARKETS = 2;

/** Máximo de mercados listados en desktop antes de «…». */
export const MULTI_MARKET_ROW_COUNT_MAX_LIST = 6;

/** En compacto, conteo por mercado solo si la mesa tiene ≤ este número. */
export const MULTI_MARKET_ROW_COUNT_COMPACT_MAX = 3;
...
/**
 * Conteos de filas analizadas por mercado de mesa (country / símbolo).
 * null si la mesa tiene <2 mercados o no hay filas con mercado identificable.
 */
export function analyzedRowsCountByScannedMarket(rows = [], scannedMarkets = []) {
  const scanned = normalizeMarketList(scannedMarkets, []).slice().sort();
  if (scanned.length < MULTI_MARKET_ROW_COUNT_MIN_MARKETS) return null;
  const counts = countRowsByMarket(rows);
  const entries = scanned.map((code) => [code, counts.get(code) || 0]);
  const total = entries.reduce((sum, [, count]) => sum + count, 0);
  if (!total) return null;
  return entries;
}

/** Segmento compacto «GB 23 · DE 40 · …» a partir de pares [código, conteo]. */
export function formatPerMarketRowCountSegment(entries = [], { compact = false } = {}) {
  if (!Array.isArray(entries) || !entries.length) return null;
  const maxVisible = compact ? MULTI_MARKET_ROW_COUNT_COMPACT_MAX : MULTI_MARKET_ROW_COUNT_MAX_LIST;
  const visible = entries.slice(0, maxVisible);
  const parts = visible.map(([code, count]) => `${code} ${count}`);
  const truncated = entries.length > maxVisible;
  return truncated ? `${parts.join(" · ")} · …` : parts.join(" · ");
}

/**
 * % de mercados de la selección presentes en mesa (no universo FIRDS).
 * null si no aplica: selección <2, cobertura total, o sin selección explícita.
 */
export function describeSelectionMarketsOnMesa({
  scannedMarkets = [],
  selectedMarkets = [],
} = {}) {
  const scanned = normalizeMarketList(scannedMarkets, []).slice().sort();
  const selected = normalizeMarketList(selectedMarkets, []).slice().sort();
  if (selected.length < MULTI_MARKET_ROW_COUNT_MIN_MARKETS) return null;
  const scannedSet = new Set(scanned);
  const presentCount = selected.filter((code) => scannedSet.has(code)).length;
  if (!presentCount || presentCount >= selected.length) return null;
  const pct = Math.round((presentCount / selected.length) * 100);
  const fraction = `${presentCount}/${selected.length}`;
  return {
    presentCount,
    selectedCount: selected.length,
    pct,
    truthSegment: `${fraction} mercados en mesa (${pct}%)`,
    peekDetail: `${pct}% selección en mesa`,
    compactSegment: `${pct}% selección en mesa`,
  };
}

function appendTruthMarketDetailSegments(segments, {
  analyzedRows = [],
  scannedMarkets = [],
  selectedMarkets = [],
  compact = false,
} = {}) {
  const rowCountSegment = formatPerMarketRowCountSegment(
    analyzedRowsCountByScannedMarket(analyzedRows, scannedMarkets),
    { compact },
  );
  if (rowCountSegment) segments.push(rowCountSegment);

  const selectionPresence = describeSelectionMarketsOnMesa({
    scannedMarkets,
    selectedMarkets,
  });
  if (selectionPresence) {
    segments.push(compact ? selectionPresence.compactSegment : selectionPresence.truthSegment);
  }
  return segments;
}
...
export function buildScreenerTruthMarketSegments({
  ...
  if (curatedGap && !europeGap) {
    ...
    const segments = compact
      ...
      : [mesaSegment, curatedGap.truthSegment];
    return appendTruthMarketDetailSegments(segments, detailContext);
  }
  ...
  if (misaligned) {
    const segments = europeGap
      ? [
        `Mostrando ${mesaLabel} · tu selección es ${seleccionLabel || "Europa"}`,
        europeGap.truthSegment,
      ]
      : [`Mostrando ${mesaLabel} · tu selección es ${seleccionLabel || "otra"}`];
    return appendTruthMarketDetailSegments(segments, detailContext);
  }
