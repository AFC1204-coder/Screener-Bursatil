...
export function buildScreenerTruthMarketSegments({
  ...
  if (misaligned && (
    scanned.length > MULTI_MARKET_COMPACT_THRESHOLD
    || selected.length > MULTI_MARKET_COMPACT_THRESHOLD
  )) {
    const segments = scanned.length <= MULTI_MARKET_COMPACT_THRESHOLD
      ? [`mesa: ${formatMarketCodesShort(scanned)}`]
      : [`${marketCountPhrase(scanned.length)} en mesa`];
    if (selected.length > MULTI_MARKET_COMPACT_THRESHOLD) {
      segments.push(`${marketCountPhrase(selected.length)} en selección`);
    } else {
      segments.push(`selección: ${formatMarketCodesShort(selected)}`);
    }
    segments.push("selección ≠ mesa");
    return segments;
  }
...
export function buildMarketsStaleNotice({
  ...
  if (marketsSelectionPartialCoverage(scannedMarkets, selectedMarkets)) {
    ...
    const detail = `Datos cargados: ${scannedCodes}${rowSuffix}. Faltan en mesa: ${missingLabels}.`;
    return {
      tone: "warn",
      label: "Cobertura parcial",
      detail,
      peekDetail: missingMarketsPeekDetail(missingCodes),
      bodyDetail: detail,
