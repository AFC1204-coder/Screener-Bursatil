...
export const MARKETS_MISALIGNMENT_EMPTY_LABEL = "La selección de mercados no coincide con los datos cargados. Pulsa «Cargar datos de la selección» arriba para ver resultados de tu mercado.";
...
export function buildScreenerTruthMarketSegments({
  ...
  const misaligned = marketsMisaligned || marketsSelectionMisaligned(scannedMarkets, selectedMarkets);
  if (misaligned) {
    const selected = normalizeMarketList(selectedMarkets, []).slice().sort();
