...
const RULE_FIELDS = [
  ...
  // Ancla «desde salida» persistida en scan (IPO-UX-D2): escalar, no ampliar chartPreview.
  "ipoAnchorClose", "ipoAnchorDate",
];
...
const BOOLEAN_RULE_FIELDS = [
  ...
  // Subestado estructural (STAGE-1): la columna Etapa, la ficha y la vista
  // rápida leen el calificador pre-fuga / con fuga. Sin estos campos, las
  // filas ligeras seguirían mostrando «Etapa 2» a secas.
  "weeklyStageStructure", "weeklyStageStructureLabel", "weeklyStageStructureDetail",
  "weeklyResistance", "weeklyResistanceDate", "weeklyDistResistancePct",
  "weeklyRng26Pct", "weeklyRuptura", "weeklyHhHl",
  "weeklyBreakoutVolRatio", "weeklyBreakoutVolWeek", "weeklyBreakoutVolDetail",
  ...
  "vcpCandidate",             // requireVcpCandidate + columna VCP
];
...
const TABLE_FIELDS = [
  ...
  "website", "logoDomain",                              // CompanyMark (favicon / Clearbit)
  "theme",                                              // 2 Tema
...
// ── Sostén de la tendencia (MET-4b/4c) ───────────────────────────────────
// Contadores de persistencia y avances derivados. La ficha puede recalcularlos
// desde barras; la vista rápida solo tiene la fila del scan. Sin estos campos
// en la proyección ligera, el modal muestra «Sin dato» en persistencia aunque
// el nocturno los haya calculado (MET-4c smoke 2026-08-31).
const TREND_SUPPORT_FIELDS = [
  "weeksAboveSma30w", "weeksAboveSma30wAbove",
  "weeksAboveSma10w", "weeksAboveSma10wAbove",
  "advanceRecentPct", "advancePriorPct",
];
...
// Campos de cribado / auditoría resumida que viajan fuera de SCAN_LIGHT_FIELDS
// pero sí debe conservar el wire de mesa (el nocturno los escribe en metrics).
const MESA_TRANSPORT_EXTRA_FIELDS = [
  "screenPassed",
  "rowProjection",
  "screenRejectField",
  "screenRejectReason",
...
// Narrativa display-only: está en SCAN_LIGHT (ficha / persistencia / nocturno) pero
// no la consultan FIELD_RULES, GATE_INPUT_FIELDS ni celdas de orden de la mesa.
// Omitirla del compacto HTTP recorta el JSON que el cliente parsea en cold path
// (~20,5 MB residual post chartPreview-defer). ?full=1 sigue llevando todo.
//
// SCREENER-COLD-PAYLOAD-1: setupDisplayReason era el mayor campo residual tras
// mesa-light-wire (~2 MB / 3574 filas con narrativa poblada); setupDisplay*
// confidence y setupVerdictShortLabel suman ~150 KB más. La mesa y los filtros
// usan setupDisplayLabel/Key/State; explainability y ficha tienen fallback.
export const MESA_TRANSPORT_OMIT_FIELDS = [
...
const MESA_TRANSPORT_OMIT = new Set(MESA_TRANSPORT_OMIT_FIELDS);

function shouldCopyMesaTransportField(row, key) {
  if (!row || typeof row !== "object" || !(key in row)) return false;
  const value = row[key];
  // null explícito (p.ej. weeklyCountryRsRating tras core hydrate) se conserva;
  // string/array vacíos no aportan al parse y se omiten del wire.
...
/**
 * Proyección de wire para GET /api/scans compacto (mesa `/`).
 *
 * Tras diferir chartPreview (~20,5 MB residual WAVE5), el cold path sigue
 * parseando (1) filas "full" con raw gordo y (2) narrativa display-only en
 * todas las filas. Allowlist = SCAN_LIGHT + marcas de cribado; además omite
 * MESA_TRANSPORT_OMIT_FIELDS y strings/arrays vacíos. No pisa screenPassed.
 *
 * @param {object} row fila ya rehidratada (RS / marketCap attached)
 * @param {{ omitChartPreview?: boolean }} options
 */
export function projectScanRowForMesaTransport(row = {}, options = {}) {
  if (!row || typeof row !== "object") return row;
  const omitChartPreview = options.omitChartPreview !== false;
  const out = {};
  for (const key of SCAN_LIGHT_FIELDS) {
    if (omitChartPreview && key === "chartPreview") continue;
    if (MESA_TRANSPORT_OMIT.has(key)) continue;
    if (!shouldCopyMesaTransportField(row, key)) continue;
    out[key] = row[key];
  }
  if (!omitChartPreview && Array.isArray(out.chartPreview)) {
    out.chartPreview = compactChartPreview(out.chartPreview);
  }
  for (const key of MESA_TRANSPORT_EXTRA_FIELDS) {
    if (!shouldCopyMesaTransportField(row, key)) continue;
    out[key] = row[key];
  }
  if (!out.metricAuditFlags) {
    const flags = metricAuditFlags(row);
    if (flags) out.metricAuditFlags = flags;
  }
  return out;
}
