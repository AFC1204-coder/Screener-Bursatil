// Detector v5 — v4 más la ZONA DE SALIDA de v3.
//
// LA REGLA DEL CONTACTO CON EL MÁXIMO (pedida por el dueño):
// La base NO termina cuando el precio toca el máximo de referencia. Producción
// sí la corta ahí (`ceiling_break`, setupPatterns.js:380, salta al +4% sobre el
// techo) y por eso no puede ver una base ascendente. La v4 ya permitía el
// contacto; lo que faltaba era definir CUÁNDO TERMINA DE VERDAD.
//
// DEFINICIÓN, y por qué esta: la base termina cuando el precio SALE DE FORMA
// SOSTENIDA de una banda alrededor del rango — dos cierres consecutivos fuera —
// y caduca si esa salida ocurrió hace más de 10 sesiones. Un toque, o incluso
// un máximo nuevo que se queda dentro de la banda, NO la termina.
//
// Los parámetros de la banda NO se ajustan aquí: son los de v3, calibrados
// sobre 48.774 rupturas y 39.818 perforaciones del universo líquido. Usarlos
// evita ajustar umbrales sobre los 21 casos etiquetados, que es lo que había
// que evitar.
//
...
const P = {
  ...
  usarBanda: true,        // zona de salida de v3
  exitConfirmBars: 2,     // dos cierres consecutivos fuera
  exitExpiraBars: 10,     // si la salida fue hace más de 10 sesiones, caducó
...
export function detectV5(bars, opts = {}) {
  const p = { ...P, ...opts };
  const n = bars.length;
  const last = n - 1;
  ...
  // ── ¿Dónde termina la base? Zona de salida de v3 ────────────────────────
  // Un contacto con el techo, o un máximo nuevo dentro de la banda, NO la
  // termina. La termina salir de forma sostenida.
  const techoSeq = Math.max(...seq.map((s) => s.hi.p));
  const sueloSeq = Math.min(...seq.map((s) => s.lo.p));
  const profSeq = (techoSeq - sueloSeq) / techoSeq * 100;
  let salida = null;
  if (p.usarBanda && profSeq > 0) {
    ...
    let uc = 0, dc = 0;
    for (let i = seq[0].hi.i + 1; i <= last; i++) {
      uc = bars[i].c > hiLim ? uc + 1 : 0;
      dc = bars[i].c < loLim ? dc + 1 : 0;
      if (uc >= p.exitConfirmBars) { salida = { i, tipo: "ruptura", up, dn }; break; }
      if (dc >= p.exitConfirmBars) { salida = { i, tipo: "rotura", up, dn }; break; }
    }
    if (salida && (last - salida.i) > p.exitExpiraBars) {
      return rejL("fuera_de_rango", { ...ctx, salida: bars[salida.i].d, tipo: salida.tipo,
        haceSesiones: last - salida.i, banda: `+${up.toFixed(1)}/-${dn.toFixed(1)}%` });
    }
  }
  ...
  return {
    ...
    salida: salida ? `${bars[salida.i].d} (${salida.tipo}, hace ${last - salida.i})` : null,
  };
}
