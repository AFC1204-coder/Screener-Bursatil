...
describe("legacy flag OFF (Supabase)", () => {
  const mustReject = [
    "NDAQ-2025-11",
    "ELV-2026-08",
    "MSGS-2026-08",
  ...
  it("flag OFF (legacy) estable en anclas golden sin romper vcpCandidate", async () => {
    const { supabaseRequestAll, supabaseConfig } = await import("@/lib/supabaseServer");
    const { toResearchBars, evaluateAtAsOf } = await import("../research/contracciones/arneses/rubric-gap.mjs");
    const cfg = supabaseConfig();
    if (!cfg.configured) return;
    ...
    for (const id of targets) {
      ...
      const legacyDefault = evaluateAtAsOf(toResearchBars(rows), evalCase.evalAsOf ?? evalCase.asOf);
      const legacyExplicit = evaluateAtAsOf(toResearchBars(rows), evalCase.evalAsOf ?? evalCase.asOf, { vcpUnified: false });
      const row = buildResultRow(evalCase, legacyExplicit);

      expect(legacyExplicit.prod.vcpCandidate, id).toBe(legacyDefault.prod.vcpCandidate);
