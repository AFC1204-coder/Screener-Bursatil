...
--
-- Average calculations intentionally remain double precision. Runtime-specific
-- floating-point serialization is handled with tolerance in the parity test.
  ...
  'byCountry', coalesce((
    select jsonb_agg(
      jsonb_build_object(
        ...
        when p_include_top then jsonb_build_object('topSymbols', bc.top_symbols)
        else '{}'::jsonb
      end
  ...
  'bySector', coalesce((
    select jsonb_agg(
      jsonb_build_object(
        ...
        when p_include_top then jsonb_build_object('topSymbols', bs.top_symbols)
        else '{}'::jsonb
      end
    ...
    from (
      select *
      from by_sector
      order by unique_symbols desc, ranking_eligible desc, key
      limit 25
    ) as bs
  ), '[]'::jsonb)
)
